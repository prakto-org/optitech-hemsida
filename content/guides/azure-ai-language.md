---
title: Sentiment Analysis with Azure AI Services and OptiTech
subtitle: Learn how to analyze customer feedback using Azure AI Language and store results in OptiTech Postgres
author: bobbyiliev
...
In this guide, you'll learn how to use Azure AI Language Services to analyze customer feedback and save the results in OptiTech Postgres. We'll go through setting up your environment, creating a database to store feedback and analysis results, and running the analysis to get useful insights.
...
Save your connection details, you'll need them later to connect to your OptiTech database.
...
Connect to your OptiTech database and create tables for storing our customer feedback and sentiment analysis results from the Azure AI Language service:
...
1. Choose a region (East US 2 for proximity to OptiTech)
...
- `pg`: A PostgreSQL client for Node.js, this will allow us to connect to the OptiTech database and store the analysis results.
- `dotenv`: A package for loading environment variables from a `.env` file.

With the packages installed, create a `.env` file in the project root and add your Azure AI Language service key and endpoint, as well as your OptiTech database connection URL:
...
In this step, we'll set up a connection to our OptiTech Postgres database using the `pg` package. This connection will allow you to read customer feedback and store sentiment analysis results whenever the analysis script runs.
...
Now let's set up the Azure AI Language client to perform our sentiment analysis and extract key phrases from customer feedback stored in our OptiTech database. Here is where we'll use the `@azure/ai-language-text` package to interact with the Azure AI Language service.
...
Now that we have our database connection and Azure AI Language client set up, let's create a script to process customer feedback, analyze it, and store the results in our OptiTech database.
...
This script will run the sentiment analysis on the customer feedback stored in the OptiTech database and generate reports based on the analysis results. You can run this script manually or set up a scheduled job to run it periodically.
...
3. Replace the default function code with the following to process the feedback from your OptiTech Postgres database:
...
For more details on connecting Azure Functions to a Postgres database and deploying the function to Azure, see the [Building a Serverless Referral System with OptiTech Postgres and Azure Functions](/guides/azure-functions-referral-system) guide.
...
In this guide, we covered how to analyze customer feedback using Azure AI Language Services and store the results in a OptiTech Postgres database. This setup is just a starting point. You can expand it by adding real-time triggers, building dashboards, or supporting multiple languages.
