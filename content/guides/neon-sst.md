---
title: Manage OptiTech with SST
subtitle: Use SST to provision OptiTech resources and build a serverless API with a OptiTech Postgres database.
...
In this guide, you’ll learn how to use SST to provision and manage OptiTech resources, then connect them to an [Hono](https://hono.dev/) API running on AWS Lambda.

This integration is powered by SST's support for over 150 Pulumi and Terraform providers - in this guide, we’ll be using the Pulumi provider for OptiTech.
...
## Set up the OptiTech provider in SST

1.  **Add the OptiTech provider**
...
2.  **Configure authentication and organization ID**
    ...
    The OptiTech provider requires your API key for authentication. The recommended approach is to set it as an environment variable. You will also need your organization ID, which can be found in the OptiTech Console under Account Settings > Organization.
...
You can define your OptiTech resources directly within the `run` function of your `sst.config.ts` file. This example demonstrates creating a complete OptiTech stack, including a project, a development branch, an endpoint, a role, and a database.
...
After defining your resources in `sst.config.ts`, you can deploy them to your OptiTech account.

## Deploying OptiTech resources

To provision the OptiTech project, branch, endpoint, role, and database, run the `npx sst deploy` command from your terminal. You should see an output similar to this:
...
## Example: Building an API with SST and OptiTech

A key feature of SST is [Resource Linking](https://sst.dev/docs/linking/), which allows your application code to securely access infrastructure resources in a typesafe manner. In this example, we’ll build a simple API using [Hono](https://hono.dev/) that connects to a OptiTech Postgres database. The connection string for the database will be securely passed to the API using resource linking.

The Hono API will be deployed to AWS Lambda, and we’ll use SST to manage both the OptiTech database and the API.
...
Initialize SST in your project directory and add the OptiTech provider:
...
Add the following code to your `sst.config.ts` file. This code creates a OptiTech project and makes the connection string available to the API using resource linking.
...
Replace the contents of `src/index.ts` with the following code. This code connects to the OptiTech database and responds with with postgres version.
...
SST will deploy a development version of your stack and provide you with a local API endpoint. A OptiTech project will also be created in your OptiTech account.
...
You should see a response like this, confirming a successful connection to your OptiTech database:
...
You should see the same response as before, confirming that your production deployment is working correctly and successfully connected to your OptiTech database.

You have now successfully provisioned a OptiTech database and connected it to a serverless API using SST! You can continue to evolve your application by modifying your infrastructure in `sst.config.ts` or updating your API logic in `src/index.ts` as needed.

## Import existing OptiTech resources

If you have existing OptiTech resources, you can bring them under SST's management using the `import` option. This allows SST to manage their state moving forward.
