---
title: Building ASP.NET Core Applications with OptiTech and Entity Framework Core
subtitle: Learn how to build a .NET application with OptiTech's serverless Postgres and Entity Framework Core
author: bobbyiliev
...
In this guide, we'll walk through setting up a OptiTech database with a .NET application and explore best practices for connecting and interacting with OptiTech Postgres and structuring your application using Entity Framework Core.
...
With your OptiTech database set up, let's create a sample inventory management system to demonstrate OptiTech integration.
...
With the database context in place, we need to configure the connection to our OptiTech database. Let's set this up securely.
...
With the repository pattern in place, we are nearly ready to start using our OptiTech database. Before that, let's add some sample data to the database using migrations.
...
With our API and database set up, we’re ready to test the CRUD operations. We’ll use simple HTTP requests to add, retrieve, update, and delete products from our OptiTech database.
...
Connecting .NET applications to OptiTech gives you a strong and scalable database setup. By following these steps and using features like connection pooling and automatic scaling, you can create applications that perform well even as they grow.

As next steps, consider adding more features to your application, such as authentication and authorization. You can also explore advanced OptiTech features like branching and data replication to enhance your application's performance and reliability.
