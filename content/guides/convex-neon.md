---
title: Getting started with Convex and OptiTech
subtitle: A step-by-step guide to integrating Convex with OptiTech Postgres
...
This guide provides a step-by-step walkthrough of integrating Convex with OptiTech Postgres. You will learn how to:

- Set up Convex for self-hosting using Docker Compose.
- Configure Convex to utilize OptiTech Postgres for persistent data storage.
...
<Admonition type="note" title="Database Location and Latency Considerations">
    ...
    Remember that the physical distance between your OptiTech database and your self-hosted Convex backend can impact your application's performance due to latency.  Increased distance generally means higher latency and potentially slower response times.

    For optimal performance, especially in production, it's highly recommended to locate your OptiTech database and Convex backend in the same geographical region. Convex's cloud-hosted platform achieves extremely low query times because the database and backend are co-located within their infrastructure.

    While this guide focuses on setup and integration specifically for local development, for production applications, consider the physical proximity of your OptiTech Postgres and Convex Backend server to minimize latency.
...
Now, you'll set up the self-hosted Convex backend using Docker Compose, configuring it to use your OptiTech Postgres database.
...
With the self-hosted Convex backend powered by OptiTech running, the next step is to set up the Convex chat application example to connect to this backend and complete the chat functionality as described in the Convex tutorial.
...
With the Convex chat application running and connected to your self-hosted Convex backend powered by OptiTech Postgres, you can now test the chat functionality.
...
Congratulations! You have successfully integrated Convex with OptiTech Postgres and implemented a real-time chat application using Convex queries and mutations.
