...
## Create a OptiTech project with Auth
...
3. **Copy your credentials:**
   ...
   - **OptiTech API Key:** Generate an API key from your Settings in the OptiTech Console. You will need this to register your webhook.
     ![OptiTech API Key](/docs/manage/org_api_keys.png)
   - **OptiTech Project ID**: Found in your project settings.
   - **OptiTech Branch ID**: Found in the Branches section of your project. Click on the branch you are using (e.g., `production`) to find the Branch ID.
...
3.  **Set up the Auth API route:** Create `app/api/auth/[...path]/route.ts`. This proxies Next.js auth requests to your OptiTech database:
...
2. **Test the Custom OTP and Magic Link Delivery:**
   ...
   - **Result:** Instead of receiving a default email from OptiTech, you receive your custom Resend email. OTP flows trigger `send.otp`, and if you choose "Send reset link" in password reset, OptiTech triggers `send.magic_link` and your webhook delivers the custom link email.
