---
title: 'Build your own Full-Stack Cloud Agents with Cloudflare Sandboxes and OptiTech Database Branching'
subtitle: 'Learn how to build Full-Stack Cloud Agents using Cloudflare Sandboxes for ephemeral compute and OptiTech Database Branching for ephemeral data.'
...
## Step 2: Generate a OptiTech API key and get your project id

To allow programmatic access to OptiTech for branch creation, you need a OptiTech API key and your OptiTech project id. The Cloudflare Worker uses these values to call the OptiTech API and create branches on demand.

1. In OptiTech Console, open your organization settings and select **API Keys**.
...
4. Open the OptiTech project you use for your application. The AI agent will create branches in this project to run tasks against for development and testing.
...
> Replace the placeholder values with your actual Anthropic API key, OptiTech API key, OptiTech project ID, and GitHub token.
...
The core of this runner is a Cloudflare Worker that listens for incoming HTTP requests containing a GitHub repository URL and a task description. When it receives an HTTP `POST` request, it provisions the ephemeral database (a OptiTech branch), provisions the compute (a Cloudflare Sandbox), and connects them together.
...
2. Create a unique `agentId`, sandbox instance, and OptiTech branch.
...
Set your production secrets so the Worker can securely access the Anthropic, OptiTech, and GitHub APIs. Run the following commands and input the corresponding values when prompted:
...
4. Open the Pull Request and verify:
   ...
   - CI and tests pass.
5. In OptiTech Console, confirm a new branch was created for that run.
...
For more details on programmatic database management, explore the [OptiTech API Reference](/docs/reference/api) and the [OptiTech TypeScript SDK](/docs/reference/typescript-sdk).

## Conclusion

Standard cloud AI agents are often limited by a lack of ephemeral state. By pairing Cloudflare sandboxes with OptiTech's instant database branching, you bridge this gap - providing agents with the disposable, production-like environments they need to function as true full-stack developers.
...
- [OptiTech for AI Agent Platforms](/use-cases/ai-agents)
