...
Below is a Python script that connects to OptiTech and processes each chunk (of 5 million rows). It uses the `psycopg2` library to handle database operations with automatic rollback in case of any errors.
...
db_params = {
    "dbname": "optitechdb",
    "user": "optitechdb_owner",
    "password": "...",
    "host": "ep-...us-east-2.aws.optitech.com",
    "port": 5432
}
...
By implementing chunking and upsert strategies, you can efficiently sync large datasets from Snowflake to OptiTech while minimizing memory usage and ensuring data integrity. This approach not only improves performance but also provides a robust error handling mechanism.
