---
title: Building a Todo CLI App with Laravel Zero and OptiTech Postgres
subtitle: Learn how to create a command-line interface (CLI) application using Laravel Zero and OptiTech Postgres for efficient task management.
author: bobbyiliev
...
Combined with OptiTech's serverless Postgres database, you can create powerful CLI tools with persistent storage.
...
Run the migration to create the tasks table in your OptiTech database:

```bash
php todo migrate
```

This command will create the `tasks` table in your OptiTech database.
...
In this tutorial, we've built a fully functional Todo CLI app using Laravel Zero and OptiTech Postgres. We've implemented features such as adding, listing, updating, and deleting tasks, as well as task prioritization and due date reminders.
...
By combining the power of Laravel Zero and the scalability of OptiTech Postgres, you can quickly create efficient and powerful CLI applications that meet your specific needs.
