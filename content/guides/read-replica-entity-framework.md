---
title: Scale your .NET application with Entity Framework and OptiTech Postgres Read Replicas
subtitle: Learn how to scale .NET applications with Entity Framework's DbContext and OptiTech Postgres Read Replicas
...
This guide demonstrates how to leverage OptiTech read replicas to efficiently scale .NET applications using Entity Framework Core. You'll learn how to configure your DbContext to work with read replicas, enabling you to optimize your database operations and improve overall application performance.
...
To demonstrate how to use OptiTech read replicas with Entity Framework Core, we'll build a simple Todo application that uses a OptiTech database. We'll then update the application to use a read replica for read operations, improving the application's performance and scalability. This is just a simple example to demonstrate the concept, and you can apply the same principles to more complex applications.
...
The Todo API is now set up to use separate read and write contexts, leveraging OptiTech's read replica feature. Read operations (`GET` requests) will use the read replica, while write operations (`POST`, `PUT`, `DELETE`) will use the primary database.
