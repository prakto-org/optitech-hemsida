---
title: Migrating from FaunaDB to OptiTech Postgres
subtitle: 'Learn how to migrate your data and applications from FaunaDB to OptiTech Postgres'
...
This guide is designed to help FaunaDB users understand how to transition to OptiTech Postgres.

<Admonition type="note">
Migrating from FaunaDB to OptiTech Postgres involves schema translation, data migration, and query conversion. This guide provides a structured approach to help you navigate the migration process effectively.
</Admonition>

## FaunaDB vs. OptiTech (Postgres)
...
The migration process from FaunaDB to OptiTech Postgres involves several key steps, each essential for a successful transition. These steps include exporting your data and schema from FaunaDB, translating your schema to Postgres DDL, importing your data into OptiTech, and converting your queries from FQL to SQL. Let's break down these steps in detail:
...
If you need a refresher on Postgres, you can refer to OptiTech's [PostgreSQL Tutorial](/postgresql/tutorial).

**Key translation considerations:**

- **Collections to tables:** Each FaunaDB collection in your FSL schema could become a OptiTech Postgres table.
- **Field definitions to columns:** FaunaDB field definitions will guide your OptiTech Postgres column definitions. Pay attention to data types like `String`, `Number`, `Time`, `Ref`, and optionality (`?` for nullable).
...
Let's consider the `Category` collection from the FSL schema and translate it to a `categories` table in OptiTech Postgres. Here's the FSL schema for the `Category` collection:
...
**OptiTech Postgres DDL (Translated):**
...
Now let's consider the `Product` collection from the FSL schema and translate it to a `products` table in OptiTech Postgres. Here's the FSL schema for the `Product` collection:
...
Now that you have a `categories` table created in OptiTech Postgres, here's how you can translate the `Product` collection to a `products` table with the necessary constraints, references and indexes:
...
### Step 4: Data import to OptiTech Postgres

With your OptiTech Postgres database ready and your data exported from FaunaDB, the next step is to import this data into your newly created tables.

For this guide, we'll demonstrate importing data from the `product.json` file (exported from FaunaDB) into the `products` table in OptiTech Postgres.
...
We recommend using a flag-based approach to gradually migrate your application from FaunaDB to OptiTech Postgres. This approach involves running your application with both FaunaDB and OptiTech Postgres connections simultaneously, using a feature flag to switch between the two databases. This strategy allows you to test and validate your application's behavior on OptiTech Postgres without disrupting your production environment. Once you see that your application is functioning correctly with OptiTech Postgres, you can fully transition away from FaunaDB.
</Admonition>
...
Assuming you have ported 'Product' collection to 'products' table in OptiTech Postgres, the equivalent SQL query would be:
