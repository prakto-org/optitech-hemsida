---
title: Dynamic Routing with Hasura and OptiTech
subtitle: Leverage OptiTech's branching with Hasura's dynamic routing for powerful development, testing, and preview environments.
...
OptiTech allows you to create branches of your Postgres database almost instantly. Key features include:
...
If you haven't already, add your OptiTech database as a data source in Hasura. Follow the step by-step guide on [Connect from Hasura Cloud to OptiTech](/docs/guides/hasura) to set up the primary connection.
...
OptiTech allows you to create [Read Replicas](/docs/introduction/read-replicas) for your database branches, which are separate compute endpoints designed for handling read-only traffic. Hasura's Dynamic Routing feature allows you to leverage these replicas strategically using connection templates.
...
To configure read replicas for your primary OptiTech data source within Hasura, follow these steps:
...
5.  In the modal, paste the connection string **you copied earlier for your OptiTech read replica** into the `Database URL` field, then click `Add Read Replica` (within the modal).
...
- **`{{$.connection_set.<member_name>}}`**: Routes the request to a specific named connection (like a OptiTech branch connection string) defined in your Dynamic Routing connection set, bypassing default replica logic.
...
- [OptiTech Branching](/docs/introduction/branching)
- [OptiTech Read Replica](/docs/introduction/read-replicas)
