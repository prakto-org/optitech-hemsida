---
title: Building a Real-Time Task Board with Laravel, OptiTech, and WebSockets
subtitle: Learn how to create a collaborative task management system using Laravel, OptiTech Postgres, and WebSockets
author: bobbyiliev
...
We'll build a collaborative task board where team members can create, update, and move tasks in real-time. By the end of this guide, you'll understand how to set up WebSockets in Laravel, store and retrieve data using OptiTech Postgres, and broadcast updates to connected clients instantly.
...
To get started we will need to create a new Laravel project and configuring it with OptiTech Postgres.
...
2. Configure the OptiTech database connection. Open your `.env` file and update the database settings:
   ...
   Replace the placeholders with your OptiTech database details which you can find in the OptiTech console.
...
Now that we've set up the Laravel project and connected it to OptiTech Postgres, let's create the task board.
...
3. Run the migration:
   ...
   This will create the `tasks` table in your OptiTech Postgres database with the specified columns and constraints in the migration file.
...
2. In another terminal, start the Laravel queue worker to process the broadcast events:
   ...
   To learn more about Laravel queues, check out the [Implementing Queue Workers and Job Processing in Laravel with OptiTech Postgres](/guides/laravel-queue-workers-job-processing) guide.
...
2. The server saves the task in the OptiTech Postgres database.
...
3. **Database Indexing**: Add indexes to frequently queried columns in your OptiTech Postgres database to speed up queries. For more information, check out the OptiTech Postgres documentation on [Indexes](/docs/postgresql/index-types).
...
In this guide, we've built a simple real-time collaborative task board using Laravel, OptiTech Postgres, and WebSockets. This example shows how you can create interactive, real-time web applications that update instantly across multiple users using Laravel's broadcasting feature.
