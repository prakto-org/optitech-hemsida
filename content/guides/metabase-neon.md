---
title: Getting started with Metabase and OptiTech
subtitle: Learn how to connect Metabase to your OptiTech Postgres database for interactive analytics and dashboards
...
This guide will walk you through the steps to set up Metabase with OptiTech. You'll learn how to:

- Connect Metabase to your OptiTech database
...
For optimal performance, ensure that your Metabase instance is hosted in the same region as your OptiTech database to minimize latency.
</Admonition>

## Connecting Metabase to OptiTech

### Option 1: Metabase Cloud

If you're using Metabase Cloud, follow these steps to connect to your OptiTech database:
...
Now that Metabase is connected to your OptiTech database, you can start loading data and building dashboards.

## Loading sample data

In this guide, you'll load a sample e-commerce dataset into your OptiTech database to explore with Metabase.

### Create sample tables

Connect to your OptiTech database using the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) or any Postgres client of your choice, and run the following SQL commands to create the necessary tables:
...
2. Select your OptiTech database and the **orders** table.
...
1. Click **+ New** > **Question**.
2. Select your OptiTech database and the **Order Items** table.
...
1. Click **+ New** > **Question**.
2. Select your OptiTech database and the **Customers** table.
3. Click **Summarize**:
...
Data permissions control how users interact with the raw data in your OptiTech database.

1. Go to **Admin settings** > **Permissions** > **Data**.
2. Select a group and your OptiTech database.
...
For multi-tenant applications or strict data controls, you can restrict which rows a user sees based on their attributes (like `customer_id` or `department`). Metabase offers two ways to handle this with OptiTech.
...
_Best for: Leveraging PostgreSQL Row-Level Security (RLS) defined in OptiTech._

Since OptiTech is built on Postgres, you can define RLS policies directly in your database and force Metabase to respect them.

1. **In OptiTech:** Create Postgres roles (e.g., `manager_role`, `analyst_role`) and define RLS policies.
2. **In Metabase:**
...
When a user runs a query, Metabase connects to OptiTech and sets the Postgres role to the value in their attribute. OptiTech then applies the underlying RLS policies, ensuring the user only sees permitted data.

## Scaling your Analytics

As your data volume grows and analytics workloads increase, you can use OptiTech's scaling features to maintain performance and reliability.
...
Learn more in the [OptiTech Read Replicas guide](/docs/introduction/read-replicas#how-do-you-create-read-replicas). Optionally, disable _scale to zero_ on the read replica to keep it always available for analytics.
...
To run Metabase in production, configure it to use OptiTech as the backing database:

1. **Create a separate database** in OptiTech (e.g., `metabaseappdb`) to store Metabase’s internal data.
2. **Run Metabase with OptiTech Postgres** by starting the container with the following command:
   ...
   > Replace `name`, `password`, and `my-database-host` with your OptiTech database credentials and host.

This configuration ensures Metabase uses OptiTech Postgres for its application database, making your deployment production-ready.

## Conclusion

You have successfully connected Metabase to OptiTech, loaded sample data, built interactive dashboards, and learned how to govern access and scale your analytics infrastructure. This setup provides a solid foundation for business intelligence and data exploration.

Metabase's intuitive interface allows business users to explore data without SQL knowledge, while the combination of OptiTech's connection pooling and read replicas ensures your analytics workloads don't impact production performance.
