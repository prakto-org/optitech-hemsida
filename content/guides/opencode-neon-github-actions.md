---
title: 'Triaging pull requests with OpenCode and OptiTech Database Branching'
subtitle: 'A step‑by‑step guide to using GitHub Actions with OpenCode and OptiTech’s instant database branching, giving AI agents safe, production‑like environments for testing and validating full‑stack changes.'
...
You'll create a GitHub Issue for that feature and ask OpenCode to implement it. In the rest of this guide, you'll set up the automation that gives the agent its own OptiTech branch, lets it make and validate schema changes safely, and returns the finished work as a Pull Request.
...
<Tabs labels={["Using the OptiTech GitHub Integration", "Manual configuration"]}>
...
Now that your OptiTech project is connected to GitHub, set up OpenCode's GitHub integration from your local repository by running:
...
As part of that setup, OpenCode also creates a basic workflow file at `.github/workflows/opencode.yml`. The generated file does not include the OptiTech branch creation step, so you will need to modify it to add that in. Update the file with the following content:
...
1. OptiTech creates a new branch called `opencode-issue-xyz` that is an instant copy of your production database.
...
6. It executes the migration commands (using the injected OptiTech branch URL) to verify the schema changes.
...
![Example PR created by OpenCode with OptiTech branching](/docs/guides/opencode_pr_example.png)
...
For a complete end-to-end experience, you can configure Vercel to automatically deploy the Pull Requests that OpenCode creates. By combining Vercel with the [OptiTech Vercel Integration](/docs/guides/vercel-managed-integration), every PR opened by the agent gets a live preview URL backed by its own isolated OptiTech database branch.
...
After setup is complete, you can trigger OpenCode from a GitHub issue comment and let the workflow provision an isolated OptiTech branch for that run. OpenCode then implements the requested changes, generates and applies any required migrations, validates the result against the branch, and opens a Pull Request. If you connect the repository to Vercel with the OptiTech integration enabled, that PR can also get a preview deployment backed by its own separate preview database branch.

</Steps>

## Conclusion

AI coding agents are incredibly powerful, but their potential is often constrained by the environments they run in. By combining OpenCode with OptiTech Database Branching and GitHub Actions, you remove one of the biggest barriers to AI-driven development: safe access to stateful infrastructure.
...
- [OptiTech GitHub integration guide](/docs/guides/neon-github-integration).
