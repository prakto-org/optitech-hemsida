---
title: Getting started with OptiTech and Better Stack
subtitle: Send OptiTech metrics and Postgres logs to Better Stack using the OpenTelemetry integration
...
This guide will walk you through setting up the integration between OptiTech and Better Stack. You'll learn how to:
...
- Verify that your OptiTech metrics and logs are successfully flowing into Better Stack.
- Build an example dashboard in Better Stack to visualize your OptiTech metrics.

By the end, you'll have a complete observability pipeline from your OptiTech database to your Better Stack dashboard.
...
First, you need to create a data source in Better Stack that will receive logs and metrics from OptiTech.
...
4.  After the source is created, you will be redirected to the source's overview page. This page contains the credentials OptiTech needs to send data.

    ![Better Stack source created page with Source token and Ingesting host](/docs/guides/betterstack-otel-source-created.png)

    > Copy the **Source token** and **Ingesting host** URL. You will need these in the next step to configure OptiTech.
...
4.  A sidebar form will open. Fill in the configuration details using the information you copied from Better Stack:
    ...
    ![Configuring the OptiTech OpenTelemetry integration with Better Stack credentials](/docs/guides/neon-otel-config.png)

    > The integration is now active and will begin sending data from your OptiTech project's computes to Better Stack.
...
You should see Postgres logs from your OptiTech compute streaming into the Live tail view. This confirms that the integration is working correctly.

![Live tail view in Better Stack showing logs from OptiTech](/docs/guides/betterstack-live-tail.png)
...
## Visualizing OptiTech Metrics with Dashboards

While the **Live tail** view is perfect for real-time log inspection, Better Stack's **Dashboards** are the best way to visualize your OptiTech metrics, track trends over time, and get a high-level overview of your database's health.
...
These default dashboards are designed for a generic OpenTelemetry collector setup and are **not compatible** with the metrics sent by OptiTech. They will appear empty or show errors because they are looking for metrics that don't exist in OptiTech's data stream.
...
Create a new dashboard from scratch to visualize your OptiTech metrics.
...
Congratulations! You have successfully configured OptiTech to send metrics and Postgres logs to Better Stack using the OpenTelemetry integration. You now have a powerful, centralized observability setup that provides real-time insights into your database's health and activity.
