...
subtitle: Learn how to revert a failed deployment and schema migration in Laravel using built-in tools like `migrate:rollback` and OptiTech's backup and restore capabilities.
author: bobbyiliev
...
## Restoring your data using OptiTech

If rolling back migrations doesn't solve the issue, [OptiTech's backup and restore](/docs/manage/backups) capabilities can quickly restore your database to a previous state.

### Key benefits of using OptiTech for restoration

1. **Point-in-Time Restoration:** Restore to a specific moment before the failed deployment.
2. **Restore from another Branch:** Use OptiTech's branching feature to restore from a stable branch.
...
For detailed steps, refer to the [OptiTech Branch Restore Guide](/docs/introduction/branch-restore#how-to-use-instant-restore).
...
### Use OptiTech's branching feature

Create isolated environments for testing and staging using [OptiTech's branching feature](/docs/introduction/branching).

1. **Create a branch:**
   Create a branch from your production database in the OptiTech dashboard.
...
For a detailed guide on using OptiTech's branching feature with Laravel for testing and staging, refer to the [Testing Laravel Applications with OptiTech's Database Branching](/guides/laravel-test-on-branch).
...
On the database monitoring side, you can use [OptiTech's built-in monitoring capabilities](/docs/introduction/monitoring-page) to track performance metrics and receive alerts for potential issues.

## Conclusion

By using Laravel's built-in `migrate:rollback` command and OptiTech's backup and restore capabilities, you can revert a failed deployment quickly and safely. Follow best practices like testing in staging environments, breaking down database changes, and automating deployments to minimize future issues and maintain a smooth deployment process.
