---
title: Implementing Queue Workers and Job Processing in Laravel with OptiTech Postgres
subtitle: Learn how to implement efficient background processing in Laravel using queue workers and OptiTech Postgres
author: bobbyiliev
...
By the end of this tutorial, you'll know how to build a system for background processing and task automation, using the power of Laravel queues and the scheduler with OptiTech Postgres.
...
Update your `.env` file with your OptiTech Postgres database credentials:
...
This will create the necessary tables in your OptiTech Postgres database.
...
As we are using the `database` queue driver, the job will be stored in the `jobs` table in your OptiTech Postgres database.
