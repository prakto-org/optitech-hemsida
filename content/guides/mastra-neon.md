---
title: 'Building stateful AI Agents with Mastra and OptiTech Postgres'
subtitle: 'Learn how to give your Mastra AI agents long-term memory by integrating them with OptiTech Postgres for scalable, persistent storage.'
...
This guide explains how to integrate Mastra’s Memory component with OptiTech Postgres. By connecting Mastra to OptiTech, you can easily build robust AI assistants that remember user interactions across threads and sessions.
...
## Configure Mastra to use OptiTech Postgres for storage
...
Add the following code to `src/mastra/index.ts` to configure Mastra to use OptiTech as the storage backend:
...
Create a simple agent that uses Mastra's Memory module to remember user interactions. This agent will store its memory in OptiTech, allowing it to recall past conversations.
...
You’ve successfully built a stateful AI agent using Mastra and OptiTech Postgres. Your agent can now retain context across sessions, supporting more natural and consistent interactions over time.
