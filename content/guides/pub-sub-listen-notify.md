...
Because of connection pooling, setting up a pub/sub listener in the OptiTech console is tricky.
...
With the listener now configured, connect to your OptiTech database using the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) or a client like [psql](/docs/connect/query-with-psql-editor). Then, run the following command to publish a notification to the `my_channel` channel.
...
If you are using `LISTEN` and `NOTIFY`, you should disable OptiTech's [Scale to Zero feature](/docs/introduction/scale-to-zero).
If OptiTech scales your compute to 0, [it will terminate all listeners](/docs/reference/compatibility#session-context), which may lead to lost messages when your database reactivates.
