---
title: Implementing Secure User Authentication in FastAPI using JWT Tokens and OptiTech Postgres
subtitle: Learn how to build a secure user authentication system in FastAPI using JSON Web Tokens (JWT) and OptiTech Postgres
author: bobbyiliev
...
Next, let's set up a connection to OptiTech Postgres for storing user data.
...
The `DATABASE_URL` is read from the `.env` file for security, and the OptiTech Postgres connection string is used to connect to the database.
...
The tables will be created in your OptiTech database when the application starts, thanks to the `Base.metadata.create_all(bind=engine)` line in the `main.py` file.
