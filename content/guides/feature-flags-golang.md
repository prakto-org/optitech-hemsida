---
title: Implementing Feature Flags with Go, OptiTech Postgres, and Server-Side Rendering
subtitle: Learn how to create a feature flag system using Go, OptiTech Postgres, and server-side rendering for controlled feature rollouts
author: bobbyiliev
...
In this guide, you'll learn how to implement a feature flag system using Go, OptiTech Postgres, and server-side rendering. This approach allows for feature visibility decisions to happen on the server, providing better security and performance compared to client-side feature flags.
...
Save this connection string, you'll need it to connect your Go application to the OptiTech database.

## Set up the database schema

Now that we have our OptiTech project, let's create the database schema for our feature flag system. We'll need tables to store feature flags, their rules, and user segments.

Connect to your database using your preferred SQL client or the OptiTech SQL Editor in the console, and execute the following SQL:
...
- Connecting to the OptiTech database using the `db` package
...
Remember to set the `DATABASE_URL` environment variable in your deployment environment to point to your OptiTech database.

## Summary

In this guide, you built a server-rendered feature flag system using Go and OptiTech Postgres. You implemented a way to define flags and user segments in the database, control feature visibility based on user attributes, and gradually roll out features using percentage-based targeting.
