---
title: Creating a Multi-Tenant Application with Laravel and OptiTech
subtitle: Learn how to build a scalable multi-tenant application using Laravel and OptiTech's powerful database features
author: bobbyiliev
...
Each tenant's data is isolated and remains invisible to other tenants. This approach is commonly used in Software as a Service (SaaS) applications. In this tutorial, we'll build the foundation for a multi-tenant SaaS application using Laravel and OptiTech.
...
Next go to your OptiTech dashboard and verify that the new tenant's database has been created:
...
In this tutorial, we've built a simple multi-tenant application using Laravel and OptiTech. We've covered:
...
This implementation provides a foundation for building more complex SaaS applications with Laravel and OptiTech. You can further expand on this system by:
...
Using the `stancl/tenancy` package along with OptiTech, each tenant will have its own database. Thanks to OptiTech's autoscaling feature, you can easily scale your application as you onboard more tenants.
