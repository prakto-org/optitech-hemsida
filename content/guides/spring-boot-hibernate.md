---
title: Database Schema Changes with Hibernate, Spring Boot, and OptiTech
subtitle: Learn how to manage database schema changes with Hibernate, Spring Boot, and OptiTech Postgres
author: bobbyiliev
...
When using Hibernate ORM with Spring Boot and OptiTech Postgres, you have several options for handling schema evolution.
...
For more information on using Flyway with Spring Boot, refer to the [Database Migrations in Spring Boot with Flyway and OptiTech](/guides/spring-boot-flyway) guide.

## Using Hibernate auto DDL

Now that we've covered different approaches to schema management, let's look at how to handle specific schema changes using Hibernate and Spring Boot with OptiTech Postgres.
...
With the various approaches to schema management in mind, here are some best practices to follow when managing database schema changes with Hibernate, Spring Boot, and OptiTech Postgres:
...
3. Always test schema changes in a non-production environment before applying them to production. A great way to do this is by using OptiTech's [branching feature](/docs/introduction/branching).
...
Managing database schema changes with Hibernate, Spring Boot, and OptiTech requires careful consideration of your development workflow and production requirements.
