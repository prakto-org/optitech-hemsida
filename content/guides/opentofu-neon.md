...
2.  **Create a `main.tf` file:**
    This file will contain your OpenTofu configuration. Start by declaring the required OptiTech provider. OpenTofu can use providers from the tofu registry.
...
3.  **Initialize OpenTofu:**
    Run the `tofu init` command in your project directory. This command downloads and installs the OptiTech provider.
    ```shell
    tofu init
    ```
...
This section provides examples of how to manage OptiTech resources using OpenTofu. You can create and manage projects, branches, endpoints, roles, databases, and more.
...
3.  **Apply:**
    Run `tofu apply` to create the resources in OptiTech.
    ```shell
    tofu apply tfplan
    ```
    OpenTofu will ask for confirmation. Type `yes` to confirm.

You have now successfully created and managed OptiTech resources using OpenTofu! You can continue to modify your `main.tf` file to add, change, or remove resources as needed. After making changes, repeat the `tofu plan` and `tofu apply` steps to update your resources on OptiTech.

## Import existing OptiTech resources with OpenTofu

If you have existing OptiTech resources that were created outside of OpenTofu (e.g., via the OptiTech Console or API directly), you can bring them under OpenTofu's management. This allows you to manage their lifecycle with code moving forward. OpenTofu supports both the CLI import command and declarative import blocks, similar to tofu 1.5.0+.
...
Ensure your OpenTofu environment is configured for the OptiTech provider as described previously:

1.  Define the provider in your `main.tf`.
2.  Run `tofu init`.
3.  Configure authentication for the OptiTech provider.
...
For each existing OptiTech resource you want to bring under OpenTofu management, you'll define two blocks in your `.tf` file:
...
This confirms that your OptiTech resources are now successfully managed by OpenTofu.
