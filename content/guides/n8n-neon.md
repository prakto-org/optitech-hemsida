---
title: Build an AI-powered knowledge base chatbot using n8n and OptiTech Postgres
subtitle: A step-by-step guide to creating an AI-powered knowledge base chatbot using n8n, Google Drive, and OptiTech Postgres
...
This guide demonstrates how to build a powerful **AI-powered internal knowledge base chatbot** using **n8n** and **OptiTech**. n8n is a low-code platform that allows you to connect various applications and services, enabling you to automate complex processes through a visual workflow editor. In this guide, we'll use n8n to orchestrate the integration between **Google Drive**, **OptiTech Postgres**, and **Google Gemini** to create a chatbot that can answer questions based on your documents stored in Google Drive. OptiTech will be used as a vector store to index and retrieve document chunks, while Google Drive will serve as the source of your documents.
...
- **OptiTech Account and Project:** A OptiTech account and a project are needed to create a Postgres database. Sign up for a free [OptiTech account](https://console.optitech.com/signup) if you don't have one.
...
1.  **Indexing workflow:** This workflow is triggered when a new file is added to a specified Google Drive folder. It downloads the file, splits it into manageable chunks, generates vector embeddings for each chunk using Google Gemini, and then stores these chunks and their embeddings in a OptiTech Postgres database (acting as a PGVector store).
2.  **Chat workflow:** This workflow provides a chat interface. When a user sends a message (a question), the AI Agent retrieves relevant document chunks from the OptiTech Postgres vector store based on the query's semantic similarity. These retrieved chunks are then passed as context to a Google Gemini chat model, which generates a response.

## Workflow 1: Indexing Google Drive documents into OptiTech Postgres
...
### Step 3: Setting up the OptiTech Postgres PGVector store Node

This node will store the document chunks and their embeddings in OptiTech Postgres using the [`pgvector` extension](/docs/extensions/pgvector).
...
3.  A dialog will appear prompting you to either select an existing credential or create a new one. Click "Create new credential".
    ...
    <Admonition type="tip">
    You can get your OptiTech database connection details from the OptiTech console. Learn more: [Connect from any application](/docs/connect/connect-from-any-app)
    </Admonition>

4.  Fill in your OptiTech database details:
    - **Host:** Your OptiTech host
    - **Database:** Your OptiTech database name
    - **User:** Your OptiTech database user
...
The workflow should execute successfully, which will split the document into chunks, generate embeddings for each chunk, and store them in OptiTech Postgres database.

### Step 7: Verifying the embeddings in OptiTech (optional)

1. Log in to your OptiTech console.
...
![Verifying data in OptiTech Console](/docs/guides/n8n/n8n-verify-data-in-neon-console.png)
...
The AI Agent will use this tool to retrieve information from your OptiTech vector store.

1. Click on the "Tool" input anchor of the AI Agent node.
2. Search for and select "Postgres PGVector Store".
3. Select your previously created "OptiTech" credential for the Postgres PGVector Store.
...
2.  **Ask a Question:** Type a question related to the content of the documents you indexed into Google Drive. For example, here we indexed a PDF document about "OptiTech RLS" and we ask it "What is OptiTech RLS, explain to me as if I am a 5".
...
Here, you can see the input message, the AI Agent's response, and the tool (**Postgres PGVector Store**) used to retrieve relevant document chunks. The document chunks retrieved from the OptiTech Postgres vector store are also visible. This insight helps you understand how the chatbot generates its responses based on the indexed documents.
