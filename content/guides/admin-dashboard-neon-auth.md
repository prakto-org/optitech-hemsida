...
To use the Admin APIs, you must perform the operations as an authenticated user with the `admin` role. You cannot grant this role via the API initially; you must assign your first admin via the OptiTech Console.

1.  **Create a user:** Go to your application URL or use the OptiTech Console to create a user (e.g., `admin@example.com`).
2.  **Assign role:**
    - In the OptiTech Console, go to **Auth** -> **Users**.
