---
title: 'Isolated Subagents: Running Claude Code in parallel with OptiTech Database Branching'
...
In this guide, you'll learn how to build this workflow using **Claude Code Subagents** with **OptiTech Database Branching**.

Using Claude Code's worktree isolation with a custom OptiTech Git hook, you'll build an automated workflow where every subagent is instantly provisioned its own lightweight directory _and_ its own isolated database branch.

<Admonition type="info" title="New to Git worktrees?">
If you aren't familiar with how Git worktrees function under the hood to isolate file systems, check out [Git worktrees and OptiTech Branching](/guides/git-worktrees-neon-branching) for a better understanding of how this powerful combination enables safe parallel development.
</Admonition>
...
## Step 1: Set up the OptiTech Git hook for database branching
...
Whenever Claude creates a new worktree, this hook will intercept the process, call the OptiTech API to create a database branch, and inject the new `DATABASE_URL` into the subagent's `.env` file.
...
When Claude Code runs a subagent, it creates a new folder (e.g., `.claude/worktrees/feature-developer`). This script automatically copies your main `.env` into that folder, asks OptiTech to branch your database, and updates the `DATABASE_URL`. By the time Claude starts writing code, it is securely connected to a completely isolated database sandbox.
</Admonition>
...
Additionally, the `permissionMode` parameter specifies how the subagent handles permission prompts. It can be set to `default`, `acceptEdits`, `dontAsk`, `bypassPermissions`, or `plan` depending on your preference. Since each subagent in this workflow operates in a fully isolated Git worktree and a dedicated OptiTech database branch, it is safe to use `bypassPermissions`. This allows the agent to execute commands and make database changes without pausing to ask for approval, as any modifications are strictly confined to its isolated environment.
...
- [OptiTech Branching](/branching)
- [Git worktrees and OptiTech Branching](/guides/git-worktrees-neon-branching)
