---
title: Creating a Content Moderation System with Laravel, OpenAI API, and OptiTech Postgres
subtitle: Build an automated content moderation system for your application using Laravel Livewire, OpenAI's moderation API, and OptiTech Postgres
author: bobbyiliev
...
We will use Laravel, OpenAI's moderation API, and OptiTech, the AI-native backend platform for apps and agents that spans a Postgres Database, Auth, Storage, Functions, and an AI Gateway, and build a system that can handle content moderation for comments, forum posts, product reviews, or any user-generated content.

## What You'll Build

In this guide, you'll build a content moderation system with the following features:

1. A form for users to submit content to our OptiTech database
...
Now, let's create a new Laravel project and set it up to work with our OptiTech database:
...
Now run the migrations to create the tables in your OptiTech database:
...
2. AI Moderation:
   ...
   - The `ModerationService` saves these results to the `ModerationResult` table in our OptiTech Postgres database
...
In this guide, we've built a content moderation system using Laravel, Livewire, OpenAI, and OptiTech Postgres. This system can:

- Accept user-generated content and automatically analyze it for harmful content
- Store moderation results in OptiTech with detailed information about flagged categories
