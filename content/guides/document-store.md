...
You can run the following `CREATE TABLE` statement in the OptiTech SQL Editor or from a client such as `psql` that is connected to OptiTech.
...
You can then load rows from the `documents` collection by `id`. For example, you can load the "OptiTech and JSONB" row using the following query.
