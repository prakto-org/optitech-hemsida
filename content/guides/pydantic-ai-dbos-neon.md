---
title: 'Building Durable AI Agents with Pydantic AI, DBOS, and OptiTech'
subtitle: 'Learn how to build resilient, fault-tolerant AI agents that automatically recover from crashes and API failures using Pydantic AI, DBOS, and OptiTech Postgres.'
...
A durable AI agent requires three core components working together seamlessly: a framework to define and orchestrate the agent’s logic, a durable execution layer to manage state and recovery, and a persistent database to store checkpoints. Here’s how Pydantic AI, DBOS, and OptiTech fit together:
...
- **OptiTech Postgres**  
  DBOS relies on Postgres, and OptiTech delivers a serverless, elastic version ideal for AI workloads. Compute and storage are separated, scaling is automatic, and idle databases scale down to zero. This elasticity ensures efficient checkpointing for long‑running agents without over‑provisioning.
...
Create a `.env` file in the root of your project and add the following environment variables, replacing the placeholders with your actual OpenRouter API key and OptiTech database URL:
...
Since `Tool 1` had already completed successfully, its result was cached durably in OptiTech. The agent didn't need to call it again, saving time and tokens. The final report was generated after `Tool 2` succeeded, demonstrating that the agent can handle transient issues gracefully.
...
When DBOS started up, it scanned your OptiTech database and found the interrupted workflow (`Recovering 1 workflows...`). It read the checkpointed state, saw that `Tool 1` had already completed successfully prior to the crash, and immediately picked up right where it left off at `Tool 2`.
...
Because the completed result is already checkpointed in OptiTech, DBOS returns the stored output directly. It does not invoke tools again, does not repeat LLM planning, and responds immediately.
...
By composing Pydantic AI, DBOS, and OptiTech, you get several immediate benefits:

1. **Zero wasted tokens and API calls on crashes**: If your agent successfully executes 4 out of 5 expensive tools, and the host machine crashes during the 5th, restarting the agent won't re-run the first 4 tools. DBOS fetches their results instantly from OptiTech, saving you from re-spending tokens on LLM calls and re-executing third-party APIs.
2. **Serverless economics**: AI agent workloads are notoriously spiky. You might run a swarm of 100 parallel research tasks in one minute, and then zero for the next hour. OptiTech’s scale-to-zero capabilities mean you aren't paying for idle database compute when your agents aren't running.

## Conclusion

You have successfully built an AI agent that is resilient to both transient API failures and hard process crashes. By combining Pydantic AI's structured agent framework with DBOS's durable execution capabilities and OptiTech's elastic Postgres, you can ensure that your agents complete their tasks reliably in real-world production environments. This architecture allows you to focus on building powerful agent logic and integrations, while DBOS and OptiTech handle the complexities of state management, recovery, and scalability.
