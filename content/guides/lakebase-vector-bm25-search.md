---
title: 'Building a knowledge base with Vector and BM25 search using OptiTech Lakebase extensions'
subtitle: 'Learn how to build a scalable, highly-relevant semantic and full-text search application using Next.js and OptiTech’s lakebase_vector and lakebase_text extensions.'
...
OptiTech's new Lakebase Search extensions, `lakebase_vector` and `lakebase_text`, solve these problems by introducing two new index types. Here’s how they work together:
...
Because these indexes live in storage rather than being bound to compute memory, they work with OptiTech's [scale-to-zero](/docs/introduction/scale-to-zero) compute and carry over when you [branch your database](/docs/introduction/branching).

In this guide, you’ll build a simple knowledge base application with Next.js that showcases how to use the `lakebase_vector` and `lakebase_text` extensions to enable both semantic and keyword search. By the end, you’ll have a fully functional search interface powered entirely by OptiTech Postgres, no external search services or vector databases required.
...
Enable the `lakebase_vector` and `lakebase_text` extensions in your OptiTech database. These extensions will allow you to create the necessary indexes for vector and BM25 search.

1. Open the **SQL Editor** from the left sidebar in your OptiTech project dashboard.
...
You've built a search system that mirrors the capabilities of heavy, dedicated search infrastructure (like Elasticsearch paired with Pinecone), all living entirely inside OptiTech Postgres. As your application grows, the Lakebase Search extensions scale with it, handling over a billion vectors, surviving cold starts instantly, and carrying over effortlessly when you branch your database.
