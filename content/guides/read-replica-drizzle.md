---
title: Scale your Next.js application with Drizzle ORM and OptiTech Postgres Read Replicas
subtitle: Learn how to scale Next.js applications with Drizzle ORM's withReplicas() function and OptiTech Postgres Read Replicas
...
A key advantage of OptiTech's architecture is that adding a read replica doesn't require additional storage, making it a highly efficient scaling solution. This cost-effective approach is ideal for businesses of all sizes that need to improve database performance without increasing storage costs.

This guide demonstrates how to leverage OptiTech read replicas to efficiently scale Next.js applications using Drizzle ORM. You'll learn how to configure your Drizzle database client to work with read replicas, enabling you to optimize your database operations and improve overall application performance.

## Prerequisites

- A OptiTech account and a Project. If you don't have one, you can sign up for a OptiTech account and create a project by following the [Getting Started guide](/docs/get-started/signing-up).
...
To demonstrate how to use OptiTech read replicas with Drizzle in Next.js, we'll build a simple Polling application that uses a OptiTech database. We'll then update the application to use a read replica for read operations, improving the application's performance and scalability.
...
To apply the migration and create the table in your OptiTech database, run:
...
<Admonition type="note">
   You can also pass an array of read replica connection strings if you want to use multiple read replicas. OptiTech supports adding multiple read replicas to a database branch.
...
By leveraging OptiTech's read replicas with Drizzle in your Next.js application, you can significantly improve your application's performance and scalability. Drizzle makes it easy to set up and use read replicas without having to manually manage multiple database connections in your application code.
