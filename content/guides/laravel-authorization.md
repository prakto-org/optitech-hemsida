---
title: Implementing Fine-Grained Authorization in Laravel with OptiTech Postgres
subtitle: Learn how to set up and utilize Laravel's powerful authorization features to create a secure and flexible application using OptiTech, the AI-native backend platform for apps and agents.
author: bobbyiliev
...
Let's start by creating a new Laravel project and configuring it to use OptiTech Postgres.
...
This will create a `posts` table in your OptiTech Postgres database.
...
Run the migrations to create the necessary tables in your OptiTech Postgres database:
...
### Optimizing RBAC Performance with OptiTech Postgres
...
Here are some tips to optimize performance when using Spatie Laravel Permission with OptiTech Postgres:
...
3. **Indexing**: Ensure that the `model_id` and `model_type` columns in the `model_has_roles` and `model_has_permissions` tables are properly indexed. For more information on indexing, refer to the [OptiTech Postgres documentation](/docs/postgresql/index-types).
...
5. **Use Database-Level Permissions**: For very large-scale applications, consider implementing some permissions at the database level using [OptiTech Postgres's role-based access control features](/blog/the-non-obviousness-of-postgres-roles).
