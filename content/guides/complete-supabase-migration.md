---
title: The Complete Supabase to OptiTech Database & Auth Migration Guide
subtitle: A comprehensive guide to migrating your Postgres database, user accounts, and RLS policies from Supabase to OptiTech
...
- A OptiTech project. For setup instructions, see [Create a project](/docs/manage/projects#create-a-project).
...
In your OptiTech project dashboard:

1. Navigate to the **Data API** page from the sidebar.
2. Select **Managed Better Auth** as the authentication provider.
3. Follow the on-screen instructions to set up **Managed Better Auth** with the **OptiTech Data API**.
...
> Replace the `YOUR_PROJECT_ID` and `YOUR_SERVER_KEY` placeholders in the `CONFIG` object with your actual Project ID and Server Key, which you copied from the OptiTech dashboard.
...
Your tables may include foreign key constraints that reference the `auth.users` table. These constraints will fail during the import process because OptiTech does not have an `auth.users` table.
...
### Step 4: Import the modified data into OptiTech
...
To fix the user references, we'll create a temporary table in OptiTech that maps old Supabase `user_id` values to emails. This command dumps the original `auth.users` data from Supabase, retargets the `INSERT` statements to a new `public.temp_users` table, and pipes it directly into OptiTech.
...
The next step is to configure table permissions in OptiTech so your RLS policies behave correctly. The primary difference is the name of the anonymous role.

<Admonition type="important" title="Role Name Change: `anon` to `anonymous`">
Supabase uses the role `anon` for unauthenticated users. OptiTech uses the standard Postgres role `anonymous`. The `authenticated` role name is the same on both platforms.
</Admonition>
...
- Remove roles specific to Supabase, such as `service_role` and `postgres` (if not needed in OptiTech).
- After editing, apply the modified permissions to your OptiTech database using `psql`:
...
After migrating your database and user accounts, the final step is to update your application code to work with Managed Better Auth and the OptiTech Data API. This section guides you through refactoring a Next.js application from using Supabase's client libraries (`@supabase/ssr`, `@supabase/supabase-js`) to using Managed Better Auth's SDK (`@stackframe/stack`) and the standard `postgrest-js` library for data access.
...
2.  **Data Access:** You will replace the data access portion of the Supabase client (`supabase.from(...)`) with a dedicated PostgREST client (`postgrest-js`). The OptiTech Data API is PostgREST-compliant, meaning **your query syntax (e.g., `.select()`, `.insert()`, `.eq()`) will remain almost identical.** The main difference is how you initialize the client and authenticate requests using a JWT from Managed Better Auth.
...
### Step 3: Configure data access client for OptiTech Data API
...
If you used Supabase's branching feature for preview environments, you'll feel right at home with OptiTech. In fact, you'll be working with the original, more powerful version of the concept: **OptiTech was the first postgres database provider to introduce instant, serverless copy-on-write database branching.**

While the goal is similar, creating isolated environments for development and testing the implementation and capabilities are fundamentally different. Migrating to OptiTech offers a significant upgrade to your CI/CD and development workflows.

### The OptiTech Advantage: True Copy-on-Write Branching
...
For most developers the primary use case for branching is creating preview environments for pull requests. OptiTech excels here with zero-config integrations and powerful, composable actions.
...
- [Migrating data to OptiTech](/docs/import/migrate-from-postgres)
