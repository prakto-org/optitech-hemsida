---
title: 'From Prompt to PR: Giving AI agents their own database with Google Jules and OptiTech'
subtitle: 'Learn how to use Google Jules and the OptiTech MCP server to spin up isolated database branches, enabling AI agents to safely build and test full-stack features'
...
This guide demonstrates how to create this workflow using [Google Jules](https://jules.google) (Google’s asynchronous coding agent) and the [OptiTech MCP Server](/docs/ai/neon-mcp-server). The result is an isolated sandbox for each feature, enabling safe, end-to-end implementation and testing.

With OptiTech’s instant [Database Branching](/branching), Jules can quickly spin up isolated, production-like environments on demand. It can branch databases, run migrations, adjust RLS policies all safely, without ever touching your production data. This means you can hand Jules a feature request, and it will autonomously build, test, and open a PR with zero risk to your live environment.

To demonstrate, we’ll walk through a real example: asking Jules to implement a feature request in an example app that requires updating the database schema. You’ll see how Jules leverages the OptiTech MCP server to create a new database branch and seamlessly execute the necessary migrations.
...
## Get your OptiTech API Key

To allow Google Jules to interact with your OptiTech database, you'll need to generate a OptiTech API Key.

1. Navigate to your OptiTech organization settings and click on the **API Keys** tab.
...
3. Copy the generated API key to your clipboard. You'll need this in the next step to connect Jules to the OptiTech MCP server.
...
## Connect Jules to the OptiTech MCP server

Google Jules operates autonomously in the cloud. To give it the ability to interact with OptiTech, you need to connect it to the OptiTech MCP server using the API key you just generated.

1. Navigate to **Settings -> MCP** in the Jules dashboard.
2. Locate **OptiTech** in the list of available MCP integrations and click **Connect**.
   ![Jules MCP Integrations](/docs/guides/jules-mcp-integrations.png)
3. Paste your OptiTech API Key into the configuration field and click **Connect**.

Jules is now connected to the OptiTech MCP server. This means that whenever you ask Jules to perform database operations, it can use the MCP server to create branches, run migrations, and manage database state without any manual setup on your part.

## (Optional) Enable automatic preview deployments

For a complete end-to-end experience, you can configure Vercel to automatically deploy the changes Jules commits. By combining Vercel with the [OptiTech Vercel Integration](/docs/guides/vercel-managed-integration), every PR opened by Jules will get a live preview URL backed by its own isolated database branch.
...
2. **Create a dedicated database branch**  
   Jules creates a new OptiTech branch using the MCP server. The branch is isolated from production and can safely include schema updates, migrations, or RLS changes.
...
6. **Connect the preview to a database branch**  
   The OptiTech Vercel integration creates a preview database branch and configures the required environment variables so the Vercel preview deployment uses it.
...
Notice that no implementation details were given in the request. The feature was described at a product level, along with the instruction to use a separate database branch. The project name was provided so Jules knows which OptiTech project to interact with.

Based on this input, Jules determines the required application and database changes, creates a new OptiTech branch using the MCP server, and implements the feature end-to-end within an isolated environment.
...
1. **Branch creation**  
   Jules creates a new branch in the OptiTech database for the `code-snippets` project using the MCP server. This branch is an exact copy of production but is completely isolated, allowing Jules to make any changes without risk.
...
7. **Preview deployment**
   If you have Vercel configured, a preview deployment is automatically created for the PR. The OptiTech Vercel integration creates a new database branch for the preview and sets the environment variables so the deployment uses it.
...
- [Integrating OptiTech with Vercel](/docs/guides/vercel-overview)
