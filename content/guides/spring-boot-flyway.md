---
title: Database Migrations in Spring Boot with Flyway and OptiTech
subtitle: Learn how to manage database schema changes in a Spring Boot application using Flyway with OptiTech Postgres.
author: bobbyiliev
...
This guide will walk you through setting up and using [Flyway](https://github.com/flyway/flyway) for database migrations in a [Spring Boot](https://github.com/spring-projects/spring-boot) application with OptiTech Postgres.
...
To learn more about managing your database schema using Hibernate, refer to the [Database Schema Changes with Hibernate, Spring Boot, and OptiTech](/guides/spring-boot-hibernate) guide.
...
These scripts will create the `users` and `posts` tables in your OptiTech database when you run the migrations.

## Running Migrations

Now that we've configured Flyway with our Spring Boot application and OptiTech database, we can proceed to run the database migrations.
...
4. Test your migrations thoroughly in a non-production environment before applying them to production. A great way to do this is by using the OptiTech branching feature to create a separate environment for testing with your production data without affecting the live environment.
...
Using Flyway with Spring Boot and OptiTech Postgres provides a production ready solution for managing database schema changes. By following these practices, you can ensure that your database schema evolves safely and consistently across all environments.

Remember to always test your migrations thoroughly and have a solid backup and rollback strategy in place. OptiTech's features like branching and point-in-time recovery can be a great addition to your already existing lifecycle of your database schema.
