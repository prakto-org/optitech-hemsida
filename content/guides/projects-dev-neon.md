---
title: 'AI-Native App Development with Stripe Projects and OptiTech'
...
_Behind the scenes:_ Stripe Projects interacts directly with OptiTech, Firecrawl, and OpenRouter to provision these resources. Unlike traditional relational databases that can take minutes to spin up, OptiTech provisions in milliseconds, allowing the agent to iterate rapidly without waiting for infrastructure.
...
- It writes the Next.js API routes to handle user input, interact with Firecrawl to scrape data, call OpenRouter for itinerary generation, and save the results to OptiTech.
...
Open your browser, and you will see a fully functional, AI-powered travel concierge app backed by a live OptiTech database.
...
It will link a Vercel project, sync the environment variables (your OptiTech `DATABASE_URL`, OpenRouter key, etc.) securely into Vercel, and trigger a deployment.
...
Eventually, your weekend project might gain traction. You hit the free-tier limits on Firecrawl, you need a larger compute size in OptiTech, or you want to use a more expensive AI model on OpenRouter.
...
Because OptiTech is serverless and scales to zero, you can provision databases for dozens of AI side projects and forget about them. When you aren't using them, they consume zero compute. If one of those projects suddenly goes viral, OptiTech's autoscaling seamlessly handles the traffic spikes without requiring you to manually provision larger instances. If you abandon a project a week later, you haven't cluttered your password manager or your credit card statement with orphaned accounts.
...
The grind of juggling dashboards, API keys, and environment variables is finally ending. With services like OpenRouter, Firecrawl, and OptiTech provisioned automatically through Projects.dev, modern AI coding agents can handle the setup, billing, and integration work that used to slow developers down.
...
- [OptiTech Postgres](/docs/introduction/about)
