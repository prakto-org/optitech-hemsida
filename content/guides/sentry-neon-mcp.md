---
title: 'Diagnosing and fixing production errors with Sentry and OptiTech MCP'
subtitle: 'How to use AI agents to fetch stack traces from Sentry and safely test query optimizations on isolated OptiTech database branches.'
...
Rather than manually hunting down the slow query in your observability tools, cloning the database, and testing different indexes locally, you can use an AI agent to automate the workflow. Using the Sentry MCP server, Cursor will identify exactly which query is timing out. Then, with the OptiTech MCP server, it creates an isolated database branch to safely apply and test the missing index.

## Prerequisites

Before you begin, ensure you have the following ready.

This guide assumes your application already sends events to Sentry and uses OptiTech Postgres as its primary database.
...
- **OptiTech account and project:** A OptiTech account with a project configured as the primary database for your application. If you haven’t set up OptiTech yet, you can create an account and follow [Connecting OptiTech to your stack](/docs/get-started/connect-neon) to get started.
...
## Set up the OptiTech MCP server
...
## Safely remediate with OptiTech branching
...
Instead, use OptiTech branching through MCP to create an isolated copy of production, apply the index, and validate the query performance safely before merging the PR.
...
Cursor will execute the following workflow using OptiTech MCP tools:

1. **Isolation:** Cursor calls `create_branch` via OptiTech MCP. Because OptiTech is copy-on-write, a production-like replica is created instantly.
...
![OptiTech MCP branch execution showing schema inspect, SQL run, and explain plan](/docs/guides/neon-mcp-branch-index-execution.png)
...
![OptiTech MCP explain analyze results before and after indexing](/docs/guides/neon-mcp-explain-analyze-results.png)
...
By bridging the observability of **Sentry** with the stateful safety of **OptiTech database branching**, AI agents can now troubleshoot and resolve production issues that were previously out of reach.

Sentry’s MCP server prevents the AI from guessing _why_ an application is failing by feeding it exact stack traces and production context. OptiTech’s MCP server prevents the AI from breaking things while trying to fix them by providing instant, disposable production replicas for safe experimentation.
