...
We'll use [Laravel Breeze](https://laravel.com/docs/11.x/starter-kits) for authentication and OptiTech Postgres as our database.
...
This will create the necessary tables in your OptiTech database.
...
Now, run the migrations to create the `todos` table in your OptiTech database:
...
Again, before you run the tests, note that the `RefreshDatabase` trait will clear the database before each test, so make sure to use a separate database for testing to avoid data loss like an in-memory SQLite database or a OptiTech database branch.
