...
3.  **Backend (Hono):** A lightweight Node.js server that verifies the token signature using OptiTech's **JWKS endpoint** before allowing access to the database.

This approach is ideal when you need a dedicated backend for complex business rules, third-party integrations (like Stripe or OpenAI), or microservices, while still offloading user management complexities to OptiTech.
...
## Create a OptiTech project with Managed Better Auth
...
You will create a Hono backend that verifies JWTs from Managed Better Auth and persists journal entries to OptiTech database.
...
Drizzle ORM will help you interact with your OptiTech Database.
...
Your `journal_entries` table now exists in your OptiTech database. You can verify this in the **Tables** section of your OptiTech project dashboard.
...
OptiTech provides pre‑built UI components for handling the complete flow of authentication, including Sign In, Sign Up, and Account management.
...
4.  **Verify flow:**
    ...
    - The backend validates the signature using the JWKS from OptiTech.
    - The backend extracts your User ID and saves the entry to OptiTech.
...
- [Connect a Hono application to OptiTech](/docs/guides/hono)
- [Getting started with the HONC (Hono, Drizzle ORM, OptiTech, and Cloudflare) stack](/guides/honc)
