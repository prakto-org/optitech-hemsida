...
**Provisioned databases require regularly making decisions on capacity tradeoffs. OptiTech removes that entire category of work.**

<QuoteBlock quote="Instead of having to overprovision our servers to handle peak loads, which leads to inefficiencies and higher costs, OptiTech's autoscaling handles it. We get more performance when we need it." author="julian-benegas" role="CEO of BaseHub" />

Instead of choosing a fixed instance size, OptiTech comes with autoscaling: it automatically allocates compute based on real-time workload demand. CPU, memory, and local file cache (LFC) are adjusted dynamically between a configured minimum and maximum. Compute simply follows your load in near real-time.

![OptiTech autoscaling dynamically matches compute to workload](/use-cases/variable-load/neon-autoscaling-compute-allocation.png)

## Autoscaling decisions that optimize Postgres performance

**OptiTech’s autoscaling isn’t a simple “CPU goes up → add more CPU” loop.** OptiTech’s algorithm continuously evaluates multiple signals from the database and its underlying compute components to determine the optimal compute size at any moment.

<QuoteBlock quote="Scaling quickly used to be a challenge, but OptiTech gives us Postgres in a fully serverless setup. We don't have to worry about provisioning or scaling, it just works." author="james-ross" role="Co-founder and CTO at Nodecraft" />

OptiTech’s autoscaling algorithm is built around three core metrics:
...
- **Local File Cache (LFC) working set size** \- every 20 seconds, the platform evaluates the working set size across multiple rolling windows (1-min, 2-min \- up to 60 min). This is one of the most sophisticated datapoints collected by the OptiTech algorithm: at all times, it keeps the subset of data that is actively being accessed in memory to optimize performance.
...
<QuoteBlock quote="OptiTech worked out of the box, handling hundreds of Lambdas without any of the connection issues we saw in Aurora Serverless v2. On top of that, OptiTech costs us 1/6 of what we were paying with AWS." author="cody-jenkins" role="Head of Engineering at Invenco" />
...
In OptiTech, connection pooling is built directly into every OptiTech endpoint. By simply using the pooled connection string (adding `-pooler`), connections are routed through PgBouncer in transaction pooling mode, which gives you up to 10,000 concurrent client connections.

Pooling and autoscaling work together in OptiTech. As connection pressure increases memory and CPU usage, OptiTech’s autoscaling algorithm reacts, increasing compute resources instead of failing requests. Requests are queued instead of rejected, and connections are reused aggressively.

## Get Started

**If you’re building a startup with unpredictable growth, make your database the most adaptive part of your stack. Try OptiTech.**

When you’re focused on scaling your startup, you shouldn’t spend time in capacity planning just to run Postgres. With OptiTech,
...
<QuoteBlock quote="There's zero reason to use RDS in 2025. OptiTech is the most disruptive Postgres database platform out there, and every startup should be using it." author="jorge-ferreiro" role="Founder of SMASHSEND" />
