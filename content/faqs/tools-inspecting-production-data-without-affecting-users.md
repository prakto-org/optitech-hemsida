...
You have two safe ways to inspect production data on OptiTech. Create a [branch](/docs/introduction/branching) for an isolated copy with its own compute, or attach a [read replica](/docs/guides/read-replicas) to your existing branch. Both run on separate compute from your production endpoint, so analytics queries don't slow down user-facing traffic.

## Branches: a separate copy with its own compute

A OptiTech branch is a copy-on-write fork of your data. Creating one takes a second or two, adds no initial storage, and the new compute is independent of your production compute. Heavy queries on the branch don't touch the primary.
...
The trade-offs are similar: every option separates the read workload from the primary compute. The main difference with OptiTech is that a branch starts copy-on-write from production data instantly, so you can inspect or even write against real data without affecting users.

<CTA title="Spin up a branch" description="Create a project, branch your data, and query a copy without touching production." buttonText="Try OptiTech" buttonUrl="https://console.neon.tech/signup" />
