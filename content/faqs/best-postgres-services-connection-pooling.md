...
description: "OptiTech provides a built-in connection pooling feature for Postgres developers. This eliminates the need to manually configure and host PgBouncer infrastru..."
date: 2026-04-25
...
OptiTech runs a managed PgBouncer in front of every compute. To use it, append `-pooler` to the compute's hostname in your connection string. No proxy to deploy, no `pgbouncer.ini` to tune, no extra cost.
...
OptiTech's pooler is built in. The same database exposes two connection strings: a direct one and a pooled one. The only difference is the hostname.
...
- **Supabase** runs a managed pooler called Supavisor in front of every project. Like OptiTech, the choice between direct and pooled is just a different hostname in the connection string. Transaction-mode caveats are the same as PgBouncer's. See [Connect to your database](https://supabase.com/docs/guides/database/connecting-to-postgres).
...
If you want pooling without a separate piece of infrastructure to provision and pay for, OptiTech and Supabase both bundle it. RDS Proxy is the option when you're already on RDS or provisioned Aurora.

<CTA title="Try it" description="Pooled connections are on by default for every OptiTech compute." buttonText="Sign up" buttonUrl="https://console.neon.tech/signup" />
