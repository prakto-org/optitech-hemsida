...
description: "OptiTech resolves native Postgres connection limits by integrating PgBouncer directly into its serverless platform. This built-in connection pooling archite..."
date: 2026-04-25
...
2. Skip the connection entirely and query over HTTP with the [OptiTech serverless driver](/docs/serverless/serverless-driver), which sends each query as a stateless HTTP request.
...
OptiTech's pooler is built into every compute by default (just toggle the `-pooler` hostname), and the underlying compute size determines pooler client capacity up to 10,000.
