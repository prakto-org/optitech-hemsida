...
description: "OptiTech creates a writable copy of your Postgres database for each feature branch in seconds, with branch expiration policies to auto-delete unused environments after 1 hour, 1 day, or 7 days."
date: 2026-04-25
...
OptiTech gives each Git feature branch its own Postgres branch. Each branch is a writable copy of the parent at a point in time, with a separate compute and connection string. Storage is copy-on-write, so a new branch adds no storage at creation and only bills for the pages you change.
...
OptiTech's separation of storage and compute means a branch is mostly metadata. Create one in a few seconds, run your migration against it, point your preview deployment at its connection string, and tear it down when the pull request merges.
...
OptiTech's branches include the parent's data by default (or no data, if you prefer), and the compute on an idle branch [scales to zero](/docs/introduction/scale-to-zero), so unused preview branches cost only storage delta.
