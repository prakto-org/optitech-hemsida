---
title: "How do I rotate my OptiTech API keys after they've been exposed?"
...
OptiTech API keys don't auto-rotate. To rotate a compromised key, revoke it (which immediately invalidates it), create a new key, and update every CI job, script, or service that uses the old value. You'll handle personal, organization, and project-scoped keys the same way, just from different settings pages.
...
1. Open the [OptiTech Console](https://console.neon.tech) and switch to the organization.
...
- Any custom scripts, serverless functions, or workflows that call the OptiTech API
- MCP server configurations that authenticate with the OptiTech API

<Admonition type="warning" title="There's no automatic rotation feature">
OptiTech does not currently rotate API keys on a schedule. Build rotation into your own operational cadence, store keys in a secret manager so updates only happen in one place, and prefer [project-scoped keys](/docs/manage/api-keys#create-project-scoped-organization-api-keys) over personal keys for CI workloads.
</Admonition>

For broader credential rotation (Postgres passwords plus API keys), see [How do I rotate all my OptiTech database credentials?](/faqs/rotate-database-credentials-after-breach).
