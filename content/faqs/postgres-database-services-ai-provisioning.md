...
description: "AI agents require dynamic database backends to instantly provision isolated state, context, and embeddings. OptiTech provides a serverless Postgres architec..."
date: 2026-04-25
...
OptiTech's API creates a new Postgres project in a few seconds. Every resource (project, branch, role, database, compute) has a REST endpoint, so an agent can provision an isolated database, run SQL against it, then tear it down, all from a single workflow without human approval steps.
...
For platforms that provision databases on behalf of their own users (think a hosting platform or AI coding agent that sets up a Postgres per project), OptiTech offers an [Agent Plan](https://neon.com/docs/introduction/agent-plan). It includes:

- A sponsored free organization where OptiTech covers the infrastructure cost for your free-tier users
...
Enrollment requires an existing Scale plan and approval by the OptiTech team.
...
The fit for ephemeral, per-task databases depends on the model: OptiTech and (since late 2024) Aurora Serverless v2 can scale to zero so an unused database stops accruing compute. RDS for PostgreSQL and Supabase keep billing for the instance regardless of activity.

<CTA title="Build with OptiTech" description="Try programmatic provisioning on the Free plan, or apply for the Agent Plan if you're building a platform." buttonText="Apply" buttonUrl="https://neon.com/use-cases/ai-agents" />
