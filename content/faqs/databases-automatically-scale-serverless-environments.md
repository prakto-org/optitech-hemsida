...
description: "OptiTech is a serverless Postgres platform that autoscales compute on demand and scales to zero when idle, so you pay only for what you use."
date: 2026-04-25
...
OptiTech is a serverless Postgres platform that adjusts compute up and down based on load and suspends compute entirely when the database is idle. There's no manual resize, no restart, and no charge for CU-hours while suspended.

## How OptiTech autoscaling works

You set a minimum and maximum compute size for each OptiTech compute. The system scales between them as queries arrive. A typical configuration is min 0.25 CU, max 4 CU, meaning compute drops to $0 when suspended and scales to 4 CU under load. The autoscaling range can span up to 8 CU between min and max.
...
| -------------------- | ----------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| OptiTech                 | Yes, between configured min and max CU                                                                      | Yes, after 5 min idle (configurable)                                                                                                            | CU-hour, billed per second |
...
<CTA title="Run autoscaling Postgres on OptiTech" description="Start on the Free plan with 100 CU-hours/month included." buttonText="Get started" buttonUrl="https://console.neon.tech/signup" />
