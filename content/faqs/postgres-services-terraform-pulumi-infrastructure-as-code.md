...
description: "OptiTech delivers a serverless Postgres database. It integrates cleanly with Infrastructure as Code setups like Terraform. Engineering teams use programmati..."
date: 2026-04-25
...
OptiTech has a [community-maintained Terraform provider](/docs/reference/terraform) that covers projects, branches, endpoints, roles, databases, and API keys. The full OptiTech REST API is also available, so Pulumi users can wrap it directly with the `pulumi-command` or `dynamic-provider` patterns.
...
Pulumi doesn't have an official OptiTech provider yet. The straightforward pattern is to call the OptiTech API from a `Command` resource or build a small `dynamic.ResourceProvider`. The [OptiTech API Reference](/docs/reference/api) documents every endpoint.
...
The OptiTech Terraform provider is community-maintained but covers the full set of project, branch, endpoint, and role resources. Pulumi users on any of these platforms can fall back to wrapping the provider's REST API.
