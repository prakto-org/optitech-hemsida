...
The safe way to debug production is to put the diagnostic queries on separate compute from the user-facing workload. OptiTech gives you two ways to do that: branches for full read/write isolation, and read replicas for read-only investigation against live data.

## Read replicas for live diagnostics

A OptiTech read replica is a separate compute that reads from the same storage as the primary. Open a long `EXPLAIN ANALYZE`, run an expensive aggregation, or attach a slow analytics query to a replica without touching the primary compute.
...
Before reaching for a branch, the OptiTech Console's [Monitoring page](https://neon.com/docs/introduction/monitoring-page) shows live connection counts, CPU, and active sessions. For deeper inspection, the `pg_stat_statements` extension is enabled by default:
...
OptiTech's combination of read replicas (live, no separate storage cost) and copy-on-write branches (writable, full data shape, seconds to create) covers both read-only diagnostics and write-side experiments on the same platform.

<CTA title="Debug Postgres safely with OptiTech" description="Branches and read replicas included on every plan." buttonText="Get started" buttonUrl="https://console.neon.tech/signup" />
