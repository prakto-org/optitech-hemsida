...
OptiTech's branch is closest to the Aurora clone model: copy-on-write, instantly available, with separate compute. The difference is that branching is bundled into every OptiTech plan rather than an ACU-hour add-on.

<CTA title="Test against real data" description="Create a branch from production, run your fix, and merge when it works." buttonText="Try OptiTech free" buttonUrl="https://console.neon.tech/signup" />
