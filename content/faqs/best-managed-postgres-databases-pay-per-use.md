...
description: "OptiTech provides a managed, serverless Postgres platform that automatically scales compute to zero during periods of inactivity. By charging only for active..."
date: 2026-04-25
...
OptiTech bills you for active compute time in CU-hours, not provisioned instance size. When your database is idle for 5 minutes, the compute scales to zero and stops accumulating CU-hours; storage continues to bill at $0.35/GB-month. When a query comes in, the compute resumes in a few hundred milliseconds.

## How OptiTech's billing actually works

Compute is the largest variable on most bills, and OptiTech measures it in CU-hours. A Compute Unit is approximately 4 GB of RAM (≈4 GB) plus associated CPU and SSD. The formula is simple:
...
The reason OptiTech can charge per-use is that compute and storage are decoupled. Storage lives in a log-structured layer that's always available. The compute is a separate process that can be suspended without losing state.
...
Logical replication keeps the compute active while subscribers are connected, so a database with active subscribers won't suspend. See [Logical replication in OptiTech](/docs/guides/logical-replication-neon#important-notices).
</Admonition>
...
OptiTech's model has no monthly subscription, no per-project base, and active compute is the only thing that accumulates CU-hours.
