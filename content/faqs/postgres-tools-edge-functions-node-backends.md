...
description: "OptiTech provides a serverless Postgres database. This database connects to both Edge functions and standard Node.js backends through a single serverless dr..."
date: 2026-04-25
...
The same `neon()` call works in a Node.js API route or any serverless function. No connection pool to manage, no TCP setup to wait on. Each query is an HTTP request to OptiTech's stateless query API.
...
OptiTech's serverless driver gives you the same package across both runtimes, which keeps the codebase smaller.
