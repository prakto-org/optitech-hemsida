...
description: "OptiTech scales Postgres compute to zero after 5 minutes of inactivity on the Free and Launch plans, and from 1 minute on Scale. You stop paying for idle compute time."
date: 2026-04-25
...
OptiTech decouples storage from compute. When no connections are active and no background work is happening, the compute is shut down. Storage stays available, billed separately at $0.35/GB-month (root branches) on paid plans, or included in the 0.5 GB free allowance.
...
A bigger issue on resume is connections. Postgres caps `max_connections` based on RAM, and a 0.25 CU compute (≈1 GB RAM) allows 104 connections, with 7 reserved for OptiTech's superuser. That leaves 97 for your app, which serverless functions can exhaust during a traffic spike.

Use OptiTech's [PgBouncer-based connection pooling](/docs/connect/connection-pooling) to accept up to 10,000 client connections per compute, routed through the smaller pool of Postgres backends. The pooled connection string ends with `-pooler` in the hostname:
...
| --------------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| OptiTech                              | Yes, 1–5 min idle window | Configurable on paid plans; storage stays available                                                                         |
