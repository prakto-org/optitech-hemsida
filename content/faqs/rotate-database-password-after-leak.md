...
subtitle: 'Reset a role password from the OptiTech Console, CLI, or SQL to invalidate the leaked credential.'
...
nextLink:
  title: 'How do I rotate my database URL or connection string in OptiTech?'
  slug: rotate-database-url-connection-string
---

## Quick answer

In OptiTech, "rotating a password" means resetting the password for the affected Postgres role. You can do this from the OptiTech Console (**Branches → branch → Roles & Databases → Reset password**), through the OptiTech API, or with an SQL `ALTER USER` statement. OptiTech generates a new password immediately and returns the updated connection string. Any new connection attempt using the old password will fail to authenticate.
...
4. Confirm. OptiTech shows the new password once. Copy it.
...
If you need to set a specific password (for example, to match a value stored in a secret manager), connect with `psql` or the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) and run:
...
If the leak might have exposed more than one role, see [How do I rotate all my OptiTech database credentials after a breach?](/faqs/rotate-database-credentials-after-breach).
