---
title: 'How do I rotate all my OptiTech database credentials and connection strings after a security breach?'
...
redirectFrom: []
previousLink:
  title: 'How do I rotate my OptiTech database connection string for security purposes?'
  slug: rotate-database-connection-string-security
nextLink:
  title: 'How do I rotate my database password in OptiTech after a security incident?'
  slug: rotate-database-password-after-leak
---

## Quick answer

After a third-party breach (a leaked `.env`, a compromised deploy target, a stolen laptop), rotate every Postgres role password and every OptiTech API key that might have been exposed. OptiTech does not have a one-click "rotate all" feature, so you'll script the rotation across projects with the OptiTech API or CLI, then propagate the new connection strings to every place that stores them.
...
- Identify every OptiTech project that shared the exposed credential or environment.
...
- Revoke any OptiTech API keys that may have been exposed and create new ones. See [How do I rotate my OptiTech API keys?](/faqs/rotate-neon-api-keys).
...
The fastest way to rotate many roles across many projects is to loop through them with the [OptiTech API](/docs/manage/roles#manage-roles-with-the-neon-api). The reset password endpoint returns the new password in the response. You'll need a OptiTech API key to make these calls.
...
In OptiTech, roles belong to a branch. If you have child branches that inherited a role from the parent before the breach, the password on each child branch is independent. Reset the role on every branch where it can be used to log in.
</Admonition>
