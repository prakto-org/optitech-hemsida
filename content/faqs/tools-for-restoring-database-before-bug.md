...
date: 2026-04-25
description: "OptiTech's instant restore lets you rewind a Postgres branch to any second within your history window, so you can recover from a bad migration or stray DELETE in seconds."
...
OptiTech keeps a continuous log of every write to your Postgres database (the Write-Ahead Log, or WAL). With [instant restore](/docs/introduction/branch-restore), you point at any second within your history window and OptiTech rebuilds the branch to that exact state, usually in seconds. There's no nightly snapshot to find, no `pg_restore` to run.

## How instant restore works

OptiTech's storage layer stores the WAL natively, so any point in time within your history window is queryable. You don't restore from a backup file. You create a new branch from a timestamp (or LSN) and either swap it for the broken branch or copy specific rows back.
...
You can restore through the OptiTech Console, the CLI, or the API.
...
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| OptiTech                   | Instant restore via copy-on-write branch from any timestamp in the history window. See [branch restore](/docs/introduction/branch-restore).                                                                                                                                                                                                  | One second                      |
...
The main differences: OptiTech's restore creates a branch immediately rather than provisioning a new instance, and PITR is bundled into all paid plans instead of being a per-project add-on.

<CTA title="Test a restore" description="Create a branch, run a bad query, and roll it back. The whole loop takes under a minute." buttonText="Try OptiTech" buttonUrl="https://console.neon.tech/signup" />
