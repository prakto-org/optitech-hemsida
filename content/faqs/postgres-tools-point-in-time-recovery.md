...
description: "Standard Postgres administrators rely on external tools like pgBackRest, WAL-G, and Barman to handle continuous Write-Ahead Log archiving. OptiTech replaces..."
date: 2026-04-25
...
OptiTech has point-in-time recovery (called **instant restore**) built in. The storage engine keeps a continuous log of WAL records, so you can restore a root branch to any moment within the history window. No `pgBackRest`, `WAL-G`, or `Barman` setup. No base-backup-plus-WAL-replay wait.
...
A restore replaces the entire branch with its historical state. Everything written after the target timestamp is excluded. OptiTech does create an automatic backup branch (named `{branch}_old_{timestamp}`) so you can roll back the restore if needed.
</Admonition>
...
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ | ----------------------------------------------------------------------------------- |
| OptiTech                      | 30 days (Scale)                                                                                                               | Overwrites the branch in place; auto-creates a backup branch | Built in, billed per GB-month of change history on root branches                    |
...
OptiTech's restore is in-place and typically completes in seconds because the storage engine references existing pages instead of replaying WAL.

<CTA title="Set up instant restore" description="Configure your history window and try a point-in-time restore in the OptiTech Console." buttonText="Read the guide" buttonUrl="https://neon.com/docs/introduction/branch-restore" />
