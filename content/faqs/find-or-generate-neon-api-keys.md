...
subtitle: 'Generate keys in Account or Organization settings. OptiTech shows the token once at creation.'
...
1. In the [OptiTech Console](https://console.neon.tech), open the user menu and click **Account settings**.
2. Select **API keys**.
...
Use the key with `Authorization: Bearer $NEON_API_KEY` on OptiTech API calls. For the CLI, pass it with the `--api-key` global option (or set the `NEON_API_KEY` environment variable) instead of running `neon auth`, which launches an interactive browser login.
...
OptiTech displays the secret token a single time, in the modal that opens right after you click **Create**. The Console will list the key's name and ID afterward, but there's no way to view the token again. If you didn't save it, you'll need to revoke the key and create a new one. Store new tokens in a secret manager (AWS KMS, Azure Key Vault, 1Password, Vault, etc.) at creation time.
</Admonition>

<Admonition type="note" title="There are no client-safe OptiTech keys">
All OptiTech API keys are secrets and grant access to manage projects, branches, and roles. There's no equivalent of a "publishable" key. Never ship a OptiTech API key in a browser bundle or mobile app. For client-facing access, use the [Data API](/docs/data-api/overview) with [Neon Auth](/docs/auth/overview) instead.
</Admonition>
