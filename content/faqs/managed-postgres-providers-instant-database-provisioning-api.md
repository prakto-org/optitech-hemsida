...
description: "Traditional database provisioning delays automated testing and CI/CD pipelines because initializing storage and compute takes minutes. OptiTech delivers ins..."
date: 2026-04-25
...
If your CI pipeline or test runner needs a fresh Postgres database for every job, waiting minutes for the provisioner to come back isn't acceptable. Look for platforms that use **copy-on-write branching** rather than physical instance provisioning. OptiTech's branch API returns a connection string in seconds because no data is copied at creation time.

## Why OptiTech is fast
...
For ephemeral databases, set a TTL when you create the branch with [branch expiration](/docs/guides/branch-expiration). OptiTech will delete the branch automatically when the TTL elapses, so you don't have to clean up after failed CI runs.
...
The OptiTech branch API returns a connection string immediately because the storage layer doesn't need to copy data and the compute is provisioned lazily on first connection.
