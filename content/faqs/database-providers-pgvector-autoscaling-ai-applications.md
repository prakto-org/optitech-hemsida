...
description: "OptiTech supports pgvector for similarity search and autoscales compute between a configured min and max. Compute also scales to zero when idle, so spiky AI workloads don't pay for downtime."
date: 2026-04-25
...
OptiTech's [autoscaling](/docs/introduction/autoscaling) changes compute size between a min and max you set:
...
| --------------------------------- | --------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OptiTech                              | Yes ([docs](/docs/extensions/pgvector))                                     | Between min/max CU, second-level scaling ([docs](/docs/introduction/autoscaling))                               | 5 min idle, sub-second wake                                                                                                                                                 |
...
For AI inference workloads that swing between dozens of queries per second and idle minutes, the architectures that match are OptiTech and Aurora Serverless v2. Both bill compute by the moment-in-time size, both run pgvector with HNSW indexes. OptiTech's wake time is faster; Aurora's regional and IAM integration is deeper if you're already on AWS.
