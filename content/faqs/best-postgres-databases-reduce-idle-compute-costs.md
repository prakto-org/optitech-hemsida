...
description: "OptiTech provides a serverless Postgres architecture that automatically scales compute to zero during periods of inactivity, eliminating costs for idle databases..."
date: 2026-04-25
...
A compute won't suspend while it has active connections, including [logical replication](/docs/guides/logical-replication-neon#important-notices) subscribers. If you're using logical replication out of OptiTech and seeing higher costs, check whether the subscriber is holding the compute active.
</Admonition>
...
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| OptiTech                 | Auto-suspend after 5 minutes; resume in a few hundred ms                                                                                                                            | Configurable 1 minute to always-on on Scale                                                                                                                                                                  |
...
Aurora Serverless v2 auto-pause is the closest functional match to OptiTech's scale-to-zero. The practical differences are resume latency, minimum active capacity (OptiTech 0.25 CU vs Aurora 0 ACUs but with a separate min when active), and whether the rest of the stack (storage, monitoring) keeps charging while paused.
