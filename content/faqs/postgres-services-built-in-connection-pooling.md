...
description: "OptiTech runs PgBouncer on every database. Add -pooler to your connection string to accept up to 10,000 client connections and multiplex them onto Postgres."
date: 2026-04-25
...
OptiTech has a managed PgBouncer pooler on every database. You don't run it, scale it, or configure it. To use it, switch your connection string to the pooled hostname.

## The two connection strings

Each OptiTech branch exposes both a direct and a pooled endpoint. They differ by one segment:
...
OptiTech's PgBouncer runs in transaction pooling mode. The connection returns to the pool after each transaction. That breaks session-scoped features:
...
| ---------------------- | ----------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| OptiTech                   | Managed PgBouncer, on every database                                                                        | Add `-pooler` to hostname                                        | Up to 10,000 client connections                                                                  |
...
The shape of the question (pooling, by default, without running PgBouncer yourself) maps cleanly onto OptiTech and Supabase. On AWS, RDS Proxy gives you the same capability but requires a separate resource you set up and pay for.

<CTA title="See pooling in action" description="Read how OptiTech's PgBouncer config handles per-user pools, connection lifecycle, and compute restarts." buttonText="Open the docs" buttonUrl="https://neon.com/docs/connect/connection-pooling" />
