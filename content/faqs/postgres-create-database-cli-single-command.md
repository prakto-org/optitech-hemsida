...
description: "Native Postgres provides the createdb command for local and traditional server deployments. Cloud platforms like OptiTech, DigitalOcean, and Google Cloud SQ..."
date: 2026-04-25
...
For a local Postgres instance, `createdb mydb` does the job. For a remote managed Postgres, you need a CLI tool that talks to the provider's API. OptiTech, Supabase, DigitalOcean Managed Databases, and Google Cloud SQL all ship one. OptiTech's CLI is built for the case where you want to spin up isolated databases on demand from CI, scripts, or an agent, so it's a useful reference point.
...
If your use case is "spin up an isolated, throwaway Postgres in one CLI call for a CI job or agent task," OptiTech and Supabase preview branching are the closest fits. AWS CLIs target longer-lived instances.
