...
1. **Create a OptiTech project** on behalf of your user in your account or organization
...
4. **User claims the project**
   - When they click the URL, OptiTech prompts them to transfer the project to their account
...
Use the OptiTech [create project API](/docs/reference/api/projects/create-project) to create a new project that you intend to transfer to your user.
...
Your user will need the connection string from the response (`connection_uri`) to [connect to the OptiTech database](/docs/get-started/connect-neon). The `{password}` placeholder represents the actual password generated for the database. You'll also use the project `id` to create a transfer request.

## Create a transfer request

With your project created, use the OptiTech [project transfer request API](/docs/reference/api/projects/create-project-transfer-request) to generate a transfer request. You can create this request immediately or at a later time when you're ready to transfer the project. Each transfer request has a configurable expiration period, specified by the `ttl_seconds` parameter.
...
- `ru={redirect_url}` (optional) - A URL-encoded destination where the user is redirected after successfully claiming the project
  - Without this parameter, users remain on the OptiTech project page after claiming
...
- They'll need a OptiTech account to claim the project (they can create one during the claim process)
...
1. OptiTech prompts them to log in or create an account
2. After authentication, OptiTech displays a confirmation screen
3. They select their destination OptiTech organization
4. Upon confirmation, OptiTech transfers the project
5. The user is then:
   ...
   - Kept on the OptiTech project page if no redirect URL was specified
...
- **Vercel orgs not supported**: Transferring a project into a Vercel-managed OptiTech [organization](/docs/reference/glossary#organization) via the claim flow is not supported, meaning that if you created your OptiTech account through the [Vercel-managed integration](/docs/guides/vercel-managed-integration), you cannot claim projects into the OptiTech organization created by that integration.
...
- [OptiTech API documentation](/docs/reference/api)
