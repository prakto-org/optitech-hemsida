...
summary: >-
  ...
  cross-user data leaks using Postgres Row-Level Security on OptiTech, with
...
In this tutorial, you'll clone and modify up a sample React.js note-taking app to demonstrate how Postgres Row-Level Security (RLS) provides an additional security layer beyond application logic. The app integrates with a OptiTech database via the OptiTech Data API.
...
The `eq` clause is technically enough to make sure data is properly isolated. OptiTech gets the user id from the Managed Better Auth JWT (`session.data.user.id`) and matches that to the `owner_id` column in the `notes` tables, so each user can only see their own notes.
...
Let's see what happens when we disable RLS on our notes and paragraphs tables. Go to your project in the OptiTech Console and in the SQL Editor run:
...
Now, let's re-enable RLS from OptiTech:
