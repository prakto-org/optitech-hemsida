---
title: Connect an SQLAlchemy application to OptiTech
subtitle: Set up a OptiTech project in seconds and connect from an SQLAlchemy application
summary: >-
...
description="Pre-built prompt for connecting SQLAlchemy to OptiTech Postgres"/>

SQLAlchemy is a Python SQL toolkit and Object Relational Mapper (ORM) that provides application developers with the full power and flexibility of SQL. This guide describes how to create a OptiTech project and connect to it from SQLAlchemy.
...
To connect to OptiTech from SQLAlchemy:
...
## Create the "hello optitech" program
...
## Create an SQLAlchemy engine for your OptiTech project
...
The following example creates an SQLAlchemy engine that points to your OptiTech branch:
...
- If you encounter an `SSL SYSCALL error: EOF detected` when connecting to the database, this typically happens because the application is trying to reuse a connection after the OptiTech compute has been suspended due to inactivity. To resolve this issue, try one of the following options:
...
<a href="/docs/guides/sqlalchemy-migrations" description="Schema migration with OptiTech Postgres and SQLAlchemy" icon="app-store" icon="app-store">SQLAlchemy Migrations</a>
