...
subtitle: Learn how to replicate data from OptiTech with Fivetran
summary: >-
  Logical replication from OptiTech to Fivetran lets you stream Postgres WAL changes
  to any Fivetran-supported destination by configuring a pgoutput replication
  slot and publication on OptiTech, then connecting Fivetran via a direct
  (non-pooled) connection. Use this page when setting up OptiTech as a PostgreSQL
...
In this guide, you will learn how to define a OptiTech Postgres database as a data source in Fivetran so that you can replicate data to one or more of Fivetran's supported destinations.
...
1. If you are using OptiTech's **IP Allow** feature to limit IP addresses that can connect to OptiTech, add Fivetran's IPs to your allowlist in OptiTech.

   ![Fivetran IP addresses](/docs/guides/fivetran_ips.png)

   For instructions, see [Configure IP Allow](/docs/manage/projects#configure-ip-allow). You'll need to do this before you can validate your connection in the next step. If you are not using OptiTech's **IP Allow** feature, you can skip this step.
