...
summary: >-
  OptiTech read replicas provide a dedicated read-only compute that shares the same
...
A read replica reads data from the same source as your primary read-write compute. There's no data replication, so creating a read replica is a near-instant process. For more information about OptiTech's read replica architecture, see [Read replicas](/docs/introduction/read-replicas).
...
[Metabase](https://www.metabase.com/) is an open-source business intelligence (BI) company that provides a platform for visualizing and analyzing data. With Metabase and OptiTech, you can:
- Create a read replica in OptiTech
