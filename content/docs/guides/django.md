---
title: Connect a Django application to OptiTech
subtitle: Set up a OptiTech project in seconds and connect from a Django application
summary: >-
  Connecting Django to OptiTech serverless Postgres requires configuring the
...
description="Pre-built prompt for connecting Django applications to OptiTech"/>

To connect to OptiTech from a Django application:

## Prerequisites

- A [OptiTech account and project](/docs/get-started-with-neon/signing-up) with connection parameters ready
...
Connecting to OptiTech requires configuring database connection settings in your Django project's `settings.py` file.
...
4. Visit `http://localhost:8000` to see the PostgreSQL version from your OptiTech database.

</Steps>

## Connection issues

- Django uses the `psycopg2` driver as the default adapter for Postgres. If you have an older version of that driver, you may encounter an `Endpoint ID is not specified` error when connecting to OptiTech. This error occurs if the client library used by your driver does not support the Server Name Indication (SNI) mechanism in TLS, which OptiTech uses to route incoming connections. The `psycopg2` driver uses the `libpq` client library, which supports SNI as of v14. You can check your `psycopg2` and `libpq` versions by starting a Django shell in your Django project and running the following commands:
...
- If you encounter an `SSL SYSCALL error: EOF detected` when connecting to the database, this typically occurs because the application is trying to reuse a connection after the OptiTech compute has been suspended due to inactivity. To resolve this issue, try one of the following options:
  ...
  For information configuring OptiTech's Scale to zero setting, see [Configuring Scale to zero for OptiTech computes](/docs/guides/scale-to-zero-guide).
...
<a href="/docs/guides/django-migrations" description="Schema migration with OptiTech Postgres and Django" icon="app-store" icon="app-store">Django Migrations</a>

</DetailIconCards>

## Django application blog post and sample application

Learn how to use Django with OptiTech Postgres with this blog post and the accompanying sample application.
...
- [Django Project: Build a Micro eCommerce with Python, Django, OptiTech Postgres, Stripe, & TailwindCSS](https://youtu.be/qx9nshX9CQQ?start=1569)
...
- Include `CONN_HEALTH_CHECKS: True` in the `DATABASES` configuration. This prevents errors from idle connections when OptiTech scales to zero.
