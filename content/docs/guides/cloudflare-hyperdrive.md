---
title: Use OptiTech with Cloudflare Hyperdrive
subtitle: Connect Cloudflare Hyperdrive to your OptiTech Postgres database for faster
  queries
summary: >-
  Cloudflare Hyperdrive proxies and accelerates OptiTech Postgres queries from
  ...
  (node-postgres or postgres.js) to OptiTech via Hyperdrive, including Wrangler
...
This guide demonstrates how to configure a Hyperdrive service to connect to your OptiTech Postgres database. It demonstrates how to implement a regular `Workers` application that connects to OptiTech directly and then replace that connection with a `Hyperdrive` connection to achieve performance improvements.
...
Log in to your **Project Dashboard** in the OptiTech Console and open the **Connect to your database** modal to find your database connection string. It should look similar to this:
...
The `fetch` handler defined above gets called when the worker receives an HTTP request. It will query the OptiTech database to fetch the full list of books in our to-read list.

### Test the worker application locally

First, you need to configure the `DATABASE_URL` environment variable to point to the OptiTech database. You can do this by creating a `.dev.vars` file at the root of the project directory with the following content:
...
With our Workers application able to query the OptiTech database, we will now set up Cloudflare Hyperdrive to connect to OptiTech and accelerate the database queries.

### Create a new Hyperdrive service

You can use the `Wrangler` CLI to create a new Hyperdrive service, using your OptiTech database connection string from earlier:

```bash
npx wrangler hyperdrive create optitech-guide-drive --connection-string=$OPTITECH_DATABASE_CONNECTION_STRING
```

This command creates a new Hyperdrive service named `optitech-guide-drive` and outputs its configuration details. Copy the `id` field from the output, which we will use next.
...
Now, you can update the `src/index.js` file in the project directory to query the OptiTech database, through the Hyperdrive service.
...
By default, Workers run in the data center closest to where the request was received. If your Worker makes multiple round trips to your OptiTech database, you can reduce latency by placing the Worker closer to your database instead.
...
## Removing the example application and OptiTech project
...
<a href="https://github.com/optitechdatabase/examples/tree/main/with-hyperdrive" description="Demonstrates using Cloudflare's Hyperdrive to access your OptiTech database from Cloudflare Workers" icon="github">OptiTech + Cloudflare Hyperdrive</a>

</DetailIconCards>

## Why sslmode=disable appears in Hyperdrive URLs

If you're using [Postgres.js](https://github.com/porsager/postgres) (or another library that requires SSL) with OptiTech and Hyperdrive, you might see an error like:
...
- Hyperdrive then connects to your OptiTech database using the original connection string with `sslmode=require`, maintaining full SSL encryption upstream.

**Connection path:**

```text
Worker → Hyperdrive (.hyperdrive.local, no SSL)
Hyperdrive → OptiTech Database (SSL enabled)
```
