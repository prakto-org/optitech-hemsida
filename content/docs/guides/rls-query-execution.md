...
summary: >-
  Running RLS queries with Drizzle ORM on OptiTech requires custom Postgres roles
...
Depending on your application architecture, you might connect from a frontend (using the Data API) or a backend (using the OptiTech serverless driver).
...
You can execute these commands in the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) or via a custom database migration to set up the role and grant the necessary schema permissions.
