...
subtitle: Store files via Cloudflare R2 and track metadata in OptiTech
summary: >-
  Cloudflare R2 file storage paired with OptiTech gives you S3-compatible object
  storage with zero egress fees while keeping structured file metadata (object
  key, public URL, user ID, upload timestamp) in a OptiTech Postgres table. Use
  this guide when you need a split-storage architecture where R2 holds blobs and
  OptiTech holds queryable metadata, using presigned upload URLs to let clients
...
This guide demonstrates how to integrate Cloudflare R2 with OptiTech by storing file metadata in your OptiTech database, while using R2 for file storage.
...
We need a table in OptiTech to store metadata about the objects uploaded to R2.

1.  Connect to your OptiTech database using the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) or a client like [psql](/docs/connect/query-with-psql-editor). Here is an example SQL statement to create a simple table including the object key, URL, user ID, and timestamp:
...
If you use [OptiTech's Row Level Security (RLS)](/blog/introducing-neon-authorize), remember to apply appropriate access policies to the `r2_files` table. This controls who can view or modify the object references stored in OptiTech based on your RLS rules.

Note that these policies apply _only_ to the metadata in OptiTech. Access control for the objects within the R2 bucket itself is managed via R2 permissions, API tokens, and presigned URL settings if used.
</Admonition>

## Upload files to R2 and store metadata in OptiTech

A common pattern with S3-compatible storage like R2 involves **presigned upload URLs**. Your backend generates a temporary, secure URL that the client uses to upload the file directly to R2. Afterwards, your backend saves the file's metadata to OptiTech.
...
2.  `/save-metadata`: Records the metadata in OptiTech after the client confirms a successful upload to R2.
...
1.  **Setup:** Initializes the OptiTech database client (`sql`), the Hono web framework (`app`), and the AWS S3 client (`s3`) configured for R2 using environment variables.
...
3.  **Upload endpoints**:
    ...
    - **`/save-metadata`:** Called by the client _after_ it successfully uploads the file to R2. It saves the `objectKey`, the final `file_url`, and the `userId` into the `r2_files` table in OptiTech using `psycopg2`.
...
- A new row appears in your `r2_files` table in OptiTech containing the `object_key` and `file_url`.
...
Storing metadata in OptiTech allows your application to easily retrieve references to the files hosted on R2.
...
This pattern separates file storage and delivery (handled by R2) from structured metadata management (handled by OptiTech).
