...
subtitle: Learn how to replicate data from OptiTech to Databricks Lakehouse using the
  Lakeflow Connect PostgreSQL connector
summary: >-
  Replicate data from OptiTech Postgres to Databricks Lakehouse Delta tables using
  ...
  OptiTech into Unity Catalog, covering publication setup, replication slot
  creation, Unity Catalog connection, and pipeline configuration. The connector
  requires PostgreSQL 13 or later on the OptiTech primary; read replicas are not
  supported.
...
- **Network connectivity**: OptiTech must accept inbound connections from your Databricks workspace (or via VPN/PrivateLink, depending on setup). If you use [IP Allow](/docs/manage/projects#configure-ip-allow) in OptiTech, add the Databricks egress addresses to your allowlist. See [Subscriber access](/docs/guides/logical-replication-neon#subscriber-access)
- **PostgreSQL**: OptiTech Postgres running v13 or later with logical replication enabled on the primary. The connector does not support logical replication from a read replica
...
## Step 1: Prepare your OptiTech Postgres database

Configure OptiTech as a logical replication publisher that Lakeflow Connect can subscribe to.
...
1. In the OptiTech Console, open your project.
...
Set `PROJECT_ID` and `BRANCH_ID` from your OptiTech project (for example from the project URL in the Console or via the OptiTech API).
...
Lakeflow Connect uses logical replication slots with the `pgoutput` plugin. From your OptiTech database, connected as your replication role:
...
Logical replication slots retain WAL until a subscriber consumes it. If the Lakeflow ingestion gateway is stopped for too long, WAL can accumulate. Keep the gateway running continuously and monitor replication lag. OptiTech also [removes inactive replication slots](/docs/guides/logical-replication-neon#unused-replication-slots) after approximately 40 hours if progress is not acknowledged.
</Admonition>

At this point OptiTech is ready: `wal_level = logical`, a dedicated replication user with the required privileges, replica identities set on tables, `databricks_publication` defined, and `databricks_slot` created with `pgoutput`.

## Step 2: Create a Unity Catalog connection to OptiTech
...
5. Enter your OptiTech connection details (from the **Connect** button on your OptiTech project dashboard):
   ...
   - **Password**: the password for that role. You can obtain the password from the **Connect** modal on the OptiTech project dashboard.
...
- **Gateway pipeline**: Connects to OptiTech via the Unity Catalog connection, continuously extracts snapshot and CDC data using logical replication, and writes raw data into a staging volume in Unity Catalog.
...
7. **Database setup (OptiTech replication config)**: If requested, for each source OptiTech database, set:
   ...
   - **Publication name**: `databricks_publication`
     These must match the slot and publication you created in OptiTech. This step may not be necessary if you have only one replication slot. Optionally, click `Validate` to ensure your database setup is correct. This may take a few minutes. Issues are reported if encountered.
...
The gateway runs continuously on classic compute to keep up with OptiTech's WAL stream. The ingestion pipeline runs on serverless compute on the schedule you define.
...
- Keep the gateway running continuously to avoid WAL buildup on OptiTech.
- Periodically check replication slots on OptiTech and [drop unused slots](/docs/guides/logical-replication-manage#remove-a-replication-slot) if you delete pipelines; Databricks does not drop slots automatically.
...
- **Primary only**: The connector does not support logical replication from read replicas or standbys. Always point Lakeflow Connect at the OptiTech primary.
- **Authentication**: Username and password only; no IAM-based auth for the PostgreSQL connector.
- **Replication slots**: The gateway must run continuously to prevent WAL bloat. Replication slots are not removed when you delete a pipeline; drop them explicitly in OptiTech.
...
- **OptiTech compatibility**: The connector supports PostgreSQL 13+ on various managed and self-hosted sources. OptiTech is a managed Postgres service that exposes a standard primary with logical replication; validate in a non-production environment and coordinate with Databricks Support for mission-critical use.

For adding tables, monitoring lag, and cleaning up slots, see Databricks' [Maintain PostgreSQL ingestion pipelines](https://docs.databricks.com/aws/en/ingestion/lakeflow-connect/postgresql-maintenance) documentation and [Logical replication in OptiTech](/docs/guides/logical-replication-neon).
...
- [Logical replication in OptiTech](/docs/guides/logical-replication-neon)
