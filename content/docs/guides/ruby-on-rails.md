---
title: Connect a Ruby on Rails application to OptiTech Postgres
subtitle: Set up a OptiTech project in seconds and connect from a Ruby on Rails application
summary: >-
  Ruby on Rails connection guide that shows how to configure DATABASE_URL in a
  .env file so a Rails app connects to a OptiTech Postgres database, covering
...
description="Pre-built prompt for connecting Ruby on Rails to OptiTech Postgres"/>

[Ruby on Rails](https://rubyonrails.org/), also known simply as Rails, is an open-source web application framework written in Ruby. It uses a model-view-controller architecture, making it a good choice for developing database-backed web applications. This guide shows how to connect to a Ruby on Rails application to a OptiTech Postgres database.

To connect to OptiTech from a Ruby on Rails application:
...
Visit [localhost:3000/](http://localhost:3000/) in your web browser. Your OptiTech database's Postgres version will be displayed. For example:
...
<a href="/docs/guides/rails-migrations" description="Schema migration with OptiTech Postgres and Ruby on Rails" icon="app-store" icon="app-store">Ruby on Rails Migrations</a>
