---
title: Connect a React Router application to OptiTech
subtitle: Set up a OptiTech project in seconds and connect from a React Router application
summary: >-
  React Router full-stack integration with OptiTech Postgres runs database queries
...
To create a OptiTech project and access it from a React Router application:
...
With React Router, data fetching is handled in "Route Modules". We will create a new route that connects to OptiTech in its `loader` function, which runs on the server.
...
Create a new file at `app/routes/version.tsx`. This file will contain both the server-side data loader and the client-side React component. The `loader` function will connect to OptiTech, query the database version, and pass the result to the `Component` via the `loaderData` prop.
...
Now, navigate to [http://localhost:5173/version](http://localhost:5173/version) in your browser. You should see a page displaying the version of your OptiTech Postgres database.
