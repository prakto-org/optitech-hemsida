...
In this guide, you will learn how to stream data from your OptiTech Postgres database to Materialize using the Materialize [PostgreSQL source](https://materialize.com/docs/sql/create-source/postgres/).
...
After logical replication is enabled in OptiTech, the next step is to create a publication for the tables that you want to replicate to Materialize.

1. From a `psql` client connected to your OptiTech database or from the **OptiTech SQL Editor**, set the [replica identity](https://www.postgresql.org/docs/current/sql-altertable.html#SQL-ALTERTABLE-REPLICA-IDENTITY) to `FULL` for each table that you want to replicate to Materialize:
...
If you use OptiTech's **IP Allow** feature to limit IP addresses that can connect to OptiTech, you will need to allow inbound traffic from Materialize IP addresses. If you are currently not limiting IP address access in OptiTech, you can skip this step.
...
In this case, you’ll create 1 new cluster containing 1 medium replica for ingesting source data from your OptiTech Postgres database.
...
Now that you’ve configured your database network and created an ingestion cluster, you can connect Materialize to your OptiTech Postgres database and start ingesting data.
...
1. From a `psql` client connected to Materialize or from the Materialize **SQL Shell**, use the [CREATE SECRET](https://materialize.com/docs/sql/create-secret/) command to securely store the password for the Postgres role you created earlier:
   ...
   You can access the password for your OptiTech Postgres role from the **Connect to your database** modal; click the **Connect** button on your **Project Dashboard** to open the modal.
...
3. Use the [CREATE SOURCE](https://materialize.com/docs/sql/create-source/) command to connect Materialize to your OptiTech Postgres database and start ingesting data from the publication you created earlier:
...
3.  Going forward, you can verify that your new replica size is sufficient as follows:
    ...
    b. From a `psql` client connected to your OptiTech database or from the **OptiTech SQL Editor**, check the replication slot lag, using the replication slot name from the previous step:
        ...
        The result of this query is the amount of data your Postgres cluster must retain in its replication log because of this replication slot. Typically, this means Materialize has not yet communicated back to your OptiTech Postgres database that it has committed this data. A high value can indicate that the source has fallen behind and that you might need to scale up your ingestion cluster.
