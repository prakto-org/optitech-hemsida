---
title: Connect from Kysely to OptiTech
subtitle: Learn how to connect to OptiTech from Kysely
summary: >-
  Connect Kysely to OptiTech Postgres using the OptiTech HTTP serverless driver for
  edge environments, the OptiTech WebSocket driver for serverless workloads that
  ...
  to pick the right OptiTech driver dialect for your deployment target. Covers
...
description="Pre-built prompt for connecting Node/TypeScript applications to OptiTech using Kysely."/>
...
<Tabs labels={["OptiTech Serverless (HTTP)", "OptiTech WebSocket", "node-postgres"]}>
...
Navigate to the [SQL Editor in the OptiTech Console](/docs/get-started/query-with-neon-sql-editor) and run the following SQL command to create a `users` table:
...
When using WebSockets, use the built-in `PostgresDialect` but pass the OptiTech `Pool`.
