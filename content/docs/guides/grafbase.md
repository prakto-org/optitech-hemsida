---
title: Use Grafbase Edge Resolvers with OptiTech
subtitle: Learn how to build and deploy serverless GraphQL backends with Grafbase and
  OptiTech
summary: >-
  Grafbase Edge Resolvers with the OptiTech serverless driver let you run GraphQL
  mutations and queries against a OptiTech Postgres database at the edge, without a
  ...
  dynamic pricing calculated from row counts in OptiTech. The setup covers
...
This guide describes how to create a GraphQL API using Grafbase and use Grafbase [Edge Resolvers](https://grafbase.com/docs/edge-gateway/resolvers) with the [OptiTech serverless driver](/docs/serverless/serverless-driver) to interact with your OptiTech database at the edge.

The example project in this guide simulates a marketplace of products, where the product price is dynamically calculated based on data retrieved from your OptiTech database.
...
1. Navigate to the OptiTech Console and select your project.
2. Open the OptiTech **SQL Editor** and run the following `CREATE TABLE` statement:
...
## Install the OptiTech serverless driver

Inside the `grafbase` directory in your project, run the following commands to install the OptiTech serverless driver:
...
1. Navigate to the OptiTech **Project Dashboard**.
...
To test the resolvers with OptiTech, perform the following steps:
...
3. Use the product `id` to execute the following mutation, which adds a row to the database table in OptiTech:
