---
title: Connect a SolidStart application to OptiTech
subtitle: Set up a OptiTech project in seconds and connect from a SolidStart application
summary: >-
  SolidStart integration with OptiTech walks through creating a OptiTech project and
...
description="Pre-built prompt for connecting SolidStart applications to OptiTech"/>

SolidStart is an open-source meta-framework designed to integrate the components that make up a web application.<sup><a target="_blank" href="https://docs.solidjs.com/solid-start#overview">1</a></sup>. This guide explains how to connect SolidStart with OptiTech using a secure server-side request.

To create a OptiTech project and access it from a SolidStart application:
...
To [load data on the server](https://docs.solidjs.com/solid-start/building-your-application/data-loading#data-loading-always-on-the-server) in SolidStart, add the following code snippet to connect to your OptiTech database:
...
In your server endpoints (API Routes) in your SolidStart application, use the following code snippet to connect to your OptiTech database:
