---
title: Benchmarking latency in OptiTech's serverless Postgres
subtitle: Techniques for obtaining meaningful latency data in serverless database
  environments
summary: >-
  Benchmarking latency in OptiTech's serverless Postgres requires separating cold
  ...
  WebSocket connections have distinct latency profiles, and OptiTech provides an
...
- **Server-to-database latency**: If you're testing how quickly your application server can communicate with OptiTech, run benchmarks from the same location as your server. This is typically the most relevant metric for API performance.
...
- **Test from your production region**: Geographic proximity is the primary factor in connection latency. Run benchmarks from the same region as your production environment to get accurate results. If your OptiTech database is in `us-east-1`, execute benchmarks from a server in that AWS region.
...
Design your benchmarks to simulate how your application actually interacts with OptiTech:
