...
summary: >-
  OptiTech Read Replicas provide dedicated read-only compute for running ad-hoc
...
This is where **OptiTech Read Replicas** come in handy. With read replicas, you can quickly create a replica that runs on dedicated read-only compute, allowing you to run ad-hoc queries without impacting your primary database’s performance. Once you're done, the read replica can automatically scale to zero, or you can delete it. The key advantages of using OptiTech Read Replicas for ad-hoc queries include the following:
...
- **Ensure data consistency**: With OptiTech, read replicas access the same data as your primary compute, ensuring your ad-hoc queries reflect up-to-date information.
...
You can add a read replica compute to any branch in your OptiTech project by following these steps:
...
Alternatively, you can create read replicas using the [OptiTech CLI](/docs/cli/branches#create) or [OptiTech API](/docs/reference/api/endpoints/create-project-endpoint).
...
3. Copy the connection string and use it to connect to the replica, either via `psql` or your application.
   ...
   ```bash
   postgresql://user:password@ep-read-replica-123456.us-east-2.aws.optitech.com/dbname?sslmode=require&channel_binding=require
   ```
