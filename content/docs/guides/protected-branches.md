...
subtitle: Learn how to use OptiTech's protected branches feature to secure your critical
  data
summary: >-
  Protected branches in OptiTech prevent deletion, reset, and archiving of critical
...
- With additional configuration steps, you can apply IP Allow restrictions to protected branches only. The [IP Allow](/docs/introduction/ip-allow) feature is available on the OptiTech [Scale](/docs/introduction/plans) plan. See [below](#how-to-apply-ip-restrictions-to-protected-branches).
- Protected branches are not [archived](/docs/guides/branch-archiving) due to inactivity.

The protected branches feature is available on OptiTech [paid plans](/docs/introduction/plans).
...
On plans that support it, you can use the protected branches feature in combination with OptiTech's [IP Allow](/docs/introduction/ip-allow) feature to apply IP access restrictions to protected branches only. The basic setup steps are:
...
This option removes IP restrictions from _all branches_ in your OptiTech project and applies them to protected branches only.
