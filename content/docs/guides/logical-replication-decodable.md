...
subtitle: Learn how to replicate data from OptiTech with Decodable
summary: >-
  Logical replication from OptiTech Postgres to Decodable uses a Postgres CDC
  ...
  capture from OptiTech with optional SQL or Flink transformations before data
  reaches its destination. Setup requires enabling wal_level=logical on the OptiTech
...
In this guide, you will learn how to configure a Postgres source connector in Decodable for ingesting changes from your OptiTech database so that you can replicate data from OptiTech to any of Decodable's [supported data sinks](https://docs.decodable.co/connections.html#sinks), optionally processing the data with SQL or custom Flink jobs.
...
To do so, issue the following statement in the **OptiTech SQL Editor**:
...
If you are using OptiTech's **IP Allow** feature to limit the IP addresses that can connect to OptiTech, you will need to allow inbound traffic from Decodable's IP addresses.
...
For information about configuring allowed IPs in OptiTech, see [Configure IP Allow](/docs/manage/projects#configure-ip-allow).
...
At this point, you have a running connector, which continuously ingests changes from a OptiTech database into Decodable with low latency.
...
Of course, you also can take your processed data back to another OptiTech database, using the Decodable sink connector for Postgres.
