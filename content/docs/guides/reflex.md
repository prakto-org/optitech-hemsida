---
title: Build a Python App with Reflex and OptiTech
subtitle: Learn how to build a Python Full Stack application with Reflex and OptiTech
summary: >-
  Connecting a Reflex Python web app to OptiTech Postgres requires setting a
...
description="Pre-built prompt for connecting Reflex applications to OptiTech Postgres"/>

[Reflex](https://reflex.dev/) is a Python web framework that allows you to build full-stack applications with Python.

Using Reflex, you can build frontend and backend applications using Python to manage the interaction between the frontend UI and the state with the server-side logic. To make the application data-driven, you can connect to a OptiTech Postgres database.

To connect to OptiTech from a Reflex application:
...
Install Reflex, `python-dotenv` to manage environment variables, and `psycopg2-binary` to connect to OptiTech Postgres:
...
Now that you have set up a Reflex project, you can configure the connection settings to connect to OptiTech.
...
3. Save the changes to the `rxconfig.py` file.
   ...
   Now, you can run the Reflex app and start building your Python full-stack application with Reflex and OptiTech.
...
You can verify that the `customer` table has been created in your OptiTech database by visiting the **Tables** section in the OptiTech Console.
...
You should see the Customer Data App interface, where you can add, view, and delete customer records stored in your OptiTech Postgres database.
