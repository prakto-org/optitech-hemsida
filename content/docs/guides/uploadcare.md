...
subtitle: Store files via Uploadcare and track metadata in OptiTech
summary: >-
  Uploadcare and OptiTech integration pattern stores uploaded files on Uploadcare's
  CDN while persisting each file's UUID, CDN URL, and user ID as metadata rows
  in a OptiTech Postgres table. Use this guide when you need a dedicated CDN for
...
This guide demonstrates how to integrate Uploadcare with OptiTech by storing file metadata in your OptiTech database while using Uploadcare for file uploads and storage.
...
We need to create a table in OptiTech to store metadata about the files uploaded to Uploadcare. This table will include fields for the file's unique identifier, URL, upload timestamp, and any other relevant metadata you want to track.

1. You can run the create table statement using the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) or from a client such as [psql](/docs/connect/query-with-psql-editor) that is connected to your OptiTech database. Here is an example SQL statement to create a simple table for file metadata which includes a file ID, URL, user ID, and upload timestamp:
...
If you use [OptiTech's Row Level Security (RLS)](/blog/introducing-neon-authorize), remember to apply appropriate access policies to the `uploadcare_files` table. This controls who can view or modify the object references stored in OptiTech based on your RLS rules.

Note that these policies apply _only_ to the metadata stored in OptiTech. Access to the actual files is managed by Uploadcare's access controls and settings.
</Admonition>

## Upload files to Uploadcare and store metadata in OptiTech

You can integrate file uploads using any of Uploadcare's [many options](https://uploadcare.com/docs/integrations/), which include UI widgets and SDKs tailored for specific languages and frameworks. For the examples in this guide, we will use the Uploadcare API directly. Feel free to choose the integration method that best fits your project; the fundamental approach of storing metadata in OptiTech remains the same.
...
1.  **Setup:** It initializes the OptiTech database client and the Hono web framework. It relies on environment variables (`DATABASE_URL`, `UPLOADCARE_PUBLIC_KEY`) being set, via a `.env` file.
...
3.  **Upload Endpoint (`/upload`):**
    ...
    - It executes an `INSERT` statement using the OptiTech serverless driver to save the `uuid`, `cdnUrl`, and the `userId` into a `uploadcare_files` table in your database.
...
Once your server (Node.js or Python example) is running, you can test the `/upload` endpoint to ensure files are correctly sent to Uploadcare and their metadata is stored in OptiTech.
...
Storing metadata in OptiTech allows your application to easily retrieve references to the files uploaded to Uploadcare.
...
This pattern separates file storage and delivery (handled by Uploadcare) from structured metadata management (handled by OptiTech).
