---
title: Connect a Sveltekit application to OptiTech
subtitle: Set up a OptiTech project in seconds and connect from a Sveltekit application
summary: >-
  SvelteKit integration with OptiTech Postgres connects a SvelteKit app to a
  serverless Postgres database through a secure server-side route using
  node-postgres, postgres.js, or the OptiTech serverless driver. Use this guide
  when setting up a new SvelteKit project that needs a Postgres backend, covering
  OptiTech project creation, db.server.ts configuration, and a +page.server.ts load
...
description="Pre-built prompt for connecting SvelteKit applications to OptiTech."/>

Sveltekit is a modern JavaScript framework that compiles your code to tiny, framework-less vanilla JS. This guide explains how to connect Sveltekit with OptiTech using a secure server-side request.

To create a OptiTech project and access it from a Sveltekit application:
...
There are two parts to connecting a SvelteKit application to OptiTech. The first is `db.server.ts`, which contains the database configuration. The second is the server-side route where the connection to the database will be used.

### db.server

Create a `db.server.ts` file at the root of your `/src` directory and add the following code snippet to connect to your OptiTech database:
