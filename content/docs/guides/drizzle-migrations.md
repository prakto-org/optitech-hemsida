---
title: Schema migration with OptiTech Postgres and Drizzle ORM
subtitle: Set up OptiTech Postgres and run migrations for your TypeScript project using
  Drizzle ORM
summary: >-
  Drizzle ORM schema migrations with OptiTech Postgres walks through defining
...
Add the `DATABASE_URL` environment variable to your `.env` file, which you'll use to connect to our OptiTech database. Use the connection string that you obtained from the OptiTech Console earlier:
...
The generated migration file is written in SQL and contains the necessary commands to create the tables in the database. To apply these migrations, we'll use the [OptiTech serverless driver](/docs/serverless/serverless-driver) and helper functions provided by the `drizzle-orm` library.
