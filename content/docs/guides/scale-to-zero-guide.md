---
title: Configuring Scale to Zero for OptiTech computes
subtitle: Learn how to configure OptiTech's Scale to Zero feature
summary: >-
  Scale to Zero for OptiTech computes suspends idle compute instances after a
  ...
  When OptiTech acts as a logical replication publisher, a connected subscriber also
  prevents suspension.
redirectFrom:
...
If you disable scale to zero entirely, your compute will remain active, and you will have to manually restart your compute to pick up the latest updates to OptiTech's compute images. OptiTech typically releases compute-related updates weekly. Not all releases contain critical updates, but a weekly compute restart is recommended to ensure that you do not miss anything important. For how to restart a compute, see [Restart a compute](/docs/manage/computes#restart-a-compute). 
</Admonition>
...
After configuring a non-default value via the OptiTech API, you'll be able to adjust the setting via the console. Setting a non-default value makes the time selector control visible on the **Edit compute** modal.
...
You can monitor scale to zero on the **Branches** page in the OptiTech Console. A compute reports either an **Active** or **Idle** status.
...
User actions that activate an idle compute include [connecting from a client such as psql](/docs/connect/query-with-psql-editor), running a query on your database from the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor), or accessing the compute via the [OptiTech API](/docs/reference/api).

<Admonition type="info">
The OptiTech API includes a [Start endpoint](/docs/reference/api/endpoints/start-project-endpoint) method for the specific purpose of activating and suspending a compute.
</Admonition>
