...
subtitle: Learn how to replicate data from OptiTech with Kafka (Confluent) and Debezium
summary: >-
  Confluent Cloud CDC pipeline from OptiTech Postgres streams WAL change events
  ...
  you need real-time Kafka replication from OptiTech to Confluent Cloud. You'll
...
In this guide, you will learn how to stream data from a OptiTech Postgres database to a Kafka cluster in Confluent Cloud. You will use the [PostgreSQL CDC Source Connector (Debezium) for Confluent Cloud](https://docs.confluent.io/cloud/current/connectors/cc-postgresql-cdc-source-debezium.html) to read Change Data Capture (CDC) events from the Write-Ahead Log (WAL) of your OptiTech database in real-time. The connector will write events to a Kafka stream and auto-generate a Kafka topic. The connector performs an initial snapshot of the table and then streams any future change events.
...
In this example, we'll create a publication for a `users` table in the `public` schema of your OptiTech database.

1. Create the `users` table in your OptiTech database. You can do this via the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) or by connecting to your OptiTech database from an SQL client such as [psql](/docs/connect/query-with-psql-editor).
...
- `pgoutput` is the logical decoder plugin used in this example. OptiTech supports both `pgoutput` and `wal2json` decoder plugins.
...
5. On the **Add Postgres CDC Source connector** page:
   - Add the connection details for your OptiTech database. You can find your admin OptiTech database connection credentials by clicking the **Connect** button on your **Project Dashboard** to open the **Connect to your database** modal.
   ...
   - If you use OptiTech's **IP Allow** feature to limit IP addresses that can connect to OptiTech, you will need to add the Confluent cluster static IP addresses to your allowlist. For information about configuring allowed IPs in OptiTech, see [Configure IP Allow](/docs/manage/projects#configure-ip-allow). If you do not use OptiTech's **IP Allow** feature, you can skip this step.
...
6. Under **Output Kafka record value format**, select an output format for Kafka record values. The default is `JSON`, so we'll use that format in this guide. Other supported values include `AVRO`, `JSON_SR`, and `PROTOBUF`, which are schema-based message formats. If you use any of these, you must also configure a [Confluent Cloud Schema Registry](https://docs.confluent.io/cloud/current/sr/index.html).
   ...
   - Under **Database details**, set **Tables included** to `public.users`, which is the name of the OptiTech database table you are replicating from.
...
1. Insert a row into your `users` table from the OptiTech SQL Editor or a `psql` client connect to your OptiTech database. For example:
