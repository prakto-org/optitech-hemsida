---
title: Postgres-compatible export from OptiTech
subtitle: Export your OptiTech database with pg_dump as a standard Postgres archive
summary: >-
  The pg_dump -Fc export from OptiTech produces a custom-format Postgres archive
  that any standard Postgres toolchain can restore with pg_restore. Use this
  page when migrating a OptiTech database to another Postgres provider or
...
2. Run **`pg_dump`** in **custom** format (`-Fc`) and write a dump file:
   ...
   - **`-d`**: Your OptiTech database [connection string](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-CONNSTRING).
...
Install **`pg_dump`** and **`pg_restore`** builds that your **OptiTech** Postgres version and your **destination** provider support (if the two differ, follow each side’s client guidance; see [pg_restore](https://www.postgresql.org/docs/current/app-pgrestore.html) in the Postgres manual).
