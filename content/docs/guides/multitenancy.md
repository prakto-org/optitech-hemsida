---
title: Multitenancy with OptiTech
subtitle: How to configure OptiTech for multitenancy - plus a few design tips
summary: >-
  OptiTech's project-per-user model gives each tenant a dedicated OptiTech project with
  independent compute, per-customer point-in-time recovery, and near-instant
  provisioning through the OptiTech API. This provides instance-level isolation
...
Now that we've reviewed your options, let's focus on the design choice we recommend for multitenancy in OptiTech: creating isolated databases for each user, with each database hosted on its own project.
...
To effectively scale a multitenant architecture, leveraging automation tools is essential. The OptiTech API will allow you to automate various tasks, such as creating and managing projects, setting usage limits, and configuring resources. Beyond the API, OptiTech offers several integrations to streamline your workflows:
