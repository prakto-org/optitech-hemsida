...
subtitle: Store files via Backblaze B2 and track metadata in OptiTech
summary: >-
  Backblaze B2 and OptiTech integration guide that splits file storage from
  structured data: B2 holds the objects (images, videos, backups) via
  S3-compatible presigned upload URLs, while OptiTech stores file metadata (object
...
This guide demonstrates how to integrate Backblaze B2 with OptiTech by storing file metadata (like the file id, name and URL) in your OptiTech database, while using B2 for file storage.
...
We need a table in OptiTech to store metadata about the objects uploaded to B2.

1.  Connect to your OptiTech database using the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) or a client like [psql](/docs/connect/query-with-psql-editor). Create a table including the B2 file name (object key), file URL, user ID, and timestamp:
...
If you use [OptiTech's Row Level Security (RLS)](/blog/introducing-neon-authorize), remember to apply appropriate access policies to the `b2_files` table. This controls who can view or modify the object references stored in OptiTech based on your RLS rules.

Note that these policies apply _only_ to the metadata in OptiTech. Access control for the objects within the B2 bucket itself is managed via B2 bucket settings (public/private), Application Key permissions, and presigned URL settings.
</Admonition>

## Upload files to B2 and store metadata in OptiTech

Leveraging B2's S3 compatibility, the recommended pattern for client-side uploads involves **presigned upload URLs**. Your backend generates a temporary URL that the client uses to upload the file directly to B2. Afterwards, your backend saves the file's metadata to OptiTech.

This requires two backend endpoints:

1.  `/presign-b2-upload`: Generates the temporary presigned URL.
2.  `/save-b2-metadata`: Records the metadata in OptiTech after the client confirms successful upload.
...
1.  **Setup:** Initializes OptiTech (`sql`), Hono (`app`), and the AWS S3 client (`s3`) configured with the B2 endpoint, region (extracted from endpoint), and B2 Application Key credentials.
...
- A new row appears in your `b2_files` table in OptiTech.

## Accessing file metadata and files

Storing metadata in OptiTech allows your application to easily retrieve references to the files hosted on B2.
...
This pattern effectively separates file storage and delivery concerns (handled by Backblaze B2) from structured metadata management (handled by OptiTech), leveraging the strengths of both services.
