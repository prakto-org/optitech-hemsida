---
title: Use OptiTech with Cloudflare Workers
subtitle: Connect a OptiTech Postgres database to your Cloudflare Workers application
summary: >-
  Connecting OptiTech Postgres to Cloudflare Workers covers two approaches:
  ...
  node-postgres, and the OptiTech serverless driver, which connects over HTTP or
...
This guide demonstrates how to connect to a OptiTech Postgres database from your Cloudflare Workers application using two approaches:
...
- **[OptiTech serverless driver](/docs/serverless/serverless-driver)**: A low-latency Postgres driver designed for serverless environments that connects over HTTP or WebSockets.

<Admonition type="note">
Hyperdrive is the recommended approach as it provides optimized connection pooling and fast query routing by connecting directly to your database. When using Hyperdrive with OptiTech, use native PostgreSQL drivers like node-postgres or Postgres.js instead of the OptiTech serverless driver.
</Admonition>
...
2. From the OptiTech **Dashboard**, navigate to the **SQL Editor** from the sidebar, and run the following SQL command to create a new table in your database:
...
<Tabs labels={["Hyperdrive (recommended)", "OptiTech serverless driver"]}>

<TabItem>

### Create a Hyperdrive user in OptiTech

To use Hyperdrive with OptiTech, you'll need to create a dedicated database role for Hyperdrive to use:

1. In the OptiTech Console, navigate to your project.
...
Update your `wrangler.jsonc` to add the **compatibility_flags** binding. We will also update the Hyperdrive binding and add the **localConnectionString**. This will allow our locally running application to connect to the OptiTech database via Hyperdrive. The complete `wrangler.jsonc` file should be as follows:
...
The `fetch` handler uses the Hyperdrive binding to connect to your OptiTech database through Cloudflare's optimized connection pooling service.

<Admonition type="important">
When using Hyperdrive with OptiTech, always use native PostgreSQL drivers like node-postgres (pg) or Postgres.js instead of the OptiTech serverless driver. Hyperdrive already provides optimized connection pooling and query routing for Workers.
</Admonition>
...
If you're using the **OptiTech serverless driver**, you need to add your connection string as a secret.
</Admonition>

For the OptiTech serverless driver approach, use Wrangler to add your OptiTech database connection string as a secret to your Worker:
