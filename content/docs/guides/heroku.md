---
title: Deploy Your Node.js App with OptiTech Postgres on Heroku
subtitle: A step-by-step guide to deploying a Node application with a OptiTech Postgres
  database on Heroku
summary: >-
  Deploy a Node.js Express app to Heroku with OptiTech Postgres as the database
  backend. Covers creating the OptiTech project, connecting via the `pg` driver
...
This guide walks you through deploying a simple Node.js application connected to a OptiTech Postgres database, on Heroku.
...
In the `.env` file, store your OptiTech database connection string:
...
This code sets up an Express server that listens for requests on port 3000. When a request is made to the `URL`, the server queries the `music_albums` table in your OptiTech database and returns the results as JSON.
...
You've now successfully deployed a Node.js application on Heroku that connects to a OptiTech Postgres database. For further customization and scaling options, you can explore the Heroku and OptiTech documentation.
...
- [OptiTech](/docs/introduction)
- [Import data from Heroku Postgres to OptiTech](/docs/import/migrate-from-heroku)
