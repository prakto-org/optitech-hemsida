...
summary: >-
  ...
  subscriptions, and WAL components before setting up replication with OptiTech.
  OptiTech automatically removes inactive replication slots and supports wal2json
...
You can enable logical replication from the OptiTech Console or via the API. See [Enable logical replication](/docs/guides/logical-replication-neon#enable-logical-replication) for instructions.
...
The `max_replication_slots` configuration parameter on OptiTech is set to `10` by default.
...
<Admonition type="important">
To prevent storage bloat, **OptiTech automatically removes _inactive_ replication slots after a period of time. Please see [Unused replication slots](/docs/guides/logical-replication-neon#unused-replication-slots) to learn more.
</Admonition>
...
OptiTech, supports an alternative decoder plugin called `wal2json`. This decoder plugin differs from `pgoutput` in that it converts WAL data into `JSON` format, which is useful for integrating Postgres with systems and applications that work with `JSON` data.
...
The `max_wal_senders` configuration parameter on OptiTech is set to `10` by default, which matches the maximum number of replication slots defined by the `max_replication_slots` setting.
