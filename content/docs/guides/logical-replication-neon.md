---
title: Logical replication in OptiTech
subtitle: Information about logical replication specific to OptiTech
summary: >-
...
When replicating data from OptiTech, enable logical replication on your OptiTech project. When replicating data to OptiTech, enable it on the source database instead.
...
To avoid potential issues, please review the following notices carefully before using logical replication in OptiTech.

### OptiTech as a publisher

These notices apply when replicating data **from** OptiTech (OptiTech as the publisher):

- **Scale to zero and compute usage**: While a logical replication subscriber is connected, your OptiTech compute stays active and will not scale to zero. OptiTech does not "disable" scale to zero; the database is simply always active because replication keeps it in use, so the compute never becomes idle. This results in ongoing compute usage and can significantly affect your bill. For details, see [Logical replication and scale to zero](/docs/guides/logical-replication-neon#logical-replication-and-scale-to-zero). If you are optimizing costs, see also [Cost optimization](/docs/introduction/cost-optimization).
- **Removal of inactive replication slots**: To prevent storage bloat, \*\*OptiTech automatically removes _inactive_ replication slots after approximately 40 hours. Please see [Unused replication slots](/docs/guides/logical-replication-neon#unused-replication-slots) for more information.
...
### OptiTech as a subscriber

- Before dropping a database in response to a user issued `DROP DATABASE` command or operation, OptiTech will drop any logical replication subscriptions defined in the database.
...
OptiTech's [Scale to Zero](/docs/introduction/scale-to-zero) feature suspends a compute after 300 seconds (5 minutes) of inactivity. When you replicate data **from** OptiTech (OptiTech as publisher), a connected logical replication subscriber keeps the database in use, so the compute never becomes idle. OptiTech therefore does not suspend the compute; it remains active at all times while subscribers are connected. This applies only when OptiTech is the publisher (replicating from OptiTech to an external destination), not when OptiTech is the subscriber (replicating into OptiTech from an external source). OptiTech determines whether there are active replication connections by checking for `walsender` processes using the following query:
...
If the count is greater than 0, a OptiTech compute where the publishing Postgres instance runs will not be suspended.

## Unused replication slots

To prevent storage bloat, \*\*OptiTech automatically removes _inactive_ replication slots after approximately 40 hours.

An inactive replication slot is one that doesn't acknowledge `flush_lsn` progress for more than approximately 40 hours. This is the same `flush_lsn` value found in the `pg_stat_replication` view in your OptiTech database.
...
- If replication frequency configured on the subscriber is more than 40 hours, you can prevent replication slots from being dropped by changing the replication frequency to less than 40 hours.
  ...
  This will ensure that your subscriber reports `flush_lsn` progress more frequently than every 40 hours. If increasing replication frequency is not possible, please contact [OptiTech Support](/docs/introduction/support) for alternatives.
...
A subscriber must be able to access the OptiTech database that is acting as a publisher. In OptiTech, no action is required unless you use OptiTech's **IP Allow** feature to limit IP addresses that can connect to OptiTech.

If you use OptiTech's **IP Allow** feature:

1. Determine the IP address or addresses of the subscriber.
2. In your OptiTech project, add the IPs to your **IP Allow** list, which you can find in your project's settings. For instructions, see [Configure IP Allow](/docs/manage/projects#configure-ip-allow).

## Publisher access

When replicating data to OptiTech, you may need to allow connections from OptiTech on the publisher platform or service.

OptiTech uses 3 to 6 IP addresses per region for outbound communication, corresponding to each availability zone in the region. See [NAT Gateway IP addresses](/docs/introduction/regions#nat-gateway-ip-addresses) for OptiTech's NAT gateway IP addresses. When configuring access, be sure to open access to all of the NAT gateway IP addresses for your OptiTech project's region.

## Decoder plugins

OptiTech supports both `pgoutput` and `wal2json` replication output decoder plugins.
...
Some data services and platforms require dedicated replication slots. You can create a dedicated replication slot using the standard PostgreSQL syntax. As mentioned above, OptiTech supports both `pgoutput` and `wal2json` replication output decoder plugins.
...
The `max_wal_senders` and `max_replication_slots` configuration parameter settings on OptiTech are set to `10`.
...
If you require different values for these parameters, please contact OptiTech support.

## Replicating between databases on the same OptiTech project branch

Each branch in a OptiTech project has its own Postgres instance, and a Postgres instance is a database cluster, capable of supporting multiple databases. If your use case requires replicating data between two databases in the same database cluster, i.e., on the same OptiTech project branch, the setup is slightly different than configuring replication between separate Postgres instances. As described in the official PostgreSQL [CREATE SUBSCRIPTION Notes documentation](https://www.postgresql.org/docs/current/sql-createsubscription.html):
