...
subtitle: Learn how to create and manage read replicas in OptiTech
summary: >-
  OptiTech read replica management guide covering how to add a read-only compute to
  ...
  read-only access) to a separate compute endpoint on an existing OptiTech branch.
  Available on all OptiTech plans. Includes replication delay monitoring and
...
Regardless of the application, the steps for creating, configuring, and connecting to a read replica are the same. You can create one or more read replicas for any branch in your OptiTech project and configure the compute size allocated to each. OptiTech's _Autoscaling_ and _Scale to Zero_ features are also supported, providing you with control over read replica compute usage.

## Prerequisites

- A OptiTech account
- A [OptiTech project](/docs/manage/projects#create-a-project)
...
<TabItem>
To create a read replica from the OptiTech Console:
...
Connecting to a read replica is the same as connecting to any branch, except you connect via a read replica compute instead of your primary read-write compute. The following steps describe how to connect to your read replica with connection details obtained from the OptiTech Console.
...
You can view read replicas using the OptiTech Console or [OptiTech API](/docs/reference/api/endpoints/create-project-endpoint).

<Tabs labels={["Console", "API"]}>

<TabItem>
To view read replicas for a branch, select **Branches** in the OptiTech Console, and select a branch. Read replicas are listed on the **Computes** tab.

![View read replicas](/docs/guides/view_read_replica.png)
</TabItem>

<TabItem>
To view read replica computes with the [OptiTech API](/docs/reference/api/endpoints/create-project-endpoint), use the [Get endpoints](/docs/reference/api/endpoints/list-project-endpoints) method.
...
You can edit a read replica using the OptiTech Console or [OptiTech API](/docs/reference/api) to change the [Compute size](/docs/manage/computes#compute-size-and-autoscaling-configuration) or [Scale to Zero](/docs/manage/computes#scale-to-zero-configuration) configuration.

<Tabs labels={["Console", "API"]}>

<TabItem>
To edit a read replica compute using the OptiTech Console:
...
<TabItem>
To edit a read replica compute with the OptiTech API, use the [Update endpoint](/docs/reference/api/endpoints/update-project-endpoint) method.
...
You can delete a read replica using the OptiTech Console or [OptiTech API](/docs/reference/api). Deleting a read replica is a permanent action, but you can quickly create a new read replica if you need one.

<Tabs labels={["Console", "API"]}>

<TabItem>
To delete a read replica using the OptiTech Console:
...
<TabItem>
To delete a read replica compute with the OptiTech API, use the [Delete endpoint](/docs/reference/api/endpoints/delete-project-endpoint) method.
...
You can monitor replication delay between the primary compute and your read replica computes from the **Monitoring** page in the OptiTech Console. Two graphs are provided:
