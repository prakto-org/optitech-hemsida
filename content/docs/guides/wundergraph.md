---
title: Use WunderGraph with OptiTech
subtitle: Leverage the power of OptiTech and WunderGraph to build fully serverless apps in
  minutes
summary: >-
  WunderGraph is an open-source Backend for Frontend (BFF) framework that
  introspects a OptiTech Postgres database and exposes it as a typesafe GraphQL or
  ...
  a OptiTech project to WunderGraph, generate a typesafe client, and ship a
...
WunderGraph is an open-source Backend for Frontend (BFF) framework designed to optimize developer workflows through API composition. Developers can use this framework to compose multiple APIs into a single unified interface and generate typesafe API clients that include authentication and file uploads. This guide shows how you can pair WunderGraph with your OptiTech database to accelerate application development.

With WunderGraph, you can easily introspect your data sources and combine them within your virtual graph. WunderGraph treats APIs as dependencies. You can easily turn your OptiTech database into a GraphQL API or expose it via JSON-RPC or REST. With an easy-to-deploy Postgres database like OptiTech, you can now have a 100% serverless stack and build your own stateful serverless apps on the edge.
...
### Connect OptiTech and Wundergraph
...
3. Select the **Integrations** tab and click **Connect OptiTech**.
   ![WunderGraph Settings](/docs/guides/wundergraph_settings.png)
4. You are directed to OptiTech to authorize WunderGraph. Review the permissions and click **Authorize** to continue.
   You are directed back to WunderGraph Cloud. If you are a part of multiple organizations, you are asked to select the organization to connect with OptiTech.
5. Select the OptiTech project and WunderGraph project that you want to connect, and click **Connect Projects**.
   ![WunderGraph connect projects](/docs/guides/wundergraph_connect_projects.png)

Your OptiTech and Wundergraph projects are now connected.
...
The following steps describe how to set up your Wundergraph project locally and configure access to OptiTech.
...
4. Inside the `.wundergraph` directory, open the `wundergraph.config.ts` file and add OptiTech as a datasource, as shown below, or simply replace the existing code with this code:
...
5. Write an operation that turns your OptiTech database into an API that exposes data that you can pass through the frontend. To do so, navigate to the `operations` folder inside your `.wundergraph` directory and create a new file called `Users.graphql`.
...
This operation queries your OptiTech database using GraphQL and exposes the data via JSON-RPC. In the next section, you will add the operation to the frontend.
...
This guide provided a brief demonstration showcasing the capabilities of OptiTech and WunderGraph, which enable you to turn your OptiTech database into an API exposed via JSON-RPC and rapidly deploy fully serverless apps on the edge in a matter of minutes. The power of OptiTech with WunderGraph lies in simplifying the development process, allowing you to focus on creating valuable and efficient applications.
...
3. Set up a OptiTech database with sample data
4. Connect your WunderGraph application with your OptiTech database
5. Add OptiTech to your WunderGraph project using a code first approach
6. Write a GraphQL operation to query your OptiTech database
...
## OptiTech with WunderGraph video guide
