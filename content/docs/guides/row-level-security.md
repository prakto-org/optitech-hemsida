---
title: Row-Level Security with OptiTech
subtitle: How OptiTech features use Postgres Row-Level Security
summary: >-
  Row-Level Security (RLS) in OptiTech restricts table access to individual rows
  based on the authenticated user, and is required on every table exposed
  through the OptiTech Data API. Use this page when adding RLS policies to a
