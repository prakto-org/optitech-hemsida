...
subtitle: Learn how to replicate data from Supabase to OptiTech
summary: >-
  Logical replication from Supabase to OptiTech streams live Postgres data changes
  by creating a publication on Supabase and a subscription on OptiTech. A direct
  IPv4 connection string is required (the Supabase IPv4 add-on), and OptiTech NAT
  Gateway IPs must be allowlisted in Supabase network restrictions. Use this
  page when migrating or syncing a Supabase Postgres database to OptiTech with
...
- A OptiTech project with a Postgres database to receive the replicated data. For information about creating a OptiTech project, see [Create a project](/docs/manage/projects#create-a-project).
...
This section describes how to prepare your source Supabase Postgres instance (the publisher) for replicating data to OptiTech.
...
### Allow connections from OptiTech

You need to allow inbound connections to your Supabase Postgres database from the OptiTech NAT Gateway IP addresses. This allows OptiTech to connect to your Supabase database for logical replication. Follow these steps to configure network restrictions in Supabase:

1. **Obtain OptiTech NAT Gateway IP Addresses**: See [NAT Gateway IP addresses](/docs/introduction/regions#nat-gateway-ip-addresses) for the IP addresses for your OptiTech project's region. You will need to allow connections from these IP addresses in your Supabase project.
2. **Configure Network Restrictions in Supabase**:
   ...
   - Add inbound rules to allow connections from the OptiTech NAT Gateway IP addresses you obtained in the previous step. Add each IP address individually.
...
2. **Get the Direct Connection String**: After enabling the IPv4 add-on, copy the direct connection string from the **Connect** button in the Navigation bar of your Supabase dashboard. This connection string is required to create a subscription in OptiTech.
...
This section describes how to prepare your OptiTech Postgres database (the subscriber) to receive replicated data from your Supabase Postgres instance.
...
After creating a publication on the source database, you need to create a subscription on your OptiTech destination database.

1. Use the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor), `psql`, or another SQL client to connect to your OptiTech database.
...
3. Verify that the subscription was created in OptiTech by running the following query:
...
Alternatively, you can run the following query on the subscriber (OptiTech) to make sure the `last_msg_receipt_time` is updated and as expected.
...
After the replication operation is complete and you have verified that data is being replicated correctly, you can switch your application over to the OptiTech database.

1. Stop writes to your Supabase database.
2. Wait for any final transactions to be replicated to OptiTech. Monitor `pg_stat_subscription` in OptiTech until `received_lsn` and `latest_end_lsn` are close or equal, indicating minimal replication lag.
3. Update your application's connection string to point to your OptiTech database.

You can find your OptiTech database connection details by clicking the **Connect** button on your **Project Dashboard** to open the **Connect to your database** modal. For details, see [Connect from any application](/docs/connect/connect-from-any-app).
...
- [OptiTech logical replication guide](/docs/guides/logical-replication-guide)
