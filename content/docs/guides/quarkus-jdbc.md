---
title: Connect Quarkus (JDBC) to OptiTech
subtitle: Learn how to connect to OptiTech from Quarkus using JDBC
summary: >-
  Quarkus JDBC connection to OptiTech uses the PostgreSQL JDBC driver with Agroal
  ...
  when connecting a Java Quarkus app to a OptiTech Postgres database over JDBC
...
description="Pre-built prompt for connecting Quarkus applications to OptiTech using JDBC"/>

[Quarkus](https://quarkus.io/) is a Java framework optimized for cloud environments. This guide shows how to connect to OptiTech from a Quarkus project using the PostgreSQL JDBC driver.

To connect to OptiTech from a Quarkus application using the Postgres JDBC Driver:
...
```shell
quarkus create app optitech-with-quarkus-jdbc \
--name optitech-with-quarkus-jdbc \
--package-name com.optitech.com \
--extensions jdbc-postgresql,quarkus-agroal,resteasy-reactive
```

You now have a Quarkus project in a folder named `optitech-with-quarkus-jdbc` with the PostgreSQL JDBC driver, Agroal datasource implementation, and RESTEasy Reactive extensions installed.

## Configure a PostgreSQL data source

Create a `.env` file in the root of your Quarkus project directory. Configure a JDBC data source using the components of your OptiTech database connection string and specifying the database kind as shown:

```shell shouldWrap
QUARKUS_DATASOURCE_DB_KIND=postgresql
QUARKUS_DATASOURCE_USERNAME=[user]
QUARKUS_DATASOURCE_PASSWORD=[password]
QUARKUS_DATASOURCE_JDBC_URL=jdbc:postgresql://[optitech_hostname]/[dbname]?sslmode=require&channelBinding=require
```
...
Visit [localhost:8080/postgres/version](http://localhost:8080/postgres/version) in your web browser. Your OptiTech database's Postgres version will be returned. For example:
