---
title: Liquibase developer workflow with OptiTech
subtitle: Implement a developer workflow with Liquibase and OptiTech branching
summary: >-
  Iterate on XML changesets against an isolated OptiTech development branch, then
...
This guide shows how to set up a developer workflow using Liquibase with OptiTech's branching feature. The workflow involves making schema changes to a database on a development branch and applying those changes back to the source database on the production branch of your OptiTech project.
...
- An installation of Liquibase. For instructions, refer to [Get started with Liquibase and OptiTech](/docs/guides/liquibase).
...
Now, let's prepare a development database in OptiTech by creating a development branch, where you can safely make changes to your database schema without affecting the source database on your `production` branch. A branch is a copy-on-write clone of the data in your OptiTech project, so it will include a copy of the `blog` database with the `authors` and `posts` tables that you just created.

To create a branch:

1. In the OptiTech Console, select **Branches**. You will see your `production` branch, where you just created your `blog` database and tables.
...
You can verify these tables were created by viewing the `blog` database on your `feature/blog-schema` branch on the **Tables** page in the OptiTech Console. Select **Tables** from the sidebar.
</Admonition>
