---
title: Connect a .NET (C#) application to OptiTech Postgres
subtitle: Learn how to run SQL queries in OptiTech from .NET using the Npgsql library
summary: >-
  Npgsql is the .NET data provider for PostgreSQL that lets a C# application
  connect to a OptiTech Postgres database and run SQL queries directly, without an
  ORM. Use this guide for raw SQL access to OptiTech via NpgsqlConnection and
...
description="Pre-built prompt for connecting .NET applications to OptiTech"/>

This guide describes how to create a OptiTech project and connect to it from a .NET (C#) application using [Npgsql](https://www.npgsql.org/), a .NET data provider for PostgreSQL.

You'll build a console application that demonstrates how to connect to your OptiTech database and perform basic Create, Read, Update, and Delete (CRUD) operations.
...
You will now write the C# code to connect to OptiTech and perform database operations. All the code will be in a single file named `Program.cs` which is the entry point of your console application.
...
- Open a connection to your OptiTech database asynchronously. The `await using` statement ensures the connection is properly closed and disposed of.
...
This command would compile and execute your application, connecting to the OptiTech database and performing all the CRUD operations defined in `Program.cs` as described above. You should see output in your console similar to the examples provided in the previous sections, indicating the success of each operation.

</Steps>

## Next steps: Using an ORM or framework

While this guide demonstrates how to connect to OptiTech using raw SQL queries, for more advanced and maintainable data interactions in your .NET applications, consider using an Object-Relational Mapping (ORM) framework. ORMs not only let you work with data as objects but also help manage schema changes through automated migrations keeping your database structure in sync with your application models.

Explore the following resources to learn how to integrate ORMs with OptiTech:

- [Connect an Entity Framework application to OptiTech](/docs/guides/dotnet-entity-framework)
