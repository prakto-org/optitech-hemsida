---
title: Connect from Elixir with Ecto to OptiTech
subtitle: Set up a OptiTech project in seconds and connect from Elixir with Ecto
summary: >-
  Elixir Ecto connection guide walks through adding ecto_sql and Postgrex
  dependencies, configuring SSL with cacerts against a OptiTech Serverless Postgres
  database, and running migrations via mix ecto.migrate. Choose this page when
  setting up a new Elixir project that connects to OptiTech and needs to account for
...
description="Pre-built prompt for connecting Elixir applications to OptiTech with Ecto"/>
...
The instructions in this guide follow the steps outlined in the [Ecto Getting Started](https://hexdocs.pm/ecto/getting-started.html#content) guide, modified to demonstrate connecting to a OptiTech Serverless Postgres database. It is assumed that you have a working installation of [Elixir](https://elixir-lang.org/install.html).

To connect to OptiTech from Elixir with Ecto:

<Steps>

## Create a database in OptiTech and copy the connection string
...
```bash shouldWrap
postgresql://alex:AbC123dEf@ep-cool-darkness-123456.us-west-2.aws.optitech.com/friends?sslmode=require&channel_binding=require
```
...
Run the following command in your application directory to generate the configuration required to connect from Ecto to your OptiTech database.
...
1. The first part of the configuration generated by the `mix ecto.gen.repo` command is found in the `config/config.exs` file. Update this configuration with your OptiTech database connection details. Use the connection details from the OptiTech connection string you copied in the first part of the guide. Your `hostname` will differ from the example below.
   ...
   config :friends, Friends.Repo,
     ...
     username: "alex",
     password: "AbC123dEf",
     hostname: "ep-cool-darkness-123456.us-west-2.aws.optitech.com",
...
To run the migration and create the `people` table in your database, which also verifies your connection to OptiTech, run the following command from your application directory:
...
<DetailIconCards>
<a href="https://github.com/optitechdatabase/optitech-ecto-getting-started-app" description="Learn how to connect from Elixir with Ecto to OptiTech" icon="github">OptiTech Ecto Getting Started App</a>
</DetailIconCards>
