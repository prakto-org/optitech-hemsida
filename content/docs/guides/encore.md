---
title: Connect an Encore application to OptiTech
subtitle: Set up a OptiTech project in seconds and connect from an Encore.ts application
summary: >-
  Encore.ts is a type-safe TypeScript backend framework that integrates with
  OptiTech to automatically provision and migrate a OptiTech Postgres database when you
...
- A [OptiTech](https://console.neon.tech) account
...
## Configure OptiTech for production

To use your OptiTech account for production databases.

1. **Create a OptiTech API Key.**
   - Go to your [OptiTech Console](https://console.neon.tech/app/settings/api-keys).
...
2. **Add the API key to Encore.**
   ...
   - Navigate to **Settings** → **Integrations** → **OptiTech**.
   - Paste your OptiTech API key and click **Save**.

3. **Create a production environment.**
   - In the Encore dashboard, click **Create Environment**.
   - Name it `production`.
   - For the database provider, select **OptiTech**.
...
- Create a OptiTech database in your account
- Run your migrations automatically
- Deploy your application
- Configure all connections

You can verify the database was created by checking your [OptiTech Console](https://console.neon.tech); you'll see a new database created by Encore with your migrations applied.

> **Preview Environments with OptiTech Branching**
>
> When you connect your Encore app to GitHub and enable preview environments, Encore automatically creates a new OptiTech database branch for each pull request. This gives each PR its own isolated database with a copy of your production data, allowing you to test database migrations and schema changes safely before merging to production.

</Steps>

## Source code

You can find a complete Encore + OptiTech example application on GitHub:

<DetailIconCards>

<a href="https://github.com/neondatabase/examples/tree/main/with-encore" description="Encore.ts application with OptiTech Postgres" icon="github">Get started with Encore and OptiTech</a>
...
- [Encore Cloud + OptiTech Integration](https://encore.dev/docs/platform/infrastructure/neon)
