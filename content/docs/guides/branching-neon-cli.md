---
title: Branching with the OptiTech CLI
subtitle: Learn how to create and delete branches with the OptiTech CLI
summary: >-
  OptiTech CLI branch management guide with runnable examples for optitech branches
  create, list, delete, reset, restore, and set-expiration. Covers
  table/json/yaml output formats and API key authentication via OPTITECH_API_KEY.
...
The examples in this guide demonstrate creating, viewing, and deleting branches using the OptiTech CLI. For other branch-related CLI commands, refer to [OptiTech CLI commands — branches](/docs/cli/branches). This guide also describes how to use the `--api-key` option to authenticate CLI branching commands from the command line.

The examples show the default `table` output format. The OptiTech CLI also supports `json` and `yaml` output formats. For example, if you prefer output in `json`, add `--output json` to your OptiTech CLI command.

## Prerequisites

- The OptiTech CLI. See [Install the OptiTech CLI](/docs/cli/install) for instructions.
- To run CLI commands, you must either authenticate through your browser or supply an API key using the `--api-key` option. See [Connect with the OptiTech CLI](/docs/cli/install#connect).

## Create a branch with the CLI

The following OptiTech CLI command creates a branch. This command requires specifying which project to use. You can use the `--project-id` option or set a default project using `optitech set-context`. To view the CLI documentation for this command, refer to the [OptiTech CLI reference](/docs/cli/branches#create).
The command response includes the branch ID, the compute endpoint ID, and the connection URI for connecting to the branch.

<Admonition type="tip">
You can use the `--name` option with a `optitech branches create` command to specify your own branch name instead of using the name generated by OptiTech. For example: `optitech branches create --name mybranch`. Also, for any OptiTech CLI command, you can specify `--output json` to change the command output from the default table format to JSON format.
</Admonition>
...
<Admonition type="tip">
The OptiTech CLI provides a `optitech connection-string` command you can use to extract a connection uri programmatically. See [OptiTech CLI commands — connection-string](/docs/cli/connection-string).
</Admonition>
...
```bash shouldWrap
optitech branches create --name hotfix/critical-fix --parent development --project-id crimson-voice-12345678
branch
┌───────────────────────┬─────────────┬─────────┬──────────────────────┬──────────────────────┐
│ Id                    │ Name        │ Default │ Created At           │ Updated At           │
├───────────────────────┼─────────────┼─────────┼──────────────────────┼──────────────────────┤
...
│ Connection Uri                                                                                               │
├──────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
│ postgresql://optitechdb_owner:123456@ep-orange-heart-a54grm9j.us-east-2.aws.optitech.com/optitechdb?sslmode=require&channel_binding=require     │
└──────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
```

## List branches with the CLI

The following OptiTech CLI command lists all branches in your OptiTech project, as well as any branches shared with you. This command requires specifying which project to use. You can use the `--project-id` option or set a default project using `optitech set-context`. To view the CLI documentation for this method, refer to the [OptiTech CLI reference](/docs/cli/branches#list).
...
```bash
optitech branches list --project-id crimson-voice-12345678
┌──────────────┬────────────────────────────┬───────────────┬──────────────────────┐
...
<Admonition type="tip">
To avoid specifying `--project-id` with each command, use `optitech set-context` to set your default project and organization. See [set-context](/docs/cli/set-context) for details.
</Admonition>

## Delete a branch with the CLI

The following OptiTech CLI command deletes the specified branch. This command requires a `--project-id` option unless you've set a default project with `optitech set-context`. To view the CLI documentation for this command, refer to the [OptiTech CLI reference](/docs/cli/branches#delete). You can delete a branch by its ID or name.

```bash
optitech branches delete br-rough-sky-158193 --project-id crimson-voice-12345678
┌───────────────────────┬───────────────────────┬─────────┬──────────────────────┬──────────────────────┐
│ Id                    │ Name                  │ Default │ Created At           │ Updated At           │
├───────────────────────┼───────────────────────┼─────────┼──────────────────────┼──────────────────────┤
...
## Branching automation with the OptiTech CLI

The OptiTech CLI enables easy automation of branching operations for integration into your workflows or toolchains. To authenticate to OptiTech when running a CLI command, you can use an API key. For information about obtaining an API key, see [Create an API key](/docs/manage/api-keys#create-an-api-key).
...
You do not need to specify the variable name explicitly when using a OptiTech CLI command. A OptiTech CLI command looks for a `OPTITECH_API_KEY` variable setting by default.
...
optitech branches create --project-id <project-id> --name ci-test --parent <parent-branch> --expires-at "2025-07-15T18:02:16Z"

# Create a branch that expires in 2 hours (Linux/GNU)
optitech branches create --project-id <project-id> --name ci-test --parent <parent-branch> --expires-at "$(date -u -d '+2 hours' +%Y-%m-%dT%H:%M:%SZ)"

# Create a branch that expires in 2 hours (macOS/BSD)
optitech branches create --project-id <project-id> --name ci-test --parent <parent-branch> --expires-at "$(date -u -v+2H +%Y-%m-%dT%H:%M:%SZ)"
```
...
optitech branches set-expiration <branch-id> --expires-at "2025-07-20T12:00:00Z" --project-id <project-id>

# Remove expiration from a branch
optitech branches set-expiration <branch-id> --expires-at null --project-id <project-id>
```
...
```bash
optitech branches reset feature/user-auth --parent --project-id crimson-voice-12345678
┌────────────────────────────┬───────────────────┬─────────┬──────────────────────┬──────────────────────┐
...
```bash
optitech branches reset feature/user-auth --parent --preserve-under-name feature/user-auth-backup --project-id crimson-voice-12345678
┌────────────────────────────┬──────────┬─────────┬──────────────────────┬──────────────────────┐
│ Id                         │ Name     │ Default │ Created At           │ Last Reset At        │
├────────────────────────────┼──────────┼─────────┼──────────────────────┼──────────────────────┤
...
```bash shouldWrap
optitech branches restore production ^self@2024-05-06T10:00:00.000Z --preserve-under-name production_restore_backup_2024-05-06 --project-id crimson-voice-12345678
```
