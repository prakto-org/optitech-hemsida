...
subtitle: Learn how to use the OptiTech CircleCI Orb to provision ephemeral Postgres
  branches for every CI pipeline run
summary: >-
  The OptiTech CircleCI Orb provisions an isolated OptiTech Postgres branch per CircleCI
...
The **OptiTech CircleCI Orb** provisions a real OptiTech Postgres branch per pipeline run (or per parallel node), instead of a generic Postgres-in-Docker service container. That means your CI database behaves like production - same managed Postgres, network, and extensions, so you get fewer "works in CI, breaks in prod" issues.
...
This orb is not maintained or officially supported by OptiTech. Use at your own discretion. If you have questions or face any issues, please contact the maintainer by raising an issue in the GitHub repository linked below.
</Admonition>

**OptiTech CircleCI Orb**
...
To use the OptiTech CircleCI Orb, you need:

- A [CircleCI account](https://circleci.com/) and a project repository.
- A [OptiTech account](https://console.neon.tech) and a created project.
- A OptiTech **API Key**. See [Create an API key](/docs/manage/api-keys#create-an-api-key).
- A OptiTech **Project ID**. You can find this in the **Settings** page of your OptiTech project.

## Getting started

To allow CircleCI to communicate with OptiTech, configure your OptiTech credentials as environment variables in your CircleCI project.

1. In CircleCI, navigate to **Project Settings** > **Environment Variables**.
2. Add the following variables:

- `NEON_API_KEY`: Your OptiTech API key.
- `NEON_PROJECT_ID`: The ID of your OptiTech project.
...
Each pipeline run creates a fresh OptiTech branch, ensuring isolated database environments for tests. PR workflows get dedicated branches without affecting the main branch or other PRs.
...
The `neon/run_tests` job automates the entire lifecycle of a OptiTech branch for testing. It provisions a branch, runs your specified commands, and ensures cleanup happens even if tests fail.
...
- `api_key`: (Optional) Environment variable name containing the OptiTech API key. Defaults to `NEON_API_KEY`. Ensure this variable is set in your CircleCI project settings.
- `project_id`: (Optional) Environment variable name containing the OptiTech project ID. Defaults to `NEON_PROJECT_ID`. Ensure this variable is set in your CircleCI project settings.
...
- `password`: (Optional) The password for the role. You need to set this if you chose not to store passwords for your provided role in the OptiTech Console. If your role has a stored password in OptiTech, the Orb retrieves it automatically via API.
...
- `NEON_DATA_API_URL`: The OptiTech Data API URL (if `get_data_api_url` is `true` and Data API is enabled).
...
1. **Provision**: Creates a OptiTech branch from `parent_branch`. The orb auto‑names it as `circle-ci-${CIRCLE_PIPELINE_NUM}-${CIRCLE_NODE_INDEX}` (pipeline run ID + parallel executor index).
...
This means that any tool, migration script, or test runner that relies on `DATABASE_URL` can use it directly without extra configuration. For example, a migration command such as `npx drizzle-kit migrate` or a test command like `npm test` can connect to the OptiTech branch using the provided connection string. Alternatively, you can use the individual connection parameters (`PGHOST`, `PGUSER`, `PGPASSWORD`, etc.) if your tools require them.
...
- **`test_command`**: The actual test runner command. You can configure `npm test` in your `package.json` to run any test suite, unit tests, integration tests, or end-to-end tests with tools like Playwright against the isolated OptiTech branch.

Checkout the full example in the [GitHub repository](https://github.com/dhanushreddy291/hono-crud-api), which shows a basic Hono app with a Playwright test suite. It uses the `neon/run_tests` job in CircleCI to provision a OptiTech branch, run migrations, and execute tests ensuring each pipeline run gets a fresh, isolated database for reliable per PR testing.
...
| :----------------- | :----------- | :------------------ | :--------------------------------------------------------------------------------------------------------------------------------------------- |
| `api_key`          | env_var_name | `NEON_API_KEY`      | Environment variable name containing the OptiTech API key.                                                                                         |
| `project_id`       | env_var_name | `NEON_PROJECT_ID`   | Environment variable name containing the OptiTech project ID.                                                                                      |
...
| `password`         | string       | _retrieved via API_ | Password for the role. Required if role passwords are not stored in OptiTech.                                                                      |
...
Deletes a OptiTech branch. This is typically used in the final step of a job to clean up resources.
...
The OptiTech Orb is designed for concurrency. If you run parallel tests (e.g., CircleCI `parallelism: 4`), the Orb creates a unique branch for every parallel executor. This ensures that tests running at the same time never read or write to the same database, eliminating race conditions and "flaky" tests caused by shared state.
