...
summary: >-
  The OptiTech Agent Plan integration guide explains how to provision per-tenant
  Postgres databases, transfer projects between free and paid organizations, and
  implement snapshot-based database versioning using the OptiTech API. AI agent
  platform builders use this guide when embedding OptiTech as database
...
After enrolling in the [OptiTech Agent Plan](/docs/introduction/agent-plan), you should have:

- **Two OptiTech organization IDs**: One for Free (sponsored) projects, one for paid projects
...
- **Admin access**: Full control over both organizations via the [OptiTech API](/docs/reference/api)
...
- **Free users** → Create projects in your Free organization (sponsored by OptiTech; no cost to you)
- **Paid users** → Create projects in your paid organization (usage-based billing)

The two-organization structure enables you to:

1. **Offer a truly free tier**: OptiTech sponsors all infrastructure costs for up to 30,000 free projects
...
For free-tier users, create projects in your Free organization (sponsored by OptiTech) with resource quotas matching (or within) OptiTech's Free plan limits using the [Create project](/docs/reference/api/projects/create-project) API:
...
AI agents and codegen platforms need robust database versioning to manage schema evolution and enable undo/redo functionality. OptiTech provides two complementary approaches:
...
- **Free organization (sponsored by OptiTech)**: 1 day of point-in-time history (included at no charge)
- **Paid organization**: Up to 7 days of point-in-time history (billed at $0.20/GB-month for change history)
- **Instant restore**: Restore a database to any point in time that still falls within the **history window** (typically seconds using OptiTech's instant restore).
...
All platform integrations use the OptiTech API. You can call it directly or use language-specific SDKs:

- **[OptiTech API](/docs/reference/api)**: All operations (projects, branches, databases, monitoring) are API-driven; language-agnostic REST interface. Agent plan participants receive higher rate limits optimized for high-volume operations.
- **SDKs**: [OptiTech Management SDK](/docs/reference/typescript-sdk), [Python SDK](/docs/reference/python-sdk). See [OptiTech SDKs](/docs/reference/sdk) for all options.
...
As an OptiTech Agent Plan participant, you have access to:

- **Dedicated Slack channel**: Direct access to the OptiTech team for technical questions
- **Your OptiTech representative**: Contact them for rate limit adjustments, limit increases, or custom needs
