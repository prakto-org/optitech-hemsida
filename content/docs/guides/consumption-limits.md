...
subtitle: Learn how to set consumption limits per project with the OptiTech API
summary: >-
  Per-project consumption quotas in OptiTech let you cap active_time_seconds,
...
By default, OptiTech tracks a variety of consumption metrics at the project level. If you want to set quotas (max limits) for these metrics, you need to explicitly [configure](#configuring-quotas) them.
...
OptiTech updates all metrics every 15 minutes but it could take up to 1 hour before they are reportable.
...
- `data_transfer_bytes` &#8212; Sets the maximum amount of egress data, measured in bytes, that can be transferred out of OptiTech from across all of a project's branches using the proxy.
...
### OptiTech default limits

In addition to the configurable limits that you can set, OptiTech also sets certain branch size limits by default. You might notice these limits in a [Get Project](#retrieving-details-about-a-project) response:
...
If you need to change the quota limits for an existing project &#8212; for example, if a user switches their plan to a higher usage tier &#8212; you can reset those limits via `PATCH` request. See [Update a project](/docs/reference/api/projects/update-project) in the OptiTech API.
