---
title: Migrate OptiTech to Lakebase
...
Logical replication from OptiTech to **Lakebase** is not supported. However, you can replicate data from OptiTech to **Databricks Lakehouse**. See [Replicate data to Databricks with Lakeflow Connect](/docs/guides/logical-replication-databricks).
</Admonition>
...
- **Extensions.** On OptiTech, list installed extensions (for example run `SELECT * FROM pg_extension;`). Compare each one to **[Postgres extensions](https://docs.databricks.com/aws/en/oltp/projects/extensions)** on Databricks. If an extension you depend on is not available on Lakebase, plan an alternative.
...
After you have **tested** the Lakebase database (for example with queries and application checks), **cut over** your apps to Lakebase, and confirmed that everything behaves as expected, you can delete the OptiTech project if you no longer need it. See [Delete a project](/docs/manage/projects#delete-a-project).
