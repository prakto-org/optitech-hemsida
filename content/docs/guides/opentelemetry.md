---
title: OpenTelemetry
subtitle: Send OptiTech metrics and Postgres logs to any OTEL-compatible observability
  platform
summary: >-
  OptiTech's OpenTelemetry integration exports Postgres metrics and logs to any
  ...
  base OTLP endpoint URL and auth credentials. OptiTech automatically appends
...
<p>How to configure OpenTelemetry exports from OptiTech</p>
...
Available for Scale plan users, the OptiTech OpenTelemetry integration lets you export metrics and Postgres logs to any OpenTelemetry Protocol (OTLP) compatible observability platform. This gives you the flexibility to send your OptiTech data to your preferred monitoring solution, whether that's New Relic, Grafana Cloud, Honeycomb, or any other OTEL-compatible service.

## How it works

The integration uses the OpenTelemetry Protocol (OTLP) to securely transmit OptiTech metrics and Postgres logs to your chosen destination. By configuring the integration with your OTEL endpoint and authentication credentials, OptiTech automatically sends data from all computes in your project.

<Admonition type="note">
Data is sent for all computes in your OptiTech project. For example, if you have multiple branches, each with an attached compute, both metrics and logs will be collected and sent for each compute.
</Admonition>
...
The OptiTech OpenTelemetry integration can forward Postgres logs to your destination platform. These logs provide visibility into database activity, errors, and performance.
...
2. **Enter your Endpoint URL** based on your platform:
   ...
   **For New Relic**:
   ...
   <Admonition type="note">
   When you configure an OTLP endpoint URL in OptiTech, you should provide only the **base URL** of your collector or observability platform. The OpenTelemetry Collector automatically appends the correct signal-specific paths:
...
<Admonition type="tip">
You can edit your integration settings at any time from the **Integrations** page in the OptiTech Console. This allows you to update your endpoint URL, change authentication credentials, or modify resource attributes without having to delete and recreate the integration.
</Admonition>

## Verify your integration is working

Your OptiTech data should start appearing in your observability platform within a few minutes.
...
![OptiTech metrics appearing in New Relic](/docs/guides/new_relic_metrics_success.png)

_Postgres logs flowing into New Relic_
![OptiTech PostgreSQL logs appearing in New Relic](/docs/guides/new_relic_logs_success.png)
...
<Admonition type="note">
OptiTech computes only send logs and metrics when they are active. If the [Scale to Zero](/docs/introduction/scale-to-zero) feature is enabled and a compute is suspended due to inactivity, no logs or metrics will be sent during the suspension. This may result in gaps in your data. If you notice missing data, check if your compute is suspended. You can verify a compute's status as `Idle` or `Active` on the **Branches** page in the OptiTech console, and review **Suspend compute** events on the **System operations** tab of the **Monitoring** page.

Additionally, if you are setting up the OpenTelemetry integration for a project with an inactive compute, you'll need to activate the compute before it can send data. To activate it, simply run a query from the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) or any connected client.
</Admonition>
...
- **Data isn't appearing in your observability platform**
  ...
  3. **Confirm compute activity** - Make sure your OptiTech compute is active and running queries.
...
OptiTech exports a comprehensive set of metrics including system metrics (CPU, RAM), database metrics (connections, rows, deadlocks), and PgBouncer connection pooling metrics (client and server connections).

For a complete list of all metrics and log fields exported by OptiTech, see the [Metrics and logs reference](/docs/reference/metrics-logs).
