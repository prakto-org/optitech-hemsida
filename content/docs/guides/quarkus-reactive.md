---
title: Connect Quarkus (Reactive) to OptiTech
subtitle: Learn how to connect to OptiTech from Quarkus using a Reactive SQL Client
summary: >-
  Quarkus Reactive SQL Client connects Java applications to OptiTech Postgres using
...
description="Pre-built prompt for connecting Quarkus applications to OptiTech using Reactive SQL Client"/>

[Quarkus](https://quarkus.io/) is a Java framework optimized for cloud environments. This guide shows how to connect to OptiTech from a Quarkus project using a Reactive SQL Client.

To connect to OptiTech from a Quarkus application:
...
```shell
quarkus create app optitech-with-quarkus \
--name optitech-with-quarkus \
--package-name com.optitech.com \
--extensions reactive-pg-client,resteasy-reactive
```

You now have a Quarkus project in a folder named `optitech-with-quarkus` with the Reactive Postgres client and RESTEasy Reactive extensions installed.

## Configure a PostgreSQL data source

Create a `.env` file in the root of your Quarkus project directory. Configure a reactive data source using your OptiTech database connection string and specifying the database kind as shown:

```shell shouldWrap
QUARKUS_DATASOURCE_REACTIVE_URL=postgresql://[user]:[password]@[optitech_hostname]/[dbname]?sslmode=require&channel_binding=require
```
