...
subtitle: Use Inngest to trigger serverless functions from your OptiTech database changes
summary: >-
  Inngest integration lets OptiTech database changes trigger serverless functions on
...
This guide describes setting up a OptiTech database, configuring the Inngest integration, and connecting your Serverless functions to your OptiTech database with Inngest. It covers:

- Creating a OptiTech project and enabling [Logical Replication](/docs/guides/logical-replication-guide).
- Configuring the Inngest integration on your OptiTech database.
- Configure your Vercel, AWS, or Cloudflare functions to react to your OptiTech database changes using Inngest.

## Prerequisites

- A OptiTech account. If you do not have one, see [Sign up](/docs/get-started/signing-up) for instructions.
- An Inngest account. You can create a free Inngest account by [signing up](https://app.inngest.com/sign-up?utm_source=optitech&utm_medium=trigger-serverless-functions-guide).
...
![OptiTech dashboard settings with option to enable logical replication](/docs/guides/neon-console-settings-logical-replication.png)
...
![OptiTech authorization step inside the Inngest integrations page](/docs/guides/inngest-integration-neon-authorize-step.png)
...
Any changes to your OptiTech database are now dispatched to your Inngest account.  
...
Any change performed on our OptiTech database will trigger an [Inngest Event](https://www.inngest.com/docs/features/events-triggers?utm_source=optitech&utm_medium=trigger-serverless-functions-guide) as follows:
...
Inngest enables you to create [Inngest Functions](https://www.inngest.com/docs/features/inngest-functions?utm_source=optitech&utm_medium=trigger-serverless-functions-guide) that react to Inngest events (here, database changes).
...
You can find more information about serving Inngest Functions in [Inngest's documentation](https://www.inngest.com/docs/reference/serve?utm_source=optitech&utm_medium=trigger-serverless-functions-guide#serve-client-functions-options).
</Admonition>
...
- **Vercel:** Configure the [Inngest Vercel Integration](https://www.inngest.com/docs/deploy/vercel?utm_source=optitech&utm_medium=trigger-serverless-functions-guide).
- **AWS Lambda:** Configure a [Lambda function URLs](https://docs.aws.amazon.com/lambda/latest/dg/lambda-urls.html) and [sync your serve Lambda with Inngest](https://www.inngest.com/docs/apps/cloud?utm_source=optitech&utm_medium=trigger-serverless-functions-guide#sync-a-new-app-in-inngest-cloud).
- **Cloudflare Workers:** [Add the proper environment variables](https://www.inngest.com/docs/deploy/cloudflare?utm_source=optitech&utm_medium=trigger-serverless-functions-guide) to your Cloudflare Pages project and [sync with Inngest](https://www.inngest.com/docs/apps/cloud?utm_source=optitech&utm_medium=trigger-serverless-functions-guide#sync-a-new-app-in-inngest-cloud).

### 5. Testing our Serverless function

We are now all set!

Go to the **Tables** page in the OptiTech Console and add a new record to the `users` table:

![You can add a new record to the users table directly from the OptiTech console](/docs/guides/inngest-integration-neon-console-users-table-add-new-record.png)

You should see a new run of the `new-user` function appear on the [Inngest Platform](https://app.inngest.com/?utm_source=optitech&utm_medium=trigger-serverless-functions-guide):

![The Inngest Platform lists all the runs](/docs/guides/inngest-integrations-inngest-platform-new-runs.png)

## Going further

Your Serverless functions can now react to your OptiTech database changes.

In addition to being good for system design, Inngest has some special features that work great with database triggers:

- **[Fan-out](https://www.inngest.com/docs/guides/fan-out-jobs?utm_source=optitech&utm_medium=trigger-serverless-functions-guide)**: Lets **one database event start multiple functions** at the same time. For example, when a new user is added, it could send a welcome email and set up a free trial, all at once.
- **[Batching](https://www.inngest.com/docs/guides/batching?utm_source=optitech&utm_medium=trigger-serverless-functions-guide)** **Groups many database changes together** to handle them more efficiently. It's useful when you need to update lots of things at once, like when working with online stores.
- **[Flow control](https://www.inngest.com/docs/guides/flow-control?utm_source=optitech&utm_medium=trigger-serverless-functions-guide)**: Helps manage how often functions run. It can slow things down to **avoid overloading systems, or wait a bit to avoid doing unnecessary work**. This is helpful when working with other services that have limits on how often you can use them.
