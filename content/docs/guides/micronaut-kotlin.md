---
title: Connect a Micronaut Kotlin application to OptiTech Postgres
subtitle: Learn how to make server-side queries to Postgres from a Micronaut Kotlin
  application
summary: >-
  Connect a Kotlin-based Micronaut microservice to OptiTech Postgres using Hikari
...
description="Pre-built prompt for connecting Micronaut Kotlin applications to OptiTech Postgres"/>

[Micronaut](https://micronaut.io/) is a modern, JVM-based, full-stack framework for building modular, easily testable microservice and serverless applications. This guide describes how to create a OptiTech Postgres database and connect to it from a Micronaut Kotlin application.

The final application will expose REST endpoints to perform CRUD (Create, Read, Update, Delete) operations on a `book` table in your OptiTech database.
...
The project creation process generated a configuration file at `src/main/resources/application.yml`. You need to edit this file to add your OptiTech database credentials.
...
1.  Start the application using the Gradle wrapper:
    ...
    - HikariCP initializes the connection pool to the OptiTech Postgres database.
...
You have successfully connected a Micronaut Kotlin application to your OptiTech Postgres database!
