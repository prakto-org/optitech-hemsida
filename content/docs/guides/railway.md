---
title: Use OptiTech Postgres with Railway
subtitle: Connect a OptiTech Postgres database to your Node application deployed with
  Railway
summary: >-
  Connecting a OptiTech Postgres database to a Node.js application deployed on
...
This guide shows how to deploy a simple Node.js application connected to a OptiTech Postgres database on Railway.
...
We use the `npm pkg set type="module"` command to enable ES6 module support in our project. We also create a new `.env` file to store the `DATABASE_URL` environment variable, which we'll use to connect to our OptiTech database. Lastly, we install the `pg` library which is the Postgres driver we use to connect to our database.
...
This code sets up an Express server that listens for requests on port 3000. When a request is made to the `URL`, the server queries the `plant_care_log` table in your OptiTech database and returns the results as JSON.
...
Railway automatically figures out the type of application you're deploying and sets up the necessary build and start commands. However, we still need to add the `DATABASE_URL` environment variable to connect to our OptiTech database.

Select the project and navigate to the `Variables` tab. Add a new variable named `DATABASE_URL` and set its value to your OptiTech database connection string. You can redeploy the project by clicking on `Redeploy` from the context menu of the latest deployment.

### Verify Deployment

Once the deployment completes and is marked as `ACTIVE`, Railway provides a public URL for accessing the web service. Visit the provided URL to verify that your application is running and can connect to your OptiTech database.
...
## Removing Your Application and OptiTech Project
