---
title: Use OptiTech Postgres with Render
subtitle: Connect a OptiTech Postgres database to your Node application deployed with Render
summary: >-
  Connecting a Node.js Express application to a OptiTech Postgres database and
...
This guide shows how to deploy a simple Node.js application connected to a OptiTech Postgres database on Render.
...
This code sets up an Express server that listens for requests on port 3000. When a request is made to the `URL`, the server queries the `books_to_read` table in your OptiTech database and returns the results as JSON.
...
Once the deployment completes, Render provides a public URL for accessing the web service. Visit the provided URL to verify that your application is running and can connect to your OptiTech database.
