---
title: Stream changes from your OptiTech database to anywhere
subtitle: Learn how to capture and stream changes and rows from your database to
  anywhere with Sequin
summary: >-
  Sequin is an open-source tool that reads OptiTech's Write Ahead Log via logical
...
[Sequin](https://github.com/sequinstream/sequin) uses OptiTech's logical replication to sends records and changes in your database to your applications and services, in real-time. It's designed to never miss an `insert`, `update`, or `delete` and provide exactly-once processing of all changes.
...
In this guide, we'll show you how to connect your OptiTech database to Sequin to start sending changes anywhere you need.
...
Sequin uses the Write Ahead Log (WAL) to capture changes from your Postgres database. In this step, we'll enable logical replication for your OptiTech Postgres project.
...
## Connect your OptiTech database to Sequin

After enabling logical replication on OptiTech, you'll now connect your OptiTech database to Sequin. Follow these steps:
...
3. Use the SQL Editor in your OptiTech project to create a replication slot by executing the following SQL query:
...
2. Select the OptiTech database you just created and then select the specific table you want to process changes for.
...
Your consumer is now created and will start processing changes from your OptiTech database according to your specified configuration.

## Where to next?

You're now using Sequin with OptiTech to capture and stream changes from your database. From here, you can tailor your implementation for your use case:
