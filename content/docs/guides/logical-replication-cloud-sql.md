...
subtitle: Learn how to replicate data from Google Cloud SQL Postgres to OptiTech
summary: >-
  Replicate data from Google Cloud SQL Postgres to OptiTech using logical
  replication. Setup involves enabling the `cloudsql.logical_decoding` flag,
  allowlisting OptiTech's NAT gateway IPs, and creating a publication on the
  source with a matching `CREATE SUBSCRIPTION` on the OptiTech destination. Use
  this guide when migrating or continuously syncing from Cloud SQL to OptiTech
...
This section describes how to prepare your source Cloud SQL Postgres instance (the publisher) for replicating data to OptiTech.
...
You need to allow connections to your Cloud SQL Postgres instance from OptiTech. To do this in Google Cloud:
...
1. In the **Network** field, enter the IP address from which you want to allow connections. You will need to perform this step for each of the NAT gateway IP addresses associated with your OptiTech project's region. OptiTech uses 3 to 6 IP addresses per region for this outbound communication, corresponding to each availability zone in the region. See [NAT Gateway IP addresses](/docs/introduction/regions#nat-gateway-ip-addresses) for OptiTech's NAT gateway IP addresses.
   ...
   In the example shown below, you can see that three addresses were added, named `OptiTech1`, `OptiTech2`, and `OptiTech3`. You can name them whatever you like. The addresses were added in CIDR format by adding `/32`.

   ![Cloud SQL network configuration](/docs/guides/cloud_sql_network_config.png)

1. Click **Done** after adding a Network entry.
1. Click **Save** when you are finished adding Network entries for all of your OptiTech project's NAT Gateway IP addresses.
...
Record the public IP address of your Cloud SQL Postgres instance. You'll need this value later when you set up a subscription from your OptiTech database. You can find the public IP address on your Cloud SQL instance's **Overview** page.
...
This section describes how to prepare your source OptiTech Postgres database (the subscriber) to receive replicated data from your Cloud SQL Postgres instance.
...
After the replication operation is complete, you can switch your application over to the destination database by swapping out your Cloud SQL source database connection details for your OptiTech destination database connection details.
