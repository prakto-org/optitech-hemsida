---
title: Connect a RedwoodSDK application to OptiTech
subtitle: Set up a OptiTech project in seconds and connect from a Redwood application
summary: >-
  RedwoodSDK is a full-stack framework for Cloudflare Workers, and connecting it
  to OptiTech Postgres requires choosing between two drivers: postgres.js or the
  OptiTech serverless driver (@optitech/serverless). This guide walks through
  creating a OptiTech project, installing the chosen driver, storing the
  DATABASE_URL connection string in .env, and configuring route handlers in
  TypeScript to query Postgres on OptiTech. Use this page when building a RedwoodSDK
...
description="Pre-built prompt for connecting RedwoodSDK applications to OptiTech Postgres"/>

[RedwoodSDK](https://rwsdk.com/) is a framework for building full-stack applications on Cloudflare. This guide describes how to create a OptiTech project and access it from a RedwoodSDK application.

To create a OptiTech project and access it from a RedwoodSDK application:
...
3.  Add project dependencies depending on the PostgreSQL driver you wish to use (`postgres.js` or `@optitech/serverless`):

    <CodeTabs reverse={true} labels={["postgres.js", "OptiTech serverless driver"]}>
...
async function getData() {
  const sql = optitech(env.DATABASE_URL);
  const response = await sql`SELECT version()`;
  return response[0].version;
}
...
Navigate to ([localhost:5173](http://localhost:5173)) in your browser. You should see a response similar to the following, indicating a successful connection to your OptiTech database:

```text
PostgreSQL 17.5 (6bc9ef8) on aarch64-unknown-linux-gnu, compiled by gcc (Debian 12.2.0-14+deb12u1) 12.2.0, 64-bit
```

> The specific version may vary depending on the PostgreSQL version of your OptiTech project.
