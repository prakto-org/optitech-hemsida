...
summary: >-
  OptiTech's Backup & Restore feature combines instant point-in-time restore (PITR)
...
Use the **Backup & restore** page in the OptiTech Console to instantly restore a branch to a previous state or create and restore snapshots of your data. This feature combines **instant point-in-time restore** and **snapshots** to help you recover from accidental changes, data loss, or schema issues.

The **Enhanced view** toggle in the OptiTech Console lets you access the Backup & Restore page with snapshot capabilities. When enabled, you can create and manage snapshots alongside instant point-in-time restore. Toggle it off to return to the original Restore page if needed.
...
Depending on billing mode and whether sizes have finished calculating, either field may be omitted. For parameter-level definitions, see each endpoint in the [OptiTech API Reference](/docs/reference/api).
...
You can view and set backup schedules for branches using the OptiTech API. For complete API documentation, refer to the [OptiTech API Reference](/docs/reference/api).
...
3. Your branch is immediately restored to the snapshot state, and the `<branch_name>_old` branch is created, which you'll find on the **Branches** page in the OptiTech Console, as shown here:
...
2.  **Inspect the new branch**
    ...
    After restoring the snapshot, you can connect to the new branch and run queries to inspect the data. You can get the branch connection string from the OptiTech Console or using the [Retrieve connection URI](/docs/reference/api/projects/get-connection-uri) endpoint.
...
3.  **Finalize the restore**
    ...
    Parameters:
    - `project_id`: The OptiTech project ID.
