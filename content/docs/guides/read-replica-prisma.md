---
title: Use OptiTech read replicas with Prisma
subtitle: Learn how to scale Prisma applications with OptiTech read replicas
summary: >-
  OptiTech read replicas for Prisma route read queries to independent read-only
...
For more information about OptiTech's read replica feature, see [Read replicas](/docs/introduction/read-replicas).

This guide shows how to use OptiTech read replicas to scale Prisma applications using Prisma Client's read replica extension: [@prisma/extension-read-replicas](https://github.com/prisma/extension-read-replicas).

## Prerequisites

- An application that uses Prisma with a OptiTech database. If you haven't set up Prisma yet, see [Connect from Prisma to OptiTech](/docs/guides/prisma).

## Create a read replica

You can create read replicas for any branch in your OptiTech project.
...
Connecting to a read replica is the same as connecting to any branch in a OptiTech project, except you connect via a read replica compute instead of your primary read-write compute. The following steps describe how to retrieve the connection string (the URL) for a read replica from the OptiTech Console.
...
[@prisma/extension-read-replicas](https://github.com/prisma/extension-read-replicas) adds support to Prisma Client for read replicas. The following steps show you how to install the extension and configure it to use a OptiTech read replica.
...
2. Extend your Prisma Client instance by importing the extension and creating separate adapters for your primary and replica connections:
   ...
   <Admonition type="note">
   You can pass multiple replica clients if you want to use multiple read replicas. OptiTech supports adding multiple read replicas to a database branch. A replica is selected randomly for each read query.
