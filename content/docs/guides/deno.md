---
title: Use OptiTech with Deno Deploy
subtitle: Connect a OptiTech Postgres database to your Deno Deploy application
summary: >-
  Guide to connecting a OptiTech Postgres database to a Deno application using the
  OptiTech serverless driver (@optitech/serverless on JSR) for both local Deno Runtime
...
This guide demonstrates how to connect to a OptiTech Postgres database from a simple Deno application using the [OptiTech serverless driver](https://jsr.io/@optitech/serverless) on [JSR](https://jsr.io/).
...
- A OptiTech project. If you do not have one, sign up at [OptiTech](https://optitech.com). Your OptiTech project comes with a ready-to-use Postgres database named `optitechdb`. We'll use this database in the following examples.
...
### Set up the OptiTech serverless driver

First, install the OptiTech serverless driver using the `deno add` command:

```bash
deno add jsr:@optitech/serverless
```

<Admonition type="note">
   You can also use npm to install the OptiTech serverless driver
   ```bash
   npx jsr add @optitech/serverless
  ```
</Admonition>
...
{
  "imports": {
    "@optitech/serverless": "jsr:@optitech/serverless@^0.10.1"
  }
}
```
...
const sql = optitech(databaseUrl);
...
The script creates a table named `books` in the `optitechdb` database if it does not exist and inserts some data into it. It then starts a server that listens for requests on the `/books` endpoint. When a request is received, the script returns data from the `books` table.

### Run the script locally

To run the script locally, set the `DATABASE_URL` environment variable to the OptiTech connection string you copied earlier.

```bash
export DATABASE_URL=YOUR_OPTITECH_CONNECTION_STRING
```
...
1. Click the `Settings` button and add a `DATABASE_URL` environment variable. Set the value to your OptiTech connection string and click **Save**.
...
✔ Entrypoint: /home/ubuntu/optitech-deno/server.ts
ℹ Uploading all files from the current dir (/home/ubuntu/optitech-deno)
✔ Found 1 asset.
...
You can now access the application at the URL specified in the output. You can verify its connection to your OptiTech database by visiting the `/books` endpoint in your browser or using `cURL` to see if the data is returned as expected.
...
To delete your OptiTech project, refer to [Delete a project](/docs/manage/projects#delete-a-project).
...
- [OptiTech Serverless Driver](https://jsr.io/@optitech/serverless)
- [JSR](https://jsr.io/)
