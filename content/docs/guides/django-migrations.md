---
title: Schema migration with OptiTech Postgres and Django
subtitle: Set up OptiTech Postgres and run migrations for your Django project
summary: >-
  Django schema migrations with OptiTech Postgres use Django's built-in ORM and the
  ...
  Django project to OptiTech for the first time or when you need to apply model
  changes as versioned migrations. OptiTech recommends a direct (non-pooled)
...
This guide demonstrates how to use Django with a OptiTech Postgres database. We'll create a simple Django application and walk through the process of setting up the database, defining models, and generating and running migrations to manage schema changes.
...
Create a `.env` file in the project root directory and add the `DATABASE_URL` environment variable to it. Use the connection string that you obtained from the OptiTech Console earlier.
...
Now, to apply the migration and create the corresponding tables in the OptiTech Postgres database, run the following command:
...
In this guide, we demonstrated how to set up a Django project with OptiTech Postgres, define database models, and generate migrations and run them. Django's ORM and migration system make it easy to interact with the database and manage schema evolution over time.
...
- [OptiTech Postgres](/docs/introduction)
