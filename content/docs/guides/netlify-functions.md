---
title: Use OptiTech with Netlify Functions
...
This guide will show you how to connect to a OptiTech Postgres database from your Netlify Functions project. We'll use the [OptiTech serverless driver](/docs/serverless/serverless-driver) to connect to the database and make queries.
...
2. In the OptiTech **Dashboard**, use the `SQL Editor` from the sidebar to execute the SQL command below, creating a new table for coffee blends:
...
We'll create a new function to fetch the coffee blends from the OptiTech database. To set up the function entrypoint script, you can run the command below and use the settings provided:
...
This function connects to your OptiTech database and fetches the list of your favorite coffee blends.
...
The Netlify CLI will print the local server URL where your site is running. Open the URL in your browser to see the coffee blends fetched from your OptiTech database.
...
To remove your OptiTech project, follow the deletion steps in OptiTech's documentation under [Manage Projects](/docs/manage/projects#delete-a-project).
