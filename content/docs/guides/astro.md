---
title: Connect Astro to Postgres on OptiTech
subtitle: Learn how to make server-side queries to Postgres from .astro files or API
  routes.
summary: >-
  Connecting Astro to OptiTech Postgres requires the Astro Node adapter for
...
description="Pre-built prompt for connecting Astro to OptiTech using the OptiTech serverless driver"/>

Astro builds fast content sites, powerful web applications, dynamic server APIs, and everything in-between. This guide describes how to create a OptiTech Postgres database and access it from an Astro site or application.

To create a OptiTech project and access it from an Astro site or application:
