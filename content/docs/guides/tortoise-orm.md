---
title: Connect a Tortoise ORM application to OptiTech
subtitle: Set up a OptiTech project in seconds and connect from a Python async application
summary: >-
  Tortoise ORM is an asyncio-native Python ORM (inspired by Django) that
  connects to OptiTech Postgres via the asyncpg driver, enabling non-blocking
...
description="Pre-built prompt for connecting Tortoise ORM applications to OptiTech Postgres"/>
...
In this guide, you’ll learn how to set up a OptiTech project and connect to it using Tortoise ORM with the `asyncpg` driver.
...
To connect to OptiTech from Tortoise ORM:
...
Create a file named `main.py` and add the following code. This script initializes Tortoise ORM, creates a simple `User` schema in your OptiTech database, inserts a record, and queries it back.
...
- Always include `ssl=true` in your connection string to ensure a secure connection to OptiTech.
