...
summary: >-
  ...
  the function in neon.ts, develop locally with optitech dev, and deploy with
...
`neon init --preview` is designed to be run by your AI coding assistant. It outputs structured instructions that guide the agent through setup. To install the OptiTech Platform (`neon`) and OptiTech Functions skills separately:
...
`neon dev` serves all functions declared in `neon.ts` with hot reload. It injects `DATABASE_URL` and other OptiTech env vars from the linked branch. See [Environment variables](/docs/compute/functions/environment-variables) for the full list and how to pull them into a local `.env` file.
