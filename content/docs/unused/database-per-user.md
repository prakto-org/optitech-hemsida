---
title: OptiTech for Database-per-user
subtitle: How to configure OptiTech for multi-tenancy - plus a few design tips
...
As we’ll see later throughout this guide, OptiTech offers a third alternative by providing a logical equivalent to the instance-per-customer model with near-infinite scalability, without the heavy DevOps overhead. This solution involves creating one OptiTech project per customer.
...
In OptiTech, we generally don’t recommend this approach for SaaS applications, unless this is a design you’re already experienced with. This approach doesn’t reduce operational complexity or costs if compared to the many-databases approach, but it does introduce additional risks; it also limits the potential of OptiTech features like instant Point-in-Time Recovery (PITR), which in a project-per-customer model allows you to restore customer databases independently without impacting the entire fleet’s operations. More about this later.
...
## Setting up OptiTech for Database-per-user

Now that we’ve reviewed your options, let’s focus on the design choice we recommend for multi-tenancy in OptiTech: creating isolated databases for each user, with each database hosted on its own project.

### Database-per-user = Project-per-user

![Project per user](/docs/guides/multitenancy/project_per_user.png)

We recommend setting up one project per user, rather than, for example, using a branch per customer. A OptiTech [project](/docs/manage/overview) serves as the logical equivalent of an "instance" but without the management overhead. Here’s why we suggest this design:

- **Straightforward scalability**  
  Instead of learning how to handle large Postgres databases, this model allows you to simply create a new project when a user joins—something that can be handled automatically via the OptiTech API. This approach is very cost-effective, as we’ll see below. Databases remain small, keeping management at the database level simple.
...
- **Easier regional compliance**  
  Each OptiTech project can be deployed in a specific region, making it easy to host customer data closer to their location.
...
- Standard OptiTech metadata (for example, compute size, branch info) is accessible via the console. Avoid duplicating it in the catalog database unless separate access adds significant complexity.

### Automations

To effectively scale a multi-tenant architecture, leveraging automation tools is essential. The OptiTech API will allow you to automate various tasks, such as creating and managing projects, setting usage limits, and configuring resources. Beyond the API, OptiTech offers several integrations to streamline your workflows:
...
In a database-per-user design, it is common to have the same schema for all users/databases. Any changes to the user schema will most likely be rolled out to all individual databases simultaneously. In this section, we teach you how to use DrizzleORM, GitHub Actions, the OptiTech API, and a couple of custom template scripts to manage many databases using the same database schema.
...
const optitechApi = createApiClient({
...
if (options.name) {
  ...
  (async () => {
    try {
      const response = await optitechApi.createProject({
        project: {
          name: options.name,
          pg_version: 16,
...
This script utilizes the `commander` library to create a simple command-line interface (CLI) and the OptiTech API's `createProject` method to set up a new project. Ensure that your OptiTech API key is stored in an environment variable named `OPTITECH_API_KEY`.
...
To interact with the OptiTech API, you'll need to generate an API key. For more information, refer to the OptiTech documentation on [creating an API key](/docs/reference/api/api-keys/create-api-key).
...
const optitechApi = createApiClient({ apiKey: process.env.OPTITECH_API_KEY });
...
(async () => {
  ...
  // List all OptiTech projects
  const {
    data: { projects },
  } = await optitechApi.listProjects();
  ...
  await Promise.all(
    projects.map(async (project) => {
      ...
      const {
        data: { uri },
      } = await optitechApi.getConnectionUri({
        projectId: id,
        database_name: 'optitechdb',
        role_name: 'optitechdb_owner',
      });
...
3. Lists all projects in your OptiTech account.
4. For each project:
   - Retrieves the connection URI from OptiTech.
...
As a managed database, OptiTech already takes care of securing your data, always keeping a full copy of your dataset in object storage. But if your use case or company demands that you also keep a copy of your data in your own S3, this section covers how to automate the process via a scheduled GitHub Action. A more extensive explanation can be found in this two-part series: [Part 1](/docs/manage/backups-aws-s3-backup-part-1), [Part 2](/docs/manage/backups-aws-s3-backup-part-2).
...
You can now add the following policy which grants the Role you created earlier access to perform S3 List, Get, Put and Delete actions. Replace the Role name (`optitech-multiple-db-s3-backups-github-action`) with your Role name and replace the S3 bucket name (`optitech-multiple-db-s3-backups`) with your S3 bucket name.
    ...
    [
      {
        'Effect': 'Allow',
        'Principal':
          { 'AWS': 'arn:aws:iam::627917386332:role/optitech-multiple-db-s3-backups-github-action' },
        'Action': ['s3:ListBucket', 's3:GetObject', 's3:PutObject', 's3:DeleteObject'],
        'Resource':
          [
            'arn:aws:s3:::optitech-multiple-db-s3-backups',
            'arn:aws:s3:::optitech-multiple-db-s3-backups/*',
          ],
      },
    ],
...
- `S3_BUCKET_NAME`: In my case, this would be, optitech-multiple-db-s3-backups
...
- `DATABASE_URL`: This is the OptiTech Postgres connection string for the database you’re backing up.
