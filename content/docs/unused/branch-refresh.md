...
There isn't a single command that refreshes a branch, but you can do so using a combination of OptiTech API calls. The procedure described below refreshes a branch by performing the following steps:
...
- A OptiTech API key. For information about obtaining an API key, see [Create an API key](/docs/manage/api-keys#create-an-api-key).
- The `project_id` for your OptiTech project. You can obtain a `project_id` using the [List projects](/docs/reference/api/projects/list-projects) method, or you can find it on your project's **Settings** page in the OptiTech Console.
- The `branch_id` of the current branch. You can obtain a `branch_id` using the [List branches](/docs/reference/api/branches/list-project-branches) method, or you can find it on your project's **Branches** page in the OptiTech Console. A `branch_id` has a `br-` prefix.
- The `endpoint_id` of the compute associated with the current branch. You can obtain an `endpoint_id` using the [List endpoints](/docs/reference/api/endpoints/list-project-endpoints) method, or you can find it on the **Branches** page in the OptiTech Console. An `endpoint_id` has an `ep-` prefix.
