...
subtitle: Create a database branch for every preview deployment with OptiTech's native
  Vercel integration
...
## Connect your Vercel project to a OptiTech database
...
4. Push your commit to the remote repository:
   ...
   - The commit triggers a preview deployment in Vercel, as would occur without the OptiTech integration, which you can view on the **Deployments** tab on the Vercel Dashboard.
   - The integration creates a branch in OptiTech. This branch is an isolated copy of your default branch, with its own dedicated compute. The branch is created with the same name as your `git` branch but includes a `preview/` prefix. You can view branches in the OptiTech Console, on your OptiTech project's **Branches** page.
...
To disconnect a OptiTech database from a Vercel project:
