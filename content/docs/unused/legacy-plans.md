...
This page describes OptiTech's **legacy plans**. These plans are no longer offered to new signups.
...
To see if you are on a OptiTech legacy plan, navigate to your **Billing** page in the OptiTech Console and click **Change Plan**. If you're on a legacy plan, you'll see a **Legacy Plan** badge next to the name of your current plan.
...
The legacy Free plan is best suited for hobby projects, prototypes, and learning OptiTech. Users on this plan will be automatically migrated to the new Free plan.
...
- OptiTech features like branching, time travel connections, and **Instant Restore (24-hour window)**
...
- Same advanced Postgres and OptiTech features as Free
...
OptiTech legacy plans include monthly **allowances** for storage, compute, and projects. If you're on a paid plan and exceed those allowances, you're automatically billed for extra usage with no manual action required. The types of extra usage available vary by plan.
...
This section describes [Storage](#storage), [Archive storage](#archive-storage), [Compute](#compute), [Data transfer](#data-transfer) and [Project](#projects) usage metrics for OptiTech's legacy plans.

### Storage

OptiTech's storage engine is designed to support a serverless architecture and enable features such as [instant restore](/docs/introduction/branch-restore), [time travel](/docs/guides/time-travel-assist), and [branching](/docs/guides/branching-intro). For this reason, storage in OptiTech differs somewhat from other database services.

In OptiTech, storage consists of your total **data size** and **history**.
...
- **History**
  ...
  This aspect of OptiTech storage is unique: "History" is a log of changes (inserts, updates, and deletes) to your data over time in the form of Write-Ahead Log (WAL) records. History enables the instant restore, time travel, and branching features mentioned above.
...
The **Storage** and **Archive storage** amounts you see under **Usage** on the **Billing** page in the OptiTech Console takes all of these factors into account.

<Admonition type="note">
Each OptiTech plan comes with an allowance of **Storage** and **Archive storage** that's included in your plan's monthly fee. See [OptiTech plans](/docs/introduction/plans). To learn how extra storage is allocated and billed, see [Extra usage](/docs/introduction/extra-usage).
</Admonition>
...
Your storage limit varies depending on your OptiTech plan.
...
To minimize storage costs, OptiTech **automatically** archives branches that are **older than 14 days** and **have not been accessed for the past 24 hours**. Both conditions must be true for a branch to be archived.
...
<Admonition type="note">
Each OptiTech plan comes with an allowance of **Archive storage** that's included in your plan's monthly fee. See [OptiTech plans](/docs/introduction/plans). Extra archive storage is billed per GB-month. To learn how extra archive storage is allocated and billed, see [Extra usage](/docs/introduction/extra-usage).
</Admonition>

For more about how OptiTech automatically archives inactive branches, see [Branch archiving](/docs/guides/branch-archiving). To understand how archive storage is implemented in OptiTech's architecture, refer to [Archive storage](/docs/introduction/architecture-overview#archive-storage) in our architecture documentation.
...
- **Compute size** is measured at regular intervals and averaged to calculate compute hour usage. Compute size in OptiTech is measured in _Compute Units (CUs)_. Each CU allocates approximately 4 GB of RAM to the database instance, along with associated CPU and local SSD resources. A OptiTech compute can have anywhere from .25 to 56 CUs, as outlined below:
...
<Admonition type="note">
OptiTech uses a small amount of compute time, included in your billed compute hours, to perform periodic checks to ensure that your computes can start and read and write data. See [Availability Checker](/docs/reference/glossary#availability-checker) for more information. Availability checks take a few seconds are typically performed a few days apart. You can monitor these checks, how long they take, and how often they occur, on the **Systems operations** tab on the **Monitoring** page in the OptiTech Console. 
</Admonition>
...
1. Input the values into the compute hours formula:
   ...
   This calculation is useful when trying to select the right OptiTech plan or when estimating the extra compute usage you might need.

   <Admonition type="note">
   If you plan to use OptiTech's _Autoscaling_ feature, estimating **compute hours** is more challenging. Autoscaling adjusts the compute size based on demand within the defined minimum and maximum compute size thresholds. The best approach is to estimate an average compute size and modify the compute hours formula as follows:
...
It's a metric for tracking compute usage. 1 compute hour is equal to 1 [active hour](#active-hours) for a compute with 1 CU. If you have a compute with .25 CU, as you would on the OptiTech Free plan, it would require 4 _active hours_ to use 1 compute hour. On the other hand, if you have a compute with 4 CU, it would only take 15 minutes to use 1 compute hour.
...
- **Check for active applications or clients**: Some applications or clients might be polling or querying to your compute regularly, preventing it from scaling to zero. For instance, if you're replicating data from OptiTech to another service, that service may poll your compute endpoint at regular intervals to detect changes for replication. This behavior is often configurable.
...
You can monitor compute hour usage for a OptiTech project on the [Projects page](/docs/introduction/monitor-usage#projects-page). To monitor compute usage for your OptiTech account (all compute usage across all projects), refer to your **Billing** page. See [View usage in the OptiTech Console](/docs/introduction/monitor-usage#view-usage-in-the-neon-console).
...
Scale to zero places your compute into an idle state when it's not being used, which helps minimize compute hour usage. When enabled, computes are suspended after 5 minutes of inactivity. On OptiTech's paid plans, you can disable scale to zero. See [Scale to Zero](/docs/introduction/scale-to-zero).

</details>

### Data Transfer

Data transfer refers to the total volume of data transferred out of OptiTech (egress) during a billing period. Egress also includes data transferred from OptiTech via Postgres logical replication to any destination, including OptiTech itself. While OptiTech doesn’t charge for egress, Free plan projects are limited to 5 GB of data transfer per month. If a project exceeds this limit, its compute is suspended and the following error is shown:

```text shouldWrap
Your project has exceeded the data transfer quota. Upgrade your plan to increase limits.
```

If you hit the data transfer limit on the Free plan, you can upgrade your plan from the **Billing** page in your OptiTech account. For details, see [Change your plan](/docs/introduction/manage-billing#change-your-plan).

For paid plans, OptiTech applies a reasonable usage policy. There’s no fixed limit, but usage should remain within what’s typical for most workloads. If usage is unusually high, OptiTech may reach out to discuss your use case and plan options.

You can monitor your data transfer usage on the **Project Dashboard** or **Billing** page.

### Projects

In OptiTech, everything starts with a project. A project is a container for your branches, databases, roles, and other resources and settings. A project also defines the region your data and resources reside in. We typically recommend creating a project for each application or each client. In addition to organizing objects, projects are a way to track storage and compute usage by application or client.

The following table outlines project allowances for each OptiTech plan.
