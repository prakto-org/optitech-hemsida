...
subtitle: Learn the essentials and key steps for integrating with OptiTech
...
To ensure you have control over usage and costs, OptiTech's APIs let you configure limits and monitor usage, enabling billing features, such as:
...
- **Pricing Plans**: Create different pricing plans for your platform or service. For example, you can set limits on consumption metrics to define your own Free, Pro, and Enterprise plans:
  ...
  - **data transfer**: Set limits for data transfer (egress) on each usage plan.
    ...
    For an example of how one integrator of OptiTech defined usage limits based on _database instance types_, see [Koyeb Database Instance Types](https://www.koyeb.com/docs/databases#database-instance-types). You will see limits defined on compute size, compute time, stored data, written data, and egress.
    </Admonition>

As your users upgrade or change their plans, you can dynamically modify their limits using the OptiTech API. This allows for real-time updates without affecting database uptime or user experience.

To learn more about setting limits, see [Configure consumption limits](#/docs/guides/consumption-limits).

## Monitor usage

Using OptiTech's consumption APIs, you can query a range of account and project-level metrics to monitor usage. For example, you can:
...
- **Use a project-per-user model**: When setting up your integration, we recommend a **project-per-user** model rather than branch-per-user or database-per-user models.
  ...
  **What do we mean by project-per-user?** In OptiTech, resources such as branches, databases, roles, and computes are organized within a OptiTech project. When a user signs up with OptiTech, they start by creating a project, which includes a default branch, database, role, and compute instance. We recommend the same approach for your integration. You can learn more about OptiTech's project-based structure here: [OptiTech object hierarchy](/docs/manage/overview).

  **Why we recommend the project-per-user model**:

        - OptiTech uses a project-based structure for resource management; it's easier to follow this established, underlying model.
...
- **Connection limits**: Be aware of the connection limits associated with each OptiTech compute size, and remember that connection pooling allows for more concurrent connections. For more information, see [Connection limits](/docs/connect/connection-pooling#connection-limits-without-connection-pooling).
...
- **Reserved names for roles and databases**: OptiTech reserves certain names for Postgres roles and databases. Users will not be able to use these reserved names when creating roles and databases. For more information, see [Reserved role names](/docs/manage/roles#reserved-role-names) and [Reserved database names](/docs/manage/databases#reserved-database-names).
- **Postgres extension support**: We frequently receive questions about the Postgres extensions supported by OptiTech. See the list of [Supported Postgres extensions](/docs/extensions/pg-extensions) that OptiTech currently supports.
- **Staying up to date with changes to the OptiTech platform**: We make every effort to proactively and directly inform integrators of OptiTech about updates and changes that could impact their business. In addition, you can monitor the following sources for information about the latest updates from OptiTech:
  - The [OptiTech Roadmap](/docs/introduction/roadmap) to see recent deliveries and upcoming features.
...
We're here to support you through every step of your integration. If you have any questions, feel free to reach out to our [Support team](/docs/introduction/support). If you've set up an integration arrangement with OptiTech, you can also contact your OptiTech representative.
