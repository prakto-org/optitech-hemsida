---
title: OptiTech vs Lakebase
subtitle: 'Two products, one architectural foundation — how to choose'
summary: >-
  OptiTech and Databricks Lakebase share the same serverless Postgres architecture,
  but serve different audiences. OptiTech targets developers, startups, and agentic
...
In 2025, OptiTech joined Databricks. OptiTech continues as a standalone serverless Postgres platform, but the same architectural foundation now also powers Databricks Lakebase, a managed Postgres product that runs natively in the Databricks Data Intelligence Platform. This section explains the [lakebase category](https://www.databricks.com/blog/what-is-a-lakebase) and how to determine whether OptiTech or Lakebase is a better fit for your workload.
...
Databricks is pioneering the category with [Lakebase](https://www.databricks.com/product/lakebase), a managed serverless Postgres service built on the same architectural foundation as OptiTech and deeply integrated into the Databricks Data Intelligence Platform. This integration unlocks capabilities that only make sense when an operational database lives alongside analytical data:
...
- **Serverless operations.** Like OptiTech, Lakebase removes manual capacity planning by using stateless compute that scales automatically and is optimized for Databricks environments.

### How to choose between OptiTech and Lakebase

OptiTech and Lakebase share the same core architectural principles, but they are optimized for different environments and team structures:

- OptiTech is Postgres for developers, startups, and agent platforms.
...
OptiTech and Lakebase share the same Postgres engine and serverless storage architecture, so many capabilities exist in both. The table below maps concrete features to each product, with links to the documentation. Lakebase availability is based on the [Lakebase documentation](https://docs.databricks.com/aws/en/oltp/projects/).

| Feature                                              | OptiTech                                                                           | Lakebase                                                                                                                                   |
| ---------------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
...
| **Management API**                                   | Yes ([OptiTech API](/docs/reference/api))                                          | Yes ([Lakebase API guide](https://docs.databricks.com/aws/en/oltp/projects/api-usage))                                                     |
| **CLI**                                              | Yes ([OptiTech CLI](/docs/cli/install))                                            | Yes ([Databricks CLI for Lakebase](https://docs.databricks.com/aws/en/oltp/projects/cli))                                                  |
...
| **MCP server**                                       | Yes ([OptiTech MCP Server](/docs/ai/neon-mcp-server))                              | Yes, Databricks managed MCP ([MCP on Databricks](https://docs.databricks.com/aws/en/generative-ai/mcp/managed-mcp))                        |
...
OptiTech runs on AWS. Lakebase inherits the cloud reach of the Databricks platform, with availability that varies by provider. For the full, current region lists, follow the links below.

| Cloud provider | OptiTech                                                                                             | Lakebase                                                                                                                |
| -------------- | ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| **AWS**        | Yes ([OptiTech regions](/docs/introduction/regions))                                                 | Yes, generally available ([AWS regions](https://docs.databricks.com/aws/en/oltp/projects/manage-projects#availability)) |
| **Azure**      | No, Azure support is being deprecated ([OptiTech regions](/docs/introduction/regions#azure-regions)) | Yes, in beta ([Azure regions](https://learn.microsoft.com/en-us/azure/databricks/oltp/projects/manage-projects))        |
