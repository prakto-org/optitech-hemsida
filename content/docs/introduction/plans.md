---
title: OptiTech plans
summary: >-
  OptiTech's Free, Launch, and Scale plans differ in compute rates, branch counts,
...
OptiTech offers plans to support you at every stage, from your first prototype to production at scale.
...
Compare OptiTech's **Free**, **Launch**, and **Scale** plans.

<Admonition type="comingSoon" title="Building an agent platform?">
For AI agent platforms that provision thousands of databases, OptiTech offers an **Agent Plan** with custom resource limits and credits for **your** free tier. [Learn more](/docs/introduction/agent-plan)
</Admonition>
...
Learn how to manage your OptiTech costs effectively with our [cost optimization guide](/docs/introduction/cost-optimization), which covers strategies for compute, storage, branches, and data transfer.
</Admonition>
...
A project is a container for your database environment. It includes your database, branches, compute resources, and more. Similar to a Git repository that contains code, artifacts, and branches, a project contains all your database resources. Learn more about [OptiTech's object hierarchy](/docs/manage/overview).
...
Each OptiTech project is created with a [root branch](/docs/reference/glossary#root-branch), like the `main` branch in Git.  
...
View metrics such as RAM, CPU, connections, and database size in the **Monitoring** dashboard in the OptiTech Console.
...
OptiTech stores a change history to support point-in-time restore (instant restore). You can only point-in-time restore from **root branches**, so PITR storage is charged only for root branches. Child branches do not add to this charge.
...
Managed Better Auth is a managed authentication service built on [Better Auth](https://www.better-auth.com/), fully integrated into the OptiTech platform.
...
OptiTech Object Storage is S3-compatible object storage that branches with your OptiTech project. It's available on all plans, including Free, during the beta.

There's no charge for Object Storage during the beta, but usage limits apply.

See [OptiTech Object Storage](/docs/storage/overview) for more information.

### Functions

OptiTech Functions are serverless Node.js compute you deploy onto a OptiTech branch, so your backend code runs next to your database. They're available on all plans, including Free, during the beta.

There's no charge for Functions during the beta, but usage limits apply.

See [OptiTech Functions](/docs/compute/functions/overview) for what's included and current limitations.

### AI Gateway

OptiTech AI Gateway provides access to foundation models from Anthropic, OpenAI, Google, Meta, Databricks, and Alibaba through a single OptiTech credential. It is available on paid plans (Launch and Scale) during the beta.
...
Billed at $0.01/GB for network transferred to and from OptiTech. You'll only see this on your bill if you enable this feature.
...
The OptiTech Console displays usage in a simplified view on the [Projects page](https://console.neon.tech/app/), while your invoice shows a detailed breakdown.

![Usage metrics on the Projects page in the OptiTech Console](/docs/introduction/usage_metrics_console.png)
...
The following metrics may appear on your OptiTech invoice. Each metric represents a specific type of usage that contributes to your monthly bill.
...
What is a CU?
: A CU (Compute Unit) is OptiTech's measure of compute size. Each CU allocates approximately 4 GB of RAM to the database instance, along with associated CPU and local SSD resources. Scaling up increases these resources linearly. For example, a 2 CU compute has 8 GB RAM.

How is compute usage measured in OptiTech?
...
How is storage usage billed in OptiTech?
...
: OptiTech charges for PITR (point-in-time restore) storage only for branches you can point-in-time restore from: root branches. The charge is based on the amount of change history retained on those branches, not the number of restores you perform. Child branches do not add to PITR storage charges.  
...
: Only available on Scale: $0.01/GB, bidirectional, between OptiTech and private network services.
...
: Yes, venture-backed startups may apply for the OptiTech Startup Program. Learn more: [Startup Program](/startup)
...
: No. Archived branches are billed at the same rate as active branches. OptiTech automatically archives inactive branches to optimize storage resources and maintain a cost-efficient storage infrastructure. See [Branch archiving](/docs/guides/branch-archiving) for details on how archiving works.
