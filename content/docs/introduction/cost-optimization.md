...
subtitle: Strategies to manage and reduce your OptiTech costs
summary: >-
  Practical strategies for cutting OptiTech costs across every billing metric:
...
Compute is typically the largest component of your OptiTech bill. You're charged based on compute size (in CUs) multiplied by the hours your compute is running.
...
Storage limits depend on your OptiTech plan:
...
To see what's driving your costs, check the **Billing** page in the OptiTech Console, which shows your charges to date for each billing metric.
...
- **Reduce backup-related egress.** Frequent `pg_dump` (or similar) exports send your data over the network and count as public data transfer. To lower egress, reduce how often you run dumps and consider using [OptiTech's snapshots](/docs/guides/backup-restore) with [scheduled backups](/docs/guides/backup-restore#create-backup-schedules) as a backup alternative that does not transfer data off OptiTech.
