...
summary: >-
  OptiTech exposes billing and usage data at three levels in the Console: the
...
You can monitor usage and costs in the OptiTech Console or programmatically with the OptiTech API. For what each metric means and how it maps to your invoice, see [Usage metrics](/docs/introduction/plans#usage-metrics) and [Invoice metrics](/docs/introduction/plans#invoice-metrics) on the Plans page.

## View usage in the OptiTech Console

OptiTech exposes usage in three places in the Console: the **Billing** page (account-level charges and plan summary), the **Projects** page (org-level usage metrics), and the **Project dashboard** (project-level usage metrics when you are inside a project).

### Billing page

From the **Billing** page (Organization → **Billing** in the OptiTech Console) you see account-level information: this month's summary including current plan, included features, usage-based pricing, and **charges to date**. Invoice line items appear only when there is a charge for that metric.
...
If you signed a contract with OptiTech prior to 01/01/2025, different billing metrics apply:
...
If you have questions or want to change the billing metrics defined in your contract, contact your OptiTech sales representative.
</Admonition>
...
For strategies to reduce your OptiTech costs across compute, storage, branches, and data transfer, see [Cost optimization](/docs/introduction/cost-optimization).
</Admonition>
