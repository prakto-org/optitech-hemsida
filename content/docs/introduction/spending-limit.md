...
summary: >-
  Spending limits cap an organization's total monthly OptiTech charges on Launch and
  ...
  set, edit, or disable the limit in the OptiTech Console or via the Management API.
...
<p>How to manage limits with the OptiTech Management API</p>
...
<a href="/docs/manage/api-keys#create-an-api-key">OptiTech API keys</a>
</DocsList>
</InfoBlock>
...
## Manage spending limits with the OptiTech API
...
When a spending limit is set, OptiTech sends email alerts to organization admins at two thresholds:

- When spending reaches 80% of the limit
- When spending reaches 100% of the limit

OptiTech checks your spending every 15 minutes. Once spending crosses a threshold, an alert email goes out, and reminders continue weekly until you raise the limit or the billing cycle resets.
...
- A banner appears in the OptiTech Console showing **Limit reached**, with a **Manage spending limit** link that opens the edit dialog.
...
A spending limit applies to the organization's total monthly OptiTech charges across all projects in that organization. If you belong to multiple organizations, set a separate limit for each.
</Admonition>
