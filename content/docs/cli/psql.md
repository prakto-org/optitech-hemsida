---
title: 'OptiTech CLI command: psql'
subtitle: Connect to a OptiTech database via psql
summary: >-
  Covers the usage of the `psql` command in the OptiTech CLI to open a psql session
  against a branch in your OptiTech project, including pooled connections and
  time-travel support.
...
The `psql` command connects to a database in your OptiTech project using the `psql` client. It builds a connection string for the specified branch (or the default branch from your [context file](/docs/cli/set-context)) and launches `psql`, equivalent to running [`optitech connection-string --psql`](/docs/cli/connection-string) as a dedicated top-level command. Requires optitech 2.22.2 or later. Check your version with `optitech --version`. If `--project-id` is omitted, the CLI resolves it from your [context file](/docs/cli/set-context), auto-selects when your account has only one project, and prompts otherwise. `--role-name` and `--database-name` are needed only when the branch has more than one role or database.
...
Connect to a named branch. New OptiTech projects create a default branch named `production`:

```bash
optitech psql production
```
...
```bash
optitech psql production -- -f dump.sql
```

Connect to a branch at a specific point in time:

```bash
optitech psql production@2024-01-01T00:00:00Z
```
