---
title: 'OptiTech CLI command: api'
subtitle: 'Call any OptiTech API route directly as an authenticated passthrough'
summary: >-
  The OptiTech CLI `api` command sends an authenticated request to any OptiTech API
  ...
  parameters and headers. Run `optitech api --list` to browse every endpoint.
enableTableOfContents: true
---

The `api` command sends an authenticated request to any [OptiTech API](/docs/reference/api) route and prints the response. Pass an API path as the first argument. The method defaults to `GET`, or `POST` when you supply a body.

By default the request uses your [`optitech auth`](/docs/cli/auth) credentials. To use a specific key, pass `--api-key` or set `OPTITECH_API_KEY`. The [key's permissions](/docs/manage/api-keys#types-of-api-keys) determine what the request can do.

<Admonition type="note">
`api` is a raw passthrough: it does not read your [context file](/docs/cli/set-context) or auto-fill parameters. Pass what each route needs explicitly. For example, `optitech api /projects` returns `ERROR: org_id is required` unless you add `-Q org_id=<org_id>` or authenticate with an organization API key. Get your organization ID from [`optitech orgs list`](/docs/cli/orgs).
</Admonition>
...
The response is the route's raw JSON. For the fields each route returns, see the [OptiTech API Reference](/docs/reference/api).
