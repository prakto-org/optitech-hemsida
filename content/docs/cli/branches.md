---
title: 'OptiTech CLI command: branches'
subtitle: 'List, create, rename, and delete branches; set default; run schema diff'
summary: >-
  The OptiTech CLI `branches` command provides subcommands (list, create, reset,
  restore, rename, schema-diff, set-default, set-expiration, add-compute,
  delete, get) to manage OptiTech project branches from the terminal. Use this
...
The `branches` command lists, creates, renames, deletes, and retrieves details about branches in your OptiTech project. It also sets the default branch, adds a compute or [read replica](/docs/introduction/read-replicas) to a branch, restores a branch to an earlier point in time, and runs a [schema diff](/docs/guides/schema-diff) between branches. For information about branches in OptiTech, see [Manage branches](/docs/manage/branches). If `--project-id` is omitted, the CLI resolves it from your [context file](/docs/cli/set-context), auto-selects when your account has only one project, and prompts otherwise.

<Admonition type="tip" title="Switch the active branch">
To pin a branch in your local `.optitech` context file so subsequent commands target it, use [`optitech checkout`](/docs/cli/checkout).
</Admonition>

<CliSubcommands command="branches" />

## optitech branches list (#list)

Lists branches in a OptiTech project.
...
```bash
optitech branches list --project-id solitary-leaf-288182
```
...
Branch names include text labels that indicate status: `[default]` marks the project's default branch, `[protected]` marks a protected branch, and `[current]` marks the branch pinned in your local `.optitech` context file.

List branches with `--output json`, which returns more information than the `table` format:

```bash
optitech branches list --project-id solitary-leaf-288182 --output json
```
...
## optitech branches create (#create)

Creates a branch in a OptiTech project.
...
```bash
optitech branches create --output json
```
...
```bash
optitech branches create --name feature/user-auth
```

Set the compute size when creating a branch:

```bash
optitech branches create --name mybranch --cu 2
```

Set the compute's autoscaling range when creating a branch:

```bash
optitech branches create --name mybranch --cu 0.5-3
```

Create a branch with a read replica compute:

```bash
optitech branches create --name my_read_replica_branch --type read_only
```

Create a branch from a parent branch other than your `main` branch:

```bash
optitech branches create --name feature/payment-api --parent development
```

Create an instant restore branch by specifying the `--parent` option with a timestamp:

```bash
optitech branches create --name data_recovery --parent 2023-07-11T10:00:00Z
```
...
```bash
optitech branches create --psql
optitech branches create --psql -- -f dump.sql
optitech branches create --psql -- -c "SELECT version()"
```

Create a schema-only branch:

```bash
optitech branches create --schema-only
```

## optitech branches reset (#reset)
...
```bash
optitech branches reset development --parent
```
...
## optitech branches restore (#restore)
...
```bash shouldWrap
optitech branches restore main ^self@2024-05-06T10:00:00.000Z --preserve-under-name main_restore_backup_2024-05-06
```
...
```bash
optitech branches restore feature/user-auth main
```
...
```bash
optitech branches restore feature/user-auth ^parent@2024-02-21T10:30:00.000Z
```
...
## optitech branches rename (#rename)
...
```bash
optitech branches rename mybranch teambranch
```
...
## optitech branches schema-diff (#schema-diff)
...
For a git-style shortcut that defaults to comparing a branch against its parent, see [`optitech diff`](/docs/cli/diff).
...
The output indicates that in the table `public.playing_with_optitech`, a new column `description character varying(255)` has been added in the `development` branch that is not present in the `production` branch.

```text filename="Output"
--- Database: optitechdb	(Branch: br-wandering-firefly-a50un462)
+++ Database: optitechdb	(Branch: br-fancy-sky-a5cydw8p)
@@ -26,9 +26,10 @@
...
```bash
optitech branches schema-diff feature/user-auth ^self@0/123456
```

Compare the schema of `feature/user-auth` to the head of its parent branch:

```bash
optitech branches schema-diff feature/user-auth ^parent
```

Compare the schema of the `production` branch to the state of `feature/payment-api` at timestamp `2024-06-01T00:00:00.000Z`:

```bash
optitech branches schema-diff production feature/payment-api@2024-06-01T00:00:00.000Z
```

## optitech branches set-default (#set-default)

Sets a branch as the default branch in your OptiTech project.
...
```bash
optitech branches set-default mybranch
```
...
## optitech branches set-expiration (#set-expiration)
...
```bash
optitech branches set-expiration mybranch --expires-at 2025-08-15T18:00:00Z
```

Remove expiration from a branch (omit the parameter):

```bash
optitech branches set-expiration mybranch
```

## optitech branches add-compute (#add-compute)

Adds a compute to an existing branch in your OptiTech project.
...
│ Id                  │ Host                                             │
├─────────────────────┼──────────────────────────────────────────────────┤
│ ep-rough-lab-865061 │ ep-rough-lab-865061.ap-southeast-1.aws.optitech.com │
└─────────────────────┴──────────────────────────────────────────────────┘
```
...
```bash
optitech branches add-compute main --cu 2
```

Set the compute's autoscaling range when adding a compute to a branch:

```bash
optitech branches add-compute main --cu 0.5-3
```

## optitech branches delete (#delete)

Deletes a branch in a OptiTech project.
...
```bash
optitech branches delete br-rough-sky-158193
```
...
```bash
optitech branches get production
```
...
```bash
optitech branches get production --output json
```
