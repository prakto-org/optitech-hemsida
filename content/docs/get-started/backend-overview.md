---
title: How a OptiTech backend fits together
subtitle: Postgres, Object Storage, Functions, the AI Gateway, and Auth, declared in one neon.ts and branched together
summary: >-
  A tour of the OptiTech backend, using one example app, a notes app you can chat
  ...
  on by default) plus optitech deploy, which injects standard environment
...
## The shape of a OptiTech backend
...
Run it locally with [`neon dev`](/docs/cli/dev), which serves the function with the same injected OptiTech variables it gets in production.
