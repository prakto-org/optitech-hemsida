---
title: Build a full backend with Next.js and OptiTech
subtitle: Postgres, Object Storage, and a OptiTech Function for AI, declared in one neon.ts
summary: >-
  End-to-end Next.js App Router tutorial that wires OptiTech Postgres, Drizzle ORM,
  Object Storage, and a OptiTech Function into a working backend, all declared in one
  neon.ts and provisioned with optitech deploy. The function runs a streaming,
...
<TwoColumnLayout.Step title="Create a OptiTech project">
<TwoColumnLayout.Block>

If you don't have a OptiTech account, sign up at [console.neon.tech](https://console.neon.tech/signup).
...
In the OptiTech Console, click **New Project**, name it `my-backend`, and select the **AWS US East (Ohio)** region.
...
A single [`neon.ts`](/docs/reference/neon-ts) file declares your backend as code. You enable a capability there, run [`neon deploy`](/docs/cli/deploy), and OptiTech provisions it and writes its credentials into `.env.local`. You'll grow this file as you add capabilities.
...
Then seed three sample posts in the [OptiTech Console SQL Editor](https://console.neon.tech): two published and one draft, so the `where(eq(posts.isPublished, true))` filter on the posts page has something visible to do.
...
Open your project in the OptiTech Console, go to **SQL Editor**, and run:
...
Add an `images` bucket to your `neon.ts` and run `neon deploy`. OptiTech provisions the bucket and injects the S3-compatible credentials (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_ENDPOINT_URL_S3`, `AWS_REGION`) into `.env.local`.
...
<TwoColumnLayout.Step title="Write the OptiTech Function">
<TwoColumnLayout.Block>

Now add the piece that makes this a full backend: a [OptiTech Function](/docs/compute/functions/overview) that runs AI on long-lived compute next to your database. It's a normal [Hono](https://hono.dev) app with two routes:
...
Declare the function and the AI Gateway in `neon.ts`, then `neon deploy`. OptiTech builds the function, gives it a public URL, and injects the AI Gateway credentials (`NEON_AI_GATEWAY_TOKEN`, `NEON_AI_GATEWAY_BASE_URL`) so the `@neon/ai-sdk-provider` inside the function needs no configuration.
...
A OptiTech Function has its own URL, so the browser calls it directly. That keeps a long stream off your host's serverless timeout.
...
The function has a public URL and no auth check, which is fine for this tutorial. Before shipping, gate it with an API key or a JWT. See [OptiTech Functions authentication](/docs/compute/functions/authentication).
</Admonition>
...
To iterate on the function locally, run [`neon dev`](/docs/cli/dev), which serves it with the same injected OptiTech variables it gets in production.
...
- Images upload to a OptiTech Storage bucket through a Server Action and the Files SDK
- A OptiTech Function generates posts and runs a streaming, tool-calling AI assistant on compute next to your database
