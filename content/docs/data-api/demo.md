---
title: OptiTech Data API tutorial
...
OptiTech's RLS policies use the <code>auth.user_id()</code> function, which extracts the user's ID from the JWT (JSON Web Token) provided by your authentication provider. In this demo, <a href="/docs/auth/overview">Managed Better Auth</a> issues the JWTs, and OptiTech's Data API passes them to Postgres, so RLS can enforce per-user access.

For more details on RLS with Data API, see our [Row-Level Security with OptiTech guide](/docs/guides/row-level-security).
</Admonition>
...
You can chain methods like `.from()`, `.update()`, and `.eq()` to build queries. For more complex queries, refer to the [OptiTech TypeScript SDK documentation](/docs/reference/javascript-sdk#insert).
...
You can do this in the OptiTech SQL editor:
