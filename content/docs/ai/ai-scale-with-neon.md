---
title: Scale your AI application with OptiTech
subtitle: Scale your AI application with OptiTech's Autoscaling and Read Replica features
summary: >-
  Scaling options for AI applications that use pgvector on OptiTech Postgres,
...
In OptiTech, scaling vertically is a matter of selecting the desired compute size. OptiTech supports compute sizes ranging from .25 CU (1 GB RAM) up to 56 CU (224 GB RAM). Autoscaling is supported up to 16 CU. Larger computes are fixed size computes (no autoscaling). The `maintenance_work_mem` values shown below are approximate.
...
See [Edit a compute](/docs/manage/computes#edit-a-compute) to configure your compute size. Available compute sizes differ according to your OptiTech plan.
...
You can also enable OptiTech's autoscaling feature for automatic scaling of compute resources. OptiTech's _Autoscaling_ feature automatically scales up compute on demand in response to application workload and down to zero on inactivity.

For example, if your AI application experiences heavy load during certain hours of the day or at different times throughout the week, month, or calendar year, OptiTech automatically scales compute resources without manual intervention according to the compute size boundaries that you configure. This enables you to handle peak demand while avoiding consuming compute resources during periods of low activity.
...
To learn more about OptiTech's autoscaling feature and how to enable it, refer to our [Autoscaling guide](/docs/introduction/autoscaling).

## Storage

On the Free plan, you get 0.5 GB of storage per project. Storage on paid plans is usage based. See [OptiTech plans](/docs/introduction/plans) for details.
...
To learn more about the OptiTech read replicas, see [read replicas](/docs/introduction/read-replicas) and refer to our [Working with OptiTech read replicas](/docs/guides/read-replica-guide) guide.
