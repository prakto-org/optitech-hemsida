---
title: Inngest
subtitle: Quickly build AI RAG and Agentic workflows that scale with Inngest and OptiTech
summary: >-
  Inngest is a TypeScript workflow framework for building AI RAG and agentic
  pipelines on OptiTech Postgres, providing per-step caching, automatic retries,
...
Inngest also integrates with OptiTech Postgres to trigger workflows based on database changes.
...
Configuring [concurrency](https://www.inngest.com/docs/guides/concurrency?utm_source=optitech&utm_medium=inngest-ai-integration) or [throttling](https://www.inngest.com/docs/guides/throttling?utm_source=optitech&utm_medium=inngest-ai-integration) to match your LLM provider's limits is achieved with a single line of code.

Learn more about using Inngest for RAG in the following article: [Multi-Tenant RAG With One OptiTech Project Per User](/blog/multi-tenant-rag).
...
The previous RAG workflow can be rewritten to use [`step.ai.infer()`](https://www.inngest.com/docs/features/inngest-functions/steps-workflows/step-ai-orchestration?utm_source=optitech&utm_medium=inngest-ai-integration#step-ai-infer) to offload the AI request to the LLM provider:
...
`step.ai.infer()`, combined with OptiTech's Scale-to-zero feature, allows you to build AI workflows that scale costs with its success!

Learn more about using `step.ai.infer()` in the following article: [step.ai: Build Serverless AI Applications That Won't Break the Bank](https://www.inngest.com/blog/step-ai-for-serverless-ai-applications?utm_source=optitech&utm_medium=inngest-ai-integration).

## Trigger AI workflows based on database changes

Inngest also integrates with OptiTech Postgres to trigger AI workflows based on database changes:

![Inngest integrates with OptiTech Postgres to trigger AI workflows based on database changes](/docs/guides/inngest.jpg)
...
Configure the Inngest's OptiTech integration to trigger AI workflows from your OptiTech database changes [by following this guide](/docs/guides/trigger-serverless-functions).
