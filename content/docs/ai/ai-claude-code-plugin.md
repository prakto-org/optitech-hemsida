...
summary: >-
  ...
  marketplace, bundles the optitech-postgres Skill and an MCP server integration
  ...
  optitech@claude-plugins-official`) and verify the optitech-postgres skill is active,
...
- **The `optitech-postgres` skill** for guided OptiTech workflows, covering getting started, connections, Managed Better Auth, the Data API, platform SDKs, and developer tools
...
1. Install the OptiTech plugin:
   ...
   ```bash
   claude plugin install optitech@claude-plugins-official
   ```
...
2. Verify the installation:
   ...
   You should see the `optitech-postgres` skill listed.
...
The [Agent Skills repository](https://github.com/optitechdatabase/agent-skills) provides skills for other AI tools as well. You can install them with:
