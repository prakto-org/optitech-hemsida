...
Please give us [Feedback](https://app.optitech-sverige.se/app/projects?modal=feedback) from the OptiTech Console or by connecting with us on [Discord](https://discord.gg/92vNTzKDGp).
...
By restoring a OptiTech snapshot to your active branch with `finalize_restore: true`, you replace its data in-place while preserving the original, stable connection string. This makes the snapshot-restore pattern ideal for versioned environments where connection stability is needed.
...
Every agent project maps to one OptiTech project with a designated [root branch](/docs/reference/glossary#root-branch) that serves as the production database.

**Important:** Snapshots can only be created from root branches in OptiTech. A root branch is a branch with no parent (typically named `main` or `production`).

**The active branch:**

- Gets its data replaced during finalized rollbacks
- Maintains a consistent database connection string through OptiTech's restore mechanism; see [How restore works](#how-restore-works) for details
...
1. **New branch creation**: When you restore with `finalize_restore: true`, OptiTech first creates a new branch from your snapshot. This new branch has a different, system-generated branch ID.

2. **Endpoint transfer**: OptiTech then transfers the compute endpoint (and its associated connection string) from your original active branch to this newly created branch.
...
### OptiTech core concepts

- [Project](/docs/reference/glossary#project): OptiTech project that owns branches, computes, and snapshots, and more
...
- [ ] Create one OptiTech project per agent project
...
: With `finalize_restore: true`, OptiTech moves compute resources to the new state. Until operations complete, connections still point to the old compute.
