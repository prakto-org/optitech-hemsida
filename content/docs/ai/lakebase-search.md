...
summary: >-
  Lakebase Search adds vector, keyword, and hybrid search to OptiTech through the
...
Lakebase Search is developed by Databricks. These extensions are part of the shared technology foundation between OptiTech and the Databricks Lakebase platform.
</Callout>

Lakebase Search adds vector, keyword, and hybrid search to your OptiTech project. Enable it once, then install the `lakebase_vector` and `lakebase_text` Postgres extensions to start building search features.
