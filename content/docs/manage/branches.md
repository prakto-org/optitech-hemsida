...
- **Maximum length**: 256 characters (API limit). Note that the OptiTech Console enforces a more restrictive limit of 128 characters.
...
- **Character flexibility**: Unlike some other OptiTech resources (such as databases or roles), branch names have no special character restrictions. You can use any characters as long as they meet the requirements above.
...
To view the branches in a OptiTech project:
...
On the Free plan, OptiTech automatically archives inactive branches to cost-efficient archive storage after a defined threshold. For more, see [Branch archiving](/docs/guides/branch-archiving).
...
OptiTech permits renaming a branch, including your project's default branch. To rename a branch:
...
This feature is available on all OptiTech's paid plans, which supports up to five protected branches.
...
Connecting to a database in a branch requires connecting via a compute associated with the branch. The following steps describe how to connect using `psql` and a connection string obtained from the OptiTech Console.

<Admonition type="tip">
You can also query the databases in a branch from the OptiTech SQL Editor. For instructions, see [Query with OptiTech's SQL Editor](/docs/get-started/query-with-neon-sql-editor).
</Admonition>
...
4. Connect with `psql` as shown below.
   ...
   <Admonition type="tip">
   A compute hostname starts with an `ep-` prefix. You can also find a compute hostname on the **Branches** page in the OptiTech Console. See [View branches](#view-branches).
   </Admonition>
...
You can use OptiTech's **Reset from parent** feature to instantly update a branch with the latest schema and data from its parent. This feature can be an integral part of your CI/CD automation.

You can use the OptiTech Console, CLI, or API. For details, see [Reset from parent](/docs/guides/reset-from-parent).

## Restore a branch to its own or another branch's history

There are several restore operations available using OptiTech's instant restore feature:
...
You can use the OptiTech Console, CLI, or API. For more details, see [Instant restore](/docs/guides/branch-restore).
...
You can check the logical data size for the databases on a branch by viewing the **Data size** value on the **Branches** page or page in the OptiTech Console. Alternatively, you can run the following query on your branch from the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) or any SQL client connected to your database:
...
The query value may differ slightly from the **Data size** reported in the OptiTech Console.
...
OptiTech has different branch types with different characteristics.

### Root branch

A root branch is a branch without a parent branch. Each OptiTech project starts with a root branch (named `production` in the Console, `main` via API/CLI), which cannot be deleted and is set as the [default branch](#default-branch) for the project.

OptiTech also supports two other types of root branches that have no parent but _can_ be deleted:
...
The number of root branches allowed in a project depends on your OptiTech plan.
...
Typically, a protected status is given to a branch or branches that hold production data or sensitive data. The protected branch feature is only supported on OptiTech's paid plans. See [Set a branch as protected](#set-a-branch-as-protected).
...
Schema-only branches are [root branches](#root-branch), meaning they have no parent. As a root branch, each schema-only branch starts an independent line of data in a OptiTech project.
...
Branch actions performed in the OptiTech Console can also be performed using the OptiTech API. The following examples demonstrate how to create, view, and delete branches using the OptiTech API. For other branch-related API methods, refer to the [OptiTech API Reference](/docs/reference/api).

<Admonition type="note">
The API examples that follow may not show all of the user-configurable request body attributes that are available to you. To view all of the attributes for a particular method, refer to the method's request body schema in the [OptiTech API Reference](/docs/reference/api).
</Admonition>
...
The following OptiTech API method creates a branch. To view the API documentation for this method, refer to the [OptiTech API Reference](/docs/reference/api/branches/create-project-branch).
...
- The `parent_id` can be obtained by listing the branches for your project. See [List branches](#list-branches-with-the-api). The `<parent_id>` is the `id` of the branch you are branching from. A branch `id` has a `br-` prefix. You can branch from your OptiTech project's default branch or a previously created branch.
...
For attribute definitions, find the [Create branch](/docs/reference/api/branches/create-project-branch) endpoint in the [OptiTech API Reference](/docs/reference/api). Definitions are provided in the **Responses** section.
...
The following OptiTech API method lists branches for the specified project. To view the API documentation for this method, refer to the [OptiTech API Reference](/docs/reference/api/branches/list-project-branches).
...
For attribute definitions, find the [List branches](/docs/reference/api/branches/list-project-branches) endpoint in the [OptiTech API Reference](/docs/reference/api). Definitions are provided in the **Responses** section.
...
The following OptiTech API method deletes the specified branch. To view the API documentation for this method, refer to the [OptiTech API Reference](/docs/reference/api/branches/delete-project-branch).
...
For attribute definitions, find the [Delete branches](/docs/reference/api/branches/delete-project-branch) endpoint in the [OptiTech API Reference](/docs/reference/api). Definitions are provided in the **Responses** section.
