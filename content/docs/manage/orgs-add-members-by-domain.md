...
summary: >-
  ...
  across multiple OptiTech organizations.
...
- You add a domain (for example, `example.com`) on the organization **People** page under **Auto-join by domain**. The domain is added in a "Pending verification" state and OptiTech generates a verification code.
...
- When a user signs up or logs in to OptiTech with an email whose domain matches one of your verified domains (for example, `alex@example.com`), they are automatically added to your organization as a Member. They do not receive an invite email; they simply see the organization in their org switcher.
...
In the OptiTech Console, switch to the organization you want to manage, then open the **People** page. Scroll to the **Auto-join by domain** section (below **Pending invites**). The section shows a short description and a **Domain** field with an **Add domain** button.
...
In your DNS provider's console, add a TXT record at your domain with the exact value shown in the OptiTech Console (copy it using the icon next to the value). Use the root or host you manage for the domain. Do not add a subdomain prefix to the value itself; paste the single string as the TXT record content.
...
After the TXT record has propagated, click **Verify** in the OptiTech Console (in the verification section for that domain). If the record matches, the domain status changes to **Verified**. Users who sign up or log in with an email at that domain will then be automatically added to the organization as members.

<Admonition type="tip" title="After verification">
You can delete the TXT record from your domain after verification. OptiTech only uses it to confirm ownership; the record does not need to remain in place.
</Admonition>
...
- **Sign up and log in:** Users are automatically added both when they create a new OptiTech account and when an existing user logs in. So if you add and verify a domain after some colleagues already have OptiTech accounts, those colleagues will be added to the org the next time they log in (as long as their email domain matches).
