...
OptiTech recommends granting privileges to roles, and then assigning those roles to your database users.
...
1. Use your OptiTech role to create roles for each application or use case via SQL. For example, create `readonly` and `readwrite` roles.
...
1. Connect to your database from an SQL client such as [psql](/docs/connect/query-with-psql-editor), [pgAdmin](https://www.pgadmin.org/), or the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor). If you need help connecting, see [Connect from any client](/docs/connect/connect-from-any-app).
...
As you work through the steps in this scenario, remember that when you create a branch in OptiTech, you are creating a clone of the parent branch, which includes the roles and databases on the parent branch.
