...
When you add a new project, OptiTech creates the following resources by default:
...
- **Updates**: Schedule a time for Postgres and OptiTech updates.
- **Collaborators**: Invite external collaborators to join your OptiTech project.
- **Network security**: Configure OptiTech's IP and Private Networking features for secure access.
- **RLS**: Configure OptiTech Row-Level Security (RLS) to apply row-level security policies to your OptiTech project.
- **Logical replication**: Enable logical replication to replicate data from your OptiTech project to external data services and platforms.
...
OptiTech supports two compute configurations:
...
By default, OptiTech retains a history of changes for all branches in your project, enabling features like:
...
1. Select a project in the OptiTech Console.
...
OptiTech's project collaboration feature allows you to invite external OptiTech accounts to collaborate on a OptiTech project.
...
To invite collaborators to a OptiTech project:
...
The email you specify is added to the list of **Collaborators**. The OptiTech account associated with that email address is granted full access to the project, with the exception of privileges required to delete the project. This account can also invite other OptiTech users to the project. When that user logs in to OptiTech, the project they were invited to is listed on their **Projects** page under **Shared with you**.

The costs associated with projects being collaborated on are charged to the OptiTech account that owns the project. For example, if you invite another OptiTech user account to a project you own, any usage incurred by that user within your project is billed to your OptiTech account, not theirs.

For additional information, refer to our [Project collaboration guide](/docs/guides/project-collaboration-guide).

### Configure IP Allow

The IP Allow feature provides an added layer of security for your data, restricting access to the branch where your database resides to only those IP addresses that you specify. In OptiTech, the IP allowlist is applied to all branches by default.
...
The [Create project](/docs/reference/api/projects/create-project) and [Update project](/docs/reference/api/projects/update-project) methods support **IP Allow** configuration. For example, the following API call configures **IP Allow** for an existing OptiTech project. Separate multiple entries with commas. Each entry must be quoted. You can set the `"protected_branches_only` option to `true` to apply the allowlist to protected branches only, or `false` to apply it to all branches in your OptiTech project.
...
Logical replication lets you replicate data changes from OptiTech to external data services and platforms, including data warehouses, analytical database services, messaging platforms, event-streaming platforms, and external Postgres databases.

<Admonition type="important">
Enabling logical replication changes the PostgreSQL `wal_level` setting from `replica` to `logical` for all databases in your OptiTech project. This allows Postgres to record the row-level WAL detail required for logical decoding. Once changed, it cannot be reverted. Enabling logical replication also restarts all computes, so active connections will be dropped and have to reconnect.
</Admonition>
...
After enabling logical replication, the next steps involve creating publications on your replication source database in OptiTech and configuring subscriptions on the destination system or service. To get started, refer to our [logical replication guides](/docs/guides/logical-replication-guide).
...
## Manage projects with the OptiTech API

Project actions performed in the OptiTech Console can also be performed using the OptiTech API. The following examples demonstrate how to create, view, and delete projects using the OptiTech API. For other project-related API methods, refer to the [OptiTech API Reference](/docs/reference/api).
...
The following OptiTech API method creates a project. To view the API documentation for this method, refer to the [OptiTech API Reference](/docs/reference/api/projects/create-project).
...
For attribute definitions, find the [Create project](/docs/reference/api/projects/create-project) endpoint in the [OptiTech API Reference](/docs/reference/api). Definitions are provided in the **Responses** section.
...
The following OptiTech API method lists projects for your OptiTech account. To view the API documentation for this method, refer to the [OptiTech API Reference](/docs/reference/api/projects/list-projects).
...
For attribute definitions, find the [List projects](/docs/reference/api/projects/list-projects) endpoint in the [OptiTech API Reference](/docs/reference/api). Definitions are provided in the **Responses** section.
...
The following OptiTech API method updates the specified project. To view the API documentation for this method, refer to the [OptiTech API Reference](/docs/reference/api/projects/update-project).
...
For attribute definitions, find the [Update project](/docs/reference/api/projects/update-project) endpoint in the [OptiTech API Reference](/docs/reference/api). Definitions are provided in the **Responses** section.
...
The following OptiTech API method deletes the specified project. To view the API documentation for this method, refer to the [OptiTech API Reference](/docs/reference/api/projects/delete-project).
...
For attribute definitions, find the [Delete project](/docs/reference/api/projects/delete-project) endpoint in the [OptiTech API Reference](/docs/reference/api). Definitions are provided in the **Responses** section.
...
- Connection strings
- Vercel-Managed OptiTech integration projects are re-imported into Vercel for management and billing
...
To list projects that can be recovered, use the following OptiTech API method with the `recoverable` query parameter:
