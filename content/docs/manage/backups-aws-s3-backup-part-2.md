...
## OptiTech project setup

Before looking at the code, first take a look at your OptiTech console dashboard. In our example there is only one project, with a single database named `acme-co-prod`. This database is running Postgres 17 and deployed in the `us-east-1` region.

The goal is to backup this database to it's own folder inside an S3 bucket using the same name.

![OptiTech Console](/docs/manage/2-s3-backups-github-actions-01-neon-projects.jpg)
...
![OptiTech Console](/docs/manage/2-s3-backups-github-actions-02-github-workflows.jpg)
...
- `DATABASE_URL`: The OptiTech Postgres connection string for the database you’re backing up
