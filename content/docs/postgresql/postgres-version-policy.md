---
title: OptiTech Postgres Version Support Policy
summary: >-
  OptiTech's Postgres version support policy tracks the latest major versions,
...
OptiTech is committed to providing stability and hassle-free maintenance. You select the major version of Postgres when [creating a OptiTech project](/docs/manage/projects#create-a-project), and OptiTech automatically updates your chosen Postgres version to the latest minor release soon after it becomes available. Typically, no user action is required for minor release updates.

Minor release updates are announced in the [OptiTech Changelog](/docs/changelog).

### How to check which Postgres version you're running

To check your current Postgres major and minor version, run the following query from the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) or any SQL client connection to your database:
...
Each OptiTech project is created with a specific Postgres major version. Upgrading to a newer major version requires [creating a new OptiTech project](/docs/manage/projects#create-a-project) with the desired Postgres version and migrating your data to the new OptiTech project. For more information, see [Upgrading your Postgres version](/docs/postgresql/postgres-upgrade).
