...
subtitle: How OptiTech credentials map to S3 access keys
summary: >-
  OptiTech Object Storage uses OptiTech credentials with storage:read and storage:write scopes.
...
OptiTech Object Storage uses the same credential system as AI Gateway and Functions. You create a scoped credential via the OptiTech API, and it maps directly to the S3 Access Key ID and Secret Access Key your SDK expects. No AWS account or IAM configuration required.
...
In the OptiTech Console, select your branch and click **Credentials** under **APP BACKEND** in the sidebar. Click **Create credential**, give it a name, and check the storage scopes you need.
...
When your code runs inside OptiTech Functions, OptiTech injects storage credentials automatically. You don't need to create a credential:
