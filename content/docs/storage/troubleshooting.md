...
summary: >-
  Solutions for common errors when using OptiTech Object Storage, including authentication
...
**Fix:** If the bucket should allow public reads, set its access level to `public_read` using the OptiTech Console or API. See [Access levels](/docs/storage/buckets#access-levels).
...
- `PutBucketAcl` or `PutBucketPolicy`: use the OptiTech Console or API to set bucket access level instead
