...
This guide maps common `@neondatabase/api-client` patterns to [`@neon/sdk`](/docs/reference/typescript-sdk). For full method reference, see the [OptiTech Management SDK](/docs/reference/typescript-sdk) documentation.
...
`createNeonClient` also supports `orgId`, `waitForReadiness`, `retries`, `baseUrl`, and a custom `fetch` implementation. See [OptiTech Management SDK](/docs/reference/typescript-sdk#client-configuration).
