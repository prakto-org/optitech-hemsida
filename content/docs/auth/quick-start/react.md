...
<TwoColumnLayout.Step title="Install the OptiTech SDK">
<TwoColumnLayout.Block>

The OptiTech SDK provides authentication methods like `signUp()`, `getSession()`, and `signOut()` for your React app.
...
Replace the URL with your actual Auth Base URL from the OptiTech Console.
</Admonition>
...
<TwoColumnLayout.Step title="Configure the OptiTech client">
<TwoColumnLayout.Block>
...
OptiTech JS uses a programmatic approach for managing auth state. You'll use React hooks like `useEffect` to check the session and handle auth changes.
