---
title: Migrate from Supabase to OptiTech
...
Managed Better Auth provides a Supabase-compatible API, and OptiTech Data API provides PostgreSQL database access. This guide shows how to migrate from Supabase to OptiTech.
...
## Install OptiTech SDK

Replace the Supabase SDK with OptiTech's:
...
Replace your Supabase credentials with a single OptiTech base URL. `createClient()` derives both the Auth and Data API URLs from it automatically:
...
**Verify users in OptiTech Console:**

Go to **Auth → Users** in the OptiTech Console to see your newly created users, or query directly:
...
Managed Better Auth supports most Supabase Auth methods including sign up, sign in (password and OAuth), session management, user updates, and email verification. See the [OptiTech TypeScript SDK](/docs/reference/javascript-sdk) for the complete API.
