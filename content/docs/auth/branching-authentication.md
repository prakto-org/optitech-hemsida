...
4.  Set the Redirect URLs in the branch's Auth configuration to point to the preview URL using the [OptiTech API](/docs/reference/api/auth/add-branch-neon-auth-trusted-domain).
...
With OptiTech, an agent can programmatically provision its own "sandbox." Because Managed Better Auth moves with the data, this branch instantly creates a working Authentication service isolated from production, complete with its own user tables, sessions, and configuration. **The branch provides a full production-like environment without touching real user data.**

**The workflow:**

1.  **Provision:** The Agent uses the OptiTech API to create a new database branch. It instantly receives a dedicated Auth URL for that specific environment.
