...
summary: >-
  ...
  OptiTech SDK without manual plugin configuration, letting server-side code create
...
Managed Better Auth is built on [Better Auth](https://www.better-auth.com/) and provides support for Admin plugin APIs through the OptiTech SDK. You do not need to manually install or configure the Better Auth Admin plugin.
...
- An existing user with an **admin** role to call Admin APIs.
  ...
  You can assign the **admin** role to a user through the OptiTech Console. Navigate to **Auth** → **Users**, open the three‑dot menu next to the user, and select **Make admin**.

  ![Assign admin role in OptiTech Console](/docs/auth/make-admin.png)

## Use Admin with SDK methods

You can call Admin plugin methods using the OptiTech SDK auth client.
