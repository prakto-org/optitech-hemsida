...
summary: >-
  ...
  Better Auth, letting each tenant share a OptiTech branch with role-based access
  ...
  and per-branch enable/disable control via the OptiTech Console or API. The plugin
...
- Workspaces or groups that share a OptiTech branch (same database)
...
Open your project in the OptiTech Console, then go to **Auth** > **Plugins** (beta) and use the **Organizations** section (per branch). This tab is available when your project uses Managed Better Auth with **Better Auth**; other Auth settings, including your **Auth URL**, stay on the **Configuration** tab. From there you can customize:
...
You can also configure the plugin via the [OptiTech API](/docs/reference/api). Use your API key in the `Authorization` header.
