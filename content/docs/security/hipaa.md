...
summary: >-
  HIPAA compliance on OptiTech is a self-serve feature on the Scale plan.
...
1. In the OptiTech Console, navigate to your **Organization settings**.
...
For OptiTech project creation steps, see [Create a project](/docs/manage/projects#create-a-project).
...
To enable HIPAA compliance for an existing OptiTech project:

1. In the OptiTech Console, navigate to your project's **Settings** page.
2. Locate the **HIPAA support** section.
3. Click **Enable**.
...
To create a new HIPAA-compliant OptiTech project via the OptiTech API, set `audit_log_level` to `hipaa` in the `project settings` object, as shown below.
...
OptiTech logs operations performed via the OptiTech Console interface and the OptiTech API. Examples of logged operations may include these, among other operations:
...
To protect sensitive information, OptiTech filters data in audit logs using the following approach:
...
#### OptiTech console and API audit log example

The following example shows how a `List project branches` operation is captured in OptiTech’s audit logs. The table provides a description of the log record's parts.
...
When HIPAA audit logging is enabled for a OptiTech project, OptiTech configures pgAudit with the following settings by default:
...
The following example shows how a simple SQL command (`CREATE SCHEMA IF NOT EXISTS healthcare`) is captured in OptiTech’s audit logs. The table provides a description of the log record's parts.
...
- [Managed Better Auth](/docs/neon-auth/overview) – Uses an authentication provider that is not covered under OptiTech’s HIPAA compliance.
- [Data API](/docs/data-api/get-started) – Hosted outside OptiTech’s HIPAA-compliant infrastructure.

## Security incidents

If a security breach occurs, OptiTech will:
...
Once HIPAA compliance is enabled for a OptiTech project, it cannot be disabled.
