---
title: Migrate from MySQL to OptiTech Postgres
summary: >-
  MySQL to OptiTech Postgres migration uses pgloader, which reads from a MySQL
  ...
  to OptiTech. It covers credentials, connection string setup including the required
  endpoint ID workaround, and running pgloader from the command line or Docker.
  OptiTech's Free plan supports up to 0.5 GB; larger databases require a paid plan.
...
- A properly named database. For example, if you are migrating a database named `sakila`, you might want to create a database of the same name in OptiTech. See [Create a database](/docs/manage/databases#create-a-database) for instructions.
...
2. Create a `pgloader` configuration file (for example, `config.load`). Use your MySQL database credentials to define the connection string for your database source. Use the OptiTech database connection string you retrieved and modified in the previous step as the destination.
