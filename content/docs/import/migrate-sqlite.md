...
summary: >-
  ...
  MySQL, CSV, or logical replication imports. OptiTech's Free plan supports up to
...
`pgloader` is an open-source data loading and migration tool that efficiently transfers data from various sources (like CSV, MySQL, SQLite, MS SQL, etc.) into Postgres, handling schema and data transformations on the fly. We'll use it to migrate a sample SQLite database to OptiTech Postgres
...
Now that you have your OptiTech database and SQLite database ready, you can use `pgloader` to migrate the data. Follow these steps:
...
Connect to your OptiTech database using [`psql`](/docs/connect/query-with-psql-editor) or [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) and check the next value for the `books` table's sequence:
