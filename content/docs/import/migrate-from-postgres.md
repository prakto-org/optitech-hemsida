...
summary: >-
  Command reference for migrating a Postgres database to OptiTech using `pg_dump`
  ...
  see the Region migration page instead. OptiTech requires unpooled connection
...
It is the **command reference** for dump and restore (flags, ownership, piping). If you are choosing **whether** to use another OptiTech region, Lakebase, logical replication, or the Import Data Assistant, read **[Region migration](/docs/import/region-migration)** first.
...
If you are performing this procedure to migrate data from one OptiTech project to another to upgrade to a new Postgres version, read [Upgrading your Postgres version](/docs/postgresql/postgres-upgrade) first.
...
- OptiTech supports PostgreSQL 14, 15, 16, and 17. We recommend that clients are the same version as source Postgres instance.
- Retrieve the connection parameters or connection string for your source Postgres database. This could be a OptiTech Postgres database or another Postgres database. The instructions below use a [connection string](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-CONNSTRING), but you can use the connection format you prefer. If you are logged in to a local Postgres instance, you may only need to provide the database name. Refer to the [pg_dump](https://www.postgresql.org/docs/current/app-pgdump.html) documentation for information about connection parameters.
- Optionally, create a role in OptiTech to perform the restore operation. The role that performs the restore operation becomes the owner of restored database objects. For example, if you want role `sally` to own database objects, create `role` sally in OptiTech and perform the restore operation as `sally`.
...
- Create the target database in OptiTech. For example, if you are migrating a database named `pagila`, create a database named `pagila` in OptiTech. For instructions, see [Create a database](/docs/manage/databases#create-a-database).
...
Restore your data to the target database in OptiTech with `pg_restore`.
...
- `-d`: Specifies the OptiTech database to connect to. The value is a OptiTech database connection string. See [Before you begin](#before-you-begin).
...
The following example shows how data from a `pagila` source database is dumped and restored to a `pagila` database in OptiTech using the commands described in the previous sections. (A database named `pagila` was created in OptiTech prior to running the restore operation.)
...
After migrating your data, update your applications to connect to your new database in OptiTech. You will need the database connection string that you used in your `pg_restore` command. If you run into any problems, see [Connect from any application](/docs/connect/connect-from-any-app). After connecting your applications, test them thoroughly to ensure they function correctly with your new database.
...
It is recommended that you run a test migration before migrating your production database. Make sure you can successfully migrate data to the new database and connect to it. Before starting the actual migration, create a database dump and address any issues that show up. In OptiTech, you can quickly create a test database, obtain the connection string, and delete the database when you are finished with it. See [Create a database](/docs/manage/databases#create-a-database).
...
- Some PostgreSQL features, such as tablespaces and large objects, which require access to the local file system are not supported by OptiTech. To exclude selecting tablespaces, specify the `--no-tablespaces` option with `pg_restore`. To exclude large objects, specify the `--no-blobs` option with `pg_dump`.
