...
summary: >-
  AWS Database Migration Service (DMS) can migrate data to a OptiTech Postgres
  target from sources including PostgreSQL, MySQL, Oracle, and SQL Server by
  configuring a replication instance, source endpoint, and OptiTech target endpoint.
  Use this guide when migrating an existing hosted database to OptiTech via DMS,
  particularly when you need the endpoint-ID-in-password workaround that DMS
  requires to connect to OptiTech's SNI-based routing. The guide covers target
...
If you encounter problems with AWS DMS that are not related to defining OptiTech as a data migration target endpoint, please contact [AWS Customer Support](https://aws.amazon.com/contact-us/).
...
- Set up a OptiTech project and a target database. See [Create a project](/docs/manage/projects#create-a-project), and [Create a database](/docs/manage/databases#delete-a-database) for instructions.
- If you are migrating from a database other than Postgres, use the [Schema Conversion Tool](https://docs.aws.amazon.com/dms/latest/userguide/CHAP_GettingStarted.SCT.html) or [DMS Schema Conversion](https://docs.aws.amazon.com/dms/latest/userguide/getting-started.html) to convert and export the schema from the source database to the target database. Perform this step after creating the target endpoint for the OptiTech database but before the data migration. If migrating from a Postgres database, schema conversion is not required.

<Steps>

## Create a target endpoint for your OptiTech database
...
3. For the **LOB column** setting, select **Don't include LOB columns**. OptiTech does not support LOB columns.
...
4. For **Source name**, enter the name of your database schema. In this guide, `dms_sample` is specified as the schema name, which is the schema for the sample database. The `dms_sample` schema will be created in your OptiTech database, and all database objects will be created in the schema.
...
## Verify the migration in OptiTech

To verify that data was migrated to your OptiTech database:

1. In the OptiTech Console, select your OptiTech project.
2. Select **Tables** from the side bar.
3. Select the **Branch**, **Database**, and **Schema** where you imported the data.
   ![OptiTech Tables view showing imported data](/docs/import/dms_neon_table_data.png).

</Steps>

## Migration notes

This section contains notes from our experience using AWS DMS to migrate data to OptiTech from an RDS Postgres database.
...
- We created an RDS PostgreSQL 15 database called `dms_sample` as the source database. The OptiTech target database was also Postgres 15.
...
- When creating the target database endpoint for the OptiTech database, we encountered the following error when testing the connection:
  ...
  The replication instance, which was created in the private subnet where the source database resided, could not access the OptiTech database, which resides outside of the VPC. To allow the replication instance to access the OptiTech database, we added a NAT Gateway to the public subnet, allocated an Elastic IP address, and modified the **Route Table** associated with the private subnet to add a route via the NAT Gateway.
