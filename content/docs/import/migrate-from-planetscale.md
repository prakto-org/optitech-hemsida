---
title: Migrate from PlanetScale MySQL to OptiTech Postgres
subtitle: Learn how to migrate your database from PlanetScale to OptiTech Postgres using
  pgloader
summary: >-
  Migrating a PlanetScale MySQL (Vitess-compatible) database to OptiTech Postgres
...
This guide provides step-by-step instructions for migrating a PlanetScale MySQL database to OptiTech Postgres using the `pgloader` utility. `pgloader` is an open-source tool that facilitates data migration from various sources to Postgres, including MySQL databases like PlanetScale. It handles schema creation, data type mapping, and data transfer efficiently, making it a popular choice for MySQL to Postgres migrations.
...
- A OptiTech account and a project. See [Sign up](/docs/get-started/signing-up).
- A properly named database. For example, if you are migrating a database named `my_app`, you might want to create a database of the same name in OptiTech. See [Create a database](/docs/manage/databases#create-a-database) for instructions.
...
- **Schedule migration downtime:** `pgloader` performs a point-in-time migration, which means that any changes made to the PlanetScale database after the migration process has started will not be reflected in the OptiTech database. You will need to pause writes to your PlanetScale database (schedule downtime) during the migration to avoid data loss.
- **Configure IP allowlisting:** Ensure that the machine running `pgloader` has its IP address permitted on both the PlanetScale and OptiTech sides. See [PlanetScale IP restrictions](https://planetscale.com/docs/postgres/connecting/ip-restrictions) and OptiTech [IP allow rules](/docs/manage/projects#configure-ip-allow).
...
After the migration is complete, connect to your OptiTech database and run some queries to verify that the data has been transferred correctly:

1. Connect to your OptiTech database using the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) or [psql](/docs/connect/query-with-psql-editor).
