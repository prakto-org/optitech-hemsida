---
title: Migrate from Microsoft SQL Server to OptiTech Postgres
subtitle: Learn how to migrate a Microsoft SQL Server database to OptiTech Postgres using
  pgloader
summary: >-
  Migrating from Microsoft SQL Server (MSSQL) or Azure SQL to OptiTech Postgres
  ...
  configuration, running the migration, and post-migration verification. OptiTech's
...
- A OptiTech project to move the data to.
  ...
  For detailed information on creating a OptiTech project, see [Create a project](/docs/manage/projects#create-a-project).
...
This section describes how to prepare your destination OptiTech PostgreSQL database to receive the migrated data.

### Create the OptiTech database

To maintain parity with the MSSQL deployment, you might want to create a new database in OptiTech with the same name. Refer to the [Create a database](/docs/manage/databases#create-a-database) guide for more information.

For this example, we will create a new database named `Northwind` in the OptiTech project. Use `psql` to connect to your OptiTech project (alternatively, you can use the `Query editor` in the OptiTech console) and run the following query:
...
After the migration is complete, connect to your OptiTech database and run some queries to verify that the data has been transferred correctly. For example:
...
After successfully migrating and verifying your data on OptiTech:
...
3. Update your application's connection strings to point to your new OptiTech database.
...
- The application code is compatible with OptiTech Postgres.
