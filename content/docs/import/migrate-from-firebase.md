---
title: Migrate from Firebase Firestore to OptiTech Postgres
subtitle: Learn how to migrate your data from Firebase Firestore to OptiTech Postgres using
  a custom Python script
summary: >-
  Firebase Firestore to OptiTech Postgres migration uses two custom Python scripts
...
We'll use a custom Python script to export data from Firestore to a local file, and then import the data into OptiTech Postgres. This approach allows us to handle Firestore's document-based structure and convert it into the relational database format suitable for Postgres.
...
- Python 3.10 or later installed on your local machine. Additionally, add the following packages to your Python virtual environment: `firebase_admin`, which is Google's python SDK for Firebase and `psycopg`, which is used to connect to OptiTech Postgres database.
...
We use another python script to import the firestore data we previously downloaded into OptiTech.
...
After running both the Firestore export and the OptiTech import scripts, you should verify that your data has been successfully migrated:
