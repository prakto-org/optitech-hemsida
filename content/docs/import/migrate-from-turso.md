---
title: Migrate from Turso to OptiTech Postgres
summary: >-
  Migration guide for moving a Turso (libSQL/SQLite) database to OptiTech Postgres
  using pgloader. Dumps the Turso database to a local SQLite file with the
  Turso CLI, then loads it into OptiTech where pgloader converts SQLite type
  affinities to Postgres types. Use this guide when the source is Turso or
  SQLite; it also covers swapping Turso drivers for the OptiTech serverless driver,
  translating `?` placeholders to `$1` parameters, adapting dialect differences
  like LIKE vs ILIKE, and verifying sequences after migration. OptiTech's Free plan
...
`pgloader` is an open-source data loading and migration tool that efficiently transfers data from various sources (like CSV, MySQL, SQLite, MS SQL, etc.) into Postgres, handling schema and data transformations on the fly. Since Turso databases are SQLite-compatible, you can dump them to a local SQLite file and then use `pgloader` to migrate that file to OptiTech Postgres.

## Prerequisites

Before you begin, ensure you have the following:

- A OptiTech account and a project. If you don't have one, see [Sign up](/docs/get-started/signing-up).
- A database created in your OptiTech project. For instructions, see [Create a database](/docs/manage/databases#create-a-database).
...
- OptiTech's Free plan supports 0.5 GB of data. If your data size is more than 0.5 GB, you'll need to upgrade to one of OptiTech's paid plans. See [OptiTech plans](/docs/introduction/plans) for more information.
...
pgloader will automatically create the necessary tables and indexes in your OptiTech Postgres database, and transfer all data from the `turso_export.db` file. The summary report at the end confirms that the migration completed successfully without errors.
...
The `reset sequences` option in the load file ensures that auto-incrementing columns start from the correct value. Connect to your OptiTech database using [`psql`](/docs/connect/query-with-psql-editor) or the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) and check the next value for a table's sequence:
...
Compare row counts between your Turso source and OptiTech destination:
...
After migrating your database schema and data to OptiTech, you must update your application code to connect to OptiTech and execute queries using a Postgres driver instead of the Turso SQLite client.

### Connection changes

First, replace your Turso database client (such as `@tursodatabase/serverless` or `@libsql/client`) with a Postgres-compatible driver. For serverless or edge environments, OptiTech's [serverless driver](/docs/serverless/serverless-driver) is a great choice.
...
You need to transition from SQLite's parameter binding (often using `?` placeholders) and driver-specific methods (`.run()`, `.all()`, `.get()`) to standard Postgres driver syntax. For example, with the OptiTech serverless driver, you can use tagged template literals for queries, while with `node-postgres`, you use parameterized queries with `$1`, `$2`, etc.
...
- [Migrating data to OptiTech](/docs/import/migrate-intro)
