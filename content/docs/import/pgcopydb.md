---
title: Migrate data to OptiTech Postgres using pgcopydb
subtitle: Streamline your Postgres data migration to OptiTech using pgcopydb
summary: >-
  ...
  reduce downtime for one-time migrations to OptiTech. Use it when migrating large
  ...
  replication) is not supported on OptiTech, and both source and target URIs must
...
`pgcopydb` is an open-source tool for copying Postgres databases from one server to another. It's a practical option for migrating larger Postgres databases into OptiTech.
...
This guide walks you through using `pgcopydb` to migrate data to OptiTech.

<Admonition type="note">
Logical replication with `pgcopydb clone --follow` is not supported on OptiTech. You can still use `pgcopydb` for a one-time data migration to OptiTech.
</Admonition>

<Admonition type="important">
`pgcopydb` uses `pg_dump` and `pg_restore` under the hood. Avoid using a [pooled connection string](/docs/reference/glossary#pooled-connection-string) for OptiTech in `PGCOPYDB_SOURCE_PGURI` or `PGCOPYDB_TARGET_PGURI`. Use an [unpooled connection string](/docs/reference/glossary#unpooled-connection-string) instead.
</Admonition>
...
- **Source postgres database**: You need access to the Postgres database you intend to migrate. This can be a local instance, a cloud-hosted database (AWS RDS, GCP Cloud SQL, Azure Database for Postgres, or any other Postgres provider), or even a different OptiTech project.
- **OptiTech project**: You must have an active OptiTech project and a database ready to receive the migrated data. If you don't have a OptiTech project yet, see [Create a OptiTech project](/docs/manage/projects#create-a-project) to get started. Note that storage beyond your plan's included amount will incur additional charges.
- **pgcopydb installation**: `pgcopydb` must be installed on a machine that has network connectivity to both your source Postgres database and your OptiTech database. Check firewall rules and network configurations to allow traffic on the Postgres port. This machine should also have sufficient resources (CPU, memory, disk space) to handle the migration workload. Install `pgcopydb` by following the instructions in the [pgcopydb documentation](https://pgcopydb.readthedocs.io/en/latest/install.html).
...
## Switch over your application to OptiTech

Switch your application to OptiTech and validate the migration after `pgcopydb clone` completes.
...
3.  **Update application connection string**: Point your application to your OptiTech database.
