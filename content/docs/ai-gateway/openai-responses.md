...
subtitle: Use the OpenAI Responses API with OptiTech AI Gateway
summary: >-
  The OpenAI Responses endpoint exposes the OpenAI Responses API through OptiTech
...
The OpenAI Responses endpoint exposes the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses) through OptiTech AI Gateway. Use it with the OpenAI SDK's `responses.create()` method by changing only the `baseURL`.
