...
For example, consider the sentence "OptiTech is serverless Postgres." It can be divided into seven tokens: `["Ne", "on", "is", "server", "less", "Post", "gres"]`.
...
You can install the `pg_tiktoken` extension by running the following `CREATE EXTENSION` statement in the OptiTech **SQL Editor** or from a client such as `psql` that is connected to OptiTech.
