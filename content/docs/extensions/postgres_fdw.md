...
This guide will walk you through the essentials of using the `postgres_fdw` extension in OptiTech. You'll learn how to enable the extension, establish connections to remote Postgres servers, define foreign tables that map to tables on those servers, and execute queries that span across your OptiTech database and remote instances. We will also cover important considerations for performance and security when working with `postgres_fdw`.

<Admonition type="note">
`postgres_fdw` is a core Postgres extension that can be installed on any OptiTech project using the instructions below. It provides a standardized way to access external Postgres databases and is widely used for data integration and cross-database querying.
</Admonition>

**Version availability:**

Please refer to the [list of all extensions](/docs/extensions/pg-extensions) available in OptiTech for up-to-date extension version information.

## Enable the `postgres_fdw` extension

You can enable the extension by running the following `CREATE EXTENSION` statement in the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) or from a client such as [psql](/docs/connect/query-with-psql-editor) that is connected to your OptiTech database.
...
- **User mapping:** Defines the authentication credentials used to connect to the foreign server. This maps a local OptiTech user to a user on the remote server.
- **Foreign table:** A locally defined object in your OptiTech database that represents a table located on the foreign server. Queries against the foreign table are transparently executed on the remote server.
...
When setting up `postgres_fdw` with a OptiTech database as the foreign server, make sure to use the hostname from an [unpooled connection string](/docs/reference/glossary#unpooled-connection-string). Pooled connection strings will result in connection errors. You can find the unpooled connection string in your project dashboard by clicking the **Connect** button and ensuring the **Connection pooling** toggle is disabled.
</Admonition>
...
For example, to map the current OptiTech user to the `read_only_user` user on the `production_db` server:
...
Once the connection to the remote server is established, you need to define foreign tables in your OptiTech database that correspond to the tables you want to access on the remote server. There are two primary ways to do this: creating foreign tables manually or importing the schema.

### Create foreign tables manually

The `CREATE FOREIGN TABLE` command allows you to explicitly define the structure of a remote table in your OptiTech database. You need to specify the column names and data types, which must match the remote table's schema.
...
- `<local_table_name>`: The name you want to give the foreign table in your OptiTech database.
...
- `<local_schema>`: The schema in your OptiTech database where the foreign tables will be created. If the schema doesn't exist, you'll need to create it first.
...
- **Materialized Views:** For frequently accessed data from foreign tables, consider creating materialized views in your OptiTech database. Materialized views store a snapshot of the remote data locally, which can significantly improve query performance. You can refresh materialized views periodically to keep the data relatively up-to-date.
...
- **Network security:** Ensure that network access is properly configured to allow connections between your OptiTech project and the remote Postgres server. Firewalls and security groups might need adjustments.
...
The `postgres_fdw` extension is a valuable tool for OptiTech users who need to access and integrate data from remote Postgres databases. By establishing connections to foreign servers, defining foreign tables, and executing queries that span across local and remote databases, you can build applications that query data from multiple sources.
