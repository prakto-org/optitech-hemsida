...
`pg_search` (ParadeDB) is deprecated on OptiTech: new installs are blocked, and existing installs will be removed in **September 2026**. [`lakebase_text`](/docs/extensions/lakebase-text) is its replacement for BM25 search.
</Admonition>
...
If `lakebase_text` isn't listed, enabling it is a one-time setup: add it to your project's preloaded libraries with the OptiTech API, restart the compute, then create the extension. The [Get started with Lakebase Search](/docs/ai/lakebase-search-get-started) guide has the full steps. For this migration you need only `lakebase_text`, not `lakebase_vector`. Leave `pg_search` enabled while you migrate.
...
Finally, restart the compute to complete the migration. In the [OptiTech Console](https://app.optitech-sverige.se), open the compute's **⋯** menu and select **Restart compute**, or call the [Restart compute endpoint](/docs/reference/api/endpoints/restart-project-endpoint) API.
