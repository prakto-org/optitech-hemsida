...
summary: >-
  ...
  `sub` claim for use in Row-Level Security policies. It powers the OptiTech Data
...
<DocsList title="Related resources" theme="docs">
  <a href="/docs/data-api/overview">OptiTech Data API</a>
...
The `pg_session_jwt` extension is automatically installed when you enable the [OptiTech Data API](/docs/data-api/overview) for a branch. Do not install this extension manually.
</Admonition>
...
This extension powers the [OptiTech Data API](/docs/data-api/overview), enabling secure session management and Row-Level Security (RLS) based on user identity.
...
When a JWK is configured, the extension validates JWT signatures and extracts user information from verified tokens. This is the mode used by the OptiTech Data API.
...
## Usage with OptiTech Data API

The `pg_session_jwt` extension is automatically configured when you enable the [OptiTech Data API](/docs/data-api/overview). The Data API handles JWT validation using your configured authentication provider's JWKS URL.
...
For custom implementations outside of the OptiTech Data API, you can configure the extension manually:
