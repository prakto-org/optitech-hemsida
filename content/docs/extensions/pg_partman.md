...
This guide covers how to set up `pg_partman` with OptiTech: why partitioning helps, how to enable the extension, how to create partitioned tables, and how to automate partition maintenance.

<Admonition type="note">
`pg_partman` is an open-source Postgres extension that can be installed in any OptiTech project using the instructions below. Detailed installation instructions and compatibility information can be found in the [pg_partman](https://github.com/pgpartman/pg_partman) documentation.
</Admonition>

## Enable the `pg_partman` extension

You can enable the extension by running the following `CREATE EXTENSION` statement in the OptiTech **SQL Editor** or from a client such as `psql` that is connected to OptiTech. Creating a `partman` schema is optional (but recommended) and you can name the schema whatever you like, but it cannot be changed after installation.
