...
summary: >-
  wal2json is a Postgres logical decoding output plugin, pre-installed in OptiTech,
  ...
  enabling logical replication in OptiTech, REPLICA IDENTITY settings for tables
...
This guide describes the `wal2json` plugin &#8212; how to enable it in OptiTech, configure its output, and use it to capture and process database changes in JSON format. WAL decoding is crucial for building robust data pipelines, implementing Change Data Capture (CDC) systems, and maintaining data consistency across distributed systems.

<Admonition type="note">
    The `wal2json` plugin is included in your OptiTech project and doesn't require a separate installation.
</Admonition>

**Version availability:**

The `wal2json` plugin is available in all Postgres versions supported by OptiTech. For the most up-to-date information on supported versions, please refer to the [list of all extensions](/docs/extensions/pg-extensions) available in OptiTech.

## Enable logical replication

Before using the `wal2json` plugin, you need to enable logical replication for your OptiTech project. Navigate to the **Settings** page in your OptiTech Project Dashboard, and select **Beta** from the list of options. Click **Enable** to enable logical replication.

<Admonition type="note">
Once enabled for a project, logical replication cannot be reverted. This action triggers a restart of all active compute endpoints in your OptiTech project. Any active connections will be dropped and have to reconnect.
</Admonition>
...
- [Manage logical replication in OptiTech - Decoder plugins](/docs/guides/logical-replication-manage#decoder-plugins)
