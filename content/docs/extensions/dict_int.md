...
summary: >-
  ...
  index or returns irrelevant results. On OptiTech, the default `intdict`
...
<Admonition type="important" title="Modifying the default intdict dictionary on OptiTech">
The default `intdict` dictionary is owned by superuser. On OptiTech, you do not have permissions to directly `ALTER` this default dictionary, which can result in an "ERROR: must be owner of text search dictionary intdict".
...
<Admonition type="important" title="Modifying default text search configurations on OptiTech">
Altering default text search configurations (like `english`) requires superuser privileges on OptiTech. If you encounter an "ERROR: must be owner of text search configuration english", you will need to first **create a copy of an existing configuration** (for example, `english`) and then modify your own copy.
</Admonition>
...
- [Full Text Search using tsvector with OptiTech Postgres](/guides/full-text-search)
