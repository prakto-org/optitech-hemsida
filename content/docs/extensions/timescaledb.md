...
summary: >-
  ...
  Choose this page to install `timescaledb` on OptiTech, convert tables to
  ...
  supported on OptiTech; compression is not available.
...
This guide provides an introduction to the `timescaledb` extension. You’ll learn how to enable the extension in OptiTech, create hypertables, run simple queries, and analyze data using `timescaledb` functions. Finally, you’ll see how to delete data to free up space.

<Admonition type="note">
`timescaledb` is an open-source extension for Postgres that can be installed on any OptiTech Project using the instructions below.
</Admonition>
...
You can enable the extension by running the following `CREATE EXTENSION` statement in the OptiTech **SQL Editor** or from a client such as `psql` that is connected to OptiTech.
...
You can use the [sample weather dataset from TimescaleDB](https://assets.timescale.com/docs/downloads/weather_small.tar.gz) and load it into your OptiTech database using [psql](/docs/connect/query-with-psql-editor).
...
Load the data into OptiTech database - enter the username, password, host and database name. You can find these details by clicking the **Connect** button on your **Project Dashboard** to open the **Connect to your database** modal.
...
You may run into space concerns as data accumulates in timescaledb hypertables. While OptiTech's Postgres service does not support compression, deleting old data is an option if you don't need to hold on to it for long periods of time.
...
<Admonition type="note">
Please be aware that OptiTech's [Scale to Zero](/docs/guides/scale-to-zero-guide) feature may affect the running of scheduled jobs. It may be necessary to start the compute before running a job.
</Admonition>
...
You were able to configure the timescaledb extension in OptiTech and create a hypertable to store `weather` data. Then you executed simple queries and analyzed data using a combination of standard SQL and `timescaledb` functions before finally using `drop_chunks()` to delete data.
