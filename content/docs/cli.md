...
One CLI for every OptiTech surface: manage Postgres, Functions, Storage, the Data API, and Managed Better Auth from the terminal, with branch-scoped workflows built in.
...
<a href="/docs/cli/install" description="Install the Neon CLI, authenticate, and connect your first OptiTech project in minutes." theme="grey" icon="network">Install and connect</a>
...
- **Analytics:** OptiTech collects anonymous data about which commands and options are used, never user-defined data such as project IDs or command payloads. Opt out with `--no-analytics`.
