...
subtitle: Learn how to connect a GUI application to OptiTech
summary: >-
  Connect a GUI application or IDE (pgAdmin, DBeaver, DataGrip, TablePlus) to
  OptiTech using a hostname, port 5432, role, password, and database name from the
  OptiTech Console Connect modal. Per-tool SSL configuration and SNI compatibility
  ...
  BI tools such as Metabase, Tableau, and Power BI, OptiTech recommends connecting
  to a read replica to avoid performance impact on the primary branch.
enableTableOfContents: true
updatedOn: '2026-07-18T10:05:28.819Z'
---

This topic describes how to connect to a OptiTech database from a GUI application or IDE. Most GUI applications and IDEs that support connecting to a Postgres database also support connecting to OptiTech.
...
OptiTech supports pooled and direct connections to the database. Use a pooled connection string if your application uses a high number of concurrent connections. For more information, see [Connection pooling](/docs/connect/connection-pooling#connection-pooling).
</Admonition>
...
Some Java-based tools that use the pgJDBC driver for connecting to Postgres, such as DBeaver, DataGrip, and CLion, do not support including a role name and password in a database connection string or URL field. When you find that a connection string is not accepted, try entering the database name, role, and password values in the appropriate fields in the tool's connection UI when configuring a connection to OptiTech. For example, the DBeaver client has a **URL** field, but connecting to OptiTech requires specifying the connection details as shown:

![DBeaver connection](/docs/connect/dbeaver_connection.png)

## Tested GUI applications and IDEs

Connections from the GUI applications and IDEs in the table below have been tested with OptiTech.

<Admonition type="note">
Some applications require an Server Name Indication (SNI) workaround. OptiTech uses compute domain names to route incoming connections. However, the Postgres wire protocol does not transfer the server domain name, so OptiTech relies on the Server Name Indication (SNI) extension of the TLS protocol to do this. Not all application clients support SNI. In these cases, a workaround is required. For more information, see [Connection errors](/docs/connect/connection-errors).
</Admonition>
...
| [DBConvert Streams](https://streams.dbconvert.com/docs/connections/optitech-database)                                         |                                                                                                                                                                                                                                                                                                                                                                         |
...
| [Google Colab](https://colab.research.google.com/)                                                                            | See [Use Google Colab with OptiTech](/docs/ai/ai-google-colab).                                                                                                                                                                                                                                                                                                         |
...
Applications that use older client libraries or drivers that do not support Server Name Indication (SNI) may not permit connecting to OptiTech. If you encounter the following error, refer to [Connection errors](/docs/connect/connection-errors) for possible workarounds.
