...
Each Postgres connection creates a new process in the operating system, consuming memory and CPU resources. Postgres limits the number of connections based on available RAM. In OptiTech, this limit is defined by `max_connections`, which varies by compute size:
...
<Admonition type="note">
Seven connections are reserved for the OptiTech superuser account. For a 0.25 CU compute, this means 97 connections are available for your application (104 total - 7 reserved).
</Admonition>
...
You can also view active connections on the [Monitoring page](/docs/introduction/monitoring-page#connections) in the OptiTech Console, which provides a visual graph of connection usage over time.

## How OptiTech's connection pooling works
...
**An important thing to understand about OptiTech's connection pooling:**
...
Each pool has a maximum size defined by `default_pool_size`. In OptiTech, this is configured as:
...
OptiTech uses PgBouncer in transaction mode (`pool_mode=transaction`), which means connections are returned to the pool after each transaction completes. This limits some Postgres features that require persistent sessions:
...
## OptiTech PgBouncer configuration

OptiTech's PgBouncer configuration:
...
The [Monitoring page](/docs/introduction/monitoring-page) in the OptiTech Console provides two dedicated graphs for tracking PgBouncer connection pool activity:
