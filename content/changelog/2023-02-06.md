...
- Integrations: You can now integrate OptiTech with your Vercel project directly from your Vercel account. The new integration, which is currently in Beta, is available from the [Vercel Integration Marketplace](https://vercel.com/integrations/optitech). The integration allows you to connect your Vercel project to a new or existing OptiTech project. It also enables creating a database branch for each Vercel preview deployment and for your Vercel development environment. For more information about the OptiTech-Vercel integration, see [Connect Vercel and OptiTech](/docs/guides/vercel-overview).
- Control Plane: You can now enable connection pooling in OptiTech for individual connections. Pooling is enabled by adding a `-pooler` suffix to the endpoint ID in the OptiTech hostname. For example:
...
- Console: Added validation to ensure that emails are provided in the expected format when creating a OptiTech account.
