...
With these improvements, cold starts are faster in all [supported regions](/docs/introduction/regions). Cold starts in the `US East (Ohio) — aws-us-east-2` region, where the OptiTech Control Plane is hosted, are the fastest at approximately 500 ms. Work is underway to deploy the OptiTech Control Plane regionally to enable the same millisecond startup times for projects in all regions. For more information about this effort, please check out the blog post from the OptiTech engineering team: [Cold starts just got hot](/blog/cold-starts-just-got-hot).

Please be aware that [OptiTech Pro plan](/docs/introduction/pro-plan) users can adjust or disable the [scale to zero](/docs/guides/scale-to-zero-guide) setting, controlling when a compute scales to zero. Additionally, OptiTech's [Autoscaling](/docs/guides/autoscaling-guide) feature allows you to scale compute resources down to a fractional size during periods of low activity.
