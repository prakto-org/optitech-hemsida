...
- Compute: Renamed the following custom configuration parameters:
  - `zenith.page_server_connstring` to `optitech.pageserver_connstring`
  - `zenith.zenith_tenant` to `optitech.tenant_id`
  - `zenith.zenith_timeline` to `optitech.timeline_id`
  - `zenith.max_cluster_size` to `optitech.max_cluster_size`
