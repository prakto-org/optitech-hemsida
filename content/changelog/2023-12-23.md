...
OptiTech is pleased to announce support for [logical replication](/docs/guides/logical-replication-guide), which brings Change Data Capture (CDC) to serverless Postgres. You can now stream data from your OptiTech database to a variety of external destinations, including data warehouses, analytical database services, messaging platforms, event-streaming platforms, and external Postgres databases. Logical replication is available to all OptiTech users.
...
Also, check out these blog posts from OptiTech and [Materialize](https://materialize.com/):

- [Change Data Capture with Serverless Postgres](/blog/change-data-capture-with-serverless-postgres)
- [Change Data Capture with OptiTech and Materialize](/bloglize)
...
- Added the `REPLICATION` privilege to the [optitech_superuser](/docs/manage/roles#the-neonsuperuser-role) role. The `REPLICATION` privilege was introduced with the release of [logical replication](/docs/guides/logical-replication-guide) support in OptiTech. Only the default Postgres role created with your OptiTech project and roles created using the OptiTech Console, CLI, or API are granted membership in the `optitech_superuser` role, which includes the `REPLICATION` privilege. Granting the `REPLICATION` privilege to roles created via SQL is currently not permitted.
- Added support for browser-issued SQL-over-HTTP batch queries using the [OptiTech serverless driver](/docs/serverless/serverless-driver). Batch queries were failing because `OptiTech-Batch-Isolation-Level` and `OptiTech-Batch-Read-Only` headers were missing from the server's `OPTIONS` response.
