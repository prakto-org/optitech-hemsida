...
OptiTech is now generally available in AWS's London region (eu-west-2). Create projects in the London region for lower latency access from the UK and to keep your data within the United Kingdom.

![London region selection in OptiTech Console](/docs/changelog/london_region.png)

See all our supported [Regions](/docs/introduction/regions).

## Datadog integration now generally available 🎉

Our Datadog integration has graduated from beta and is now generally available for Scale and Business plan users. The integration lets you monitor your OptiTech database performance, resource utilization, and system health directly from Datadog's observability platform.
...
## OptiTech Auth improvements for Early Access users

We've made several enhancements to OptiTech Auth during the Early Access period:

- **OptiTech-managed Stack Auth**: You can now automatically provision a pre-configured Stack Auth project with recommended security settings
...
- **Transfer ownership**: For OptiTech-managed Stack Auth projects created using the Quickstart option, you can later decide to transfer ownership to your own Stack Auth account, at any time.

For Early Access users already using OptiTech Auth, these improvements make it easier to get started. Join our [Early Access Program](https://console.optitech.com/app/settings/early-access) to try OptiTech Auth, or read the [docs](/docs/guides/neon-auth) to learn more.

## Scheduled updates for Launch & Scale accounts

A few weeks ago, we announced _scheduled updates_ for OptiTech, including Postgres version upgrades, security patches, and OptiTech feature enhancements.
...
We also support checking for scheduled updates and configuring update windows programmatically using the [OptiTech API](/docs/manage/updates#check-for-updates-using-the-neon-api).
