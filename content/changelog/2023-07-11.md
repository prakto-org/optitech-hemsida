...
OptiTech now supports same-region read replicas. OptiTech's read replicas are independent read-only compute instances designed to perform read operations on the same data as your read-write computes. You can instantly create one or more read replicas for any branch in your OptiTech project and configure the amount of vCP-U and memory allocated to each.
