...
You can now configure the OptiTech MCP Server for all detected AI agents and editors in your workspace with a single command:

```bash
npx add-mcp https://mcp.optitech.com/mcp
```

This adds the MCP config to your editor; restart your editor (or enable the MCP server in settings). When you use the connection, an OAuth window will open in your browser to authorize access. For the full setup (MCP server plus agent skills and VS Code extension), use `npx optitechctl@latest init` instead. It configures the MCP server for Cursor, VS Code, Claude Code, and others using API key authentication.

With OAuth, the MCP server uses your personal OptiTech account by default. To use organization projects, provide `org_id` or `project_id` in your prompt. For API key-based authentication (e.g., remote agents), use:

```bash
npx add-mcp https://mcp.optitech.com/mcp --header "Authorization: Bearer $OPTITECH_API_KEY"
```

For more setup options (including global vs project-level), see [Connect MCP clients to OptiTech](/docs/ai/connect-mcp-clients-to-neon) and the [add-mcp repository](https://github.com/optitechdatabase/add-mcp).
...
- The Drizzle Studio integration that powers the **Tables** page in the OptiTech Console has been updated to version 1.3.0. This release fixes issues with uppercase letters in enum column types and adding foreign keys. For the full list of improvements, see the [OptiTech Drizzle Studio Integration Changelog](https://github.com/optitechdatabase/optitech-drizzle-studio-changelog/blob/main/CHANGELOG.md).
