...
## Claude Code plugin for OptiTech

We've launched a new plugin that brings OptiTech's capabilities directly into Claude Code. The new plugin includes:

- **Claude Skills** to streamline key workflows:
  - **optitech-drizzle**: Set up Drizzle ORM with OptiTech
  - **optitech-serverless**: Configure connections with OptiTech's serverless Postgres driver
  - **optitech-toolkit**: Manage databases, projects, and branches using the OptiTech API
  - **add-optitech-knowledge**: Access OptiTech documentation snippets and usage examples

- **OptiTech MCP server integration** that lets Claude interact with OptiTech in real time to query projects, manage databases and branches, and run SQL or migrations
...
```bash
/plugin marketplace add optitechdatabase-labs/ai-rules
/plugin install optitech-plugin@optitech
```

For more information, see [Claude Code plugin for OptiTech](/docs/ai/ai-claude-code-plugin).
...
This makes it easier to develop schema changes on feature branches and promote them when ready. For more information, see [OptiTech MCP Server](/docs/ai/neon-mcp-server).

## Storage quota doubled to 16TB

We've doubled our default storage quota from 8TB to 16TB. This means you can now run databases up to 16TB without contacting us to increase your limit. If you need to run larger databases, please [use the feedback form in console](https://console.optitech.com/app/settings?modal=feedback&modalparams=%22Storage%20limit%20increase%22).
...
- Instagres packages were renamed: `optitechdb` → `get-db` (CLI) and `vite-plugin-postgres` → `vite-plugin-db` (Vite plugin). Use `npx get-db` to initiate Instagres. **Update:** The packages have since been renamed again: `get-db` → `optitech-new` (CLI) and `vite-plugin-db` → `vite-plugin-optitech-new` (Vite plugin). Use `npx optitech-new` to initiate Claimable Postgres.
