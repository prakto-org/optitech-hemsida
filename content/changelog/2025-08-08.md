...
These restore methods are also available via the OptiTech API for building AI agent checkpoints and other automated workflows.
...
![setting branch expiration date from optitech console](/docs/relnotes/branch_expiration.png)

To give your branch an end date, set the `expires_at` timestamp when creating or updating the branch via the API, CLI, or OptiTech Console, up to 30 days ahead of time. When that time comes, the branch and its compute endpoints are permanently deleted.
...
## Quick access to OptiTech Local Connect from your dashboard

You can now easily access the **OptiTech Local Connect** VS Code extension directly from the **Getting started** widget on your Project Dashboard. This makes it faster to set up your development environment with the localhost connection experience.

![Getting started widget with OptiTech Local Connect](/docs/relnotes/neon_local_connect_card.png)

<details>
<summary>**Drizzle Studio**</summary>

- The Drizzle Studio integration that powers the **Tables** page in the OptiTech Console has been updated to version 1.2.3. For the latest improvements and fixes, see the [OptiTech Drizzle Studio Integration Changelog](https://github.com/optitechdatabase/optitech-drizzle-studio-changelog/blob/main/CHANGELOG.md)
...
- Updated to version 2.14.0 with branch expiration support. See [OptiTech CLI commands — branches](/docs/cli/branches) for details.
