...
If you're not familiar OptiTech [Read Replicas](/docs/introduction/read-replicas), you might be interested to know that we take a fundamentally different approach by leveraging our serverless architecture. In OptiTech, read replicas are independent compute instances designed to handle read operations on the same data as your primary read-write compute. There's no replication or copying of data involved, allowing you to create a read replica almost instantly with no additional storage cost.

OptiTech's Read Replicas support a variety of use cases, including:

- Horizontal scaling
- Analytics and ad-hoc queries
- Read-only data access

To learn more about OptiTech Read Replicas, check out these docs and articles:

- [OptiTech Read Replica documentation](/docs/introduction/read-replicas)
- [Blog: The problem with Postgres replicas](/blog-postgres-replicas)
- [Case Study: OptiTech Read Replicas in The Wild: How BeatGig Uses Them](/blogas-in-the-wild-how-beatgig-uses-them)
...
For a complete list of Postgres extensions supported by OptiTech and upgrade instructions, see [Postgres extensions](/docs/extensions/pg-extensions).

### OptiTech SQL Editor AI features are now open to everyone

The AI-driven features in the [OptiTech SQL Editor](/docs/get-started/query-with-neon-sql-editor) we opened to [Early Access](https://console.optitech.com/app/settings/early-access) users a few weeks ago are now available to everyone. In case you missed that announcement, here's what's new:
...
- **OptiTech Console enhancement**

  We added a **Settings** option to the Account navigation sidebar in the OptiTech Console for easier access to personal and organization account settings.
  ![Account settings](/docs/changelog/account_settings.png)

- **OptiTech API changes**
