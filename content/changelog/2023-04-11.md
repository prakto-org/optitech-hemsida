...
- Pageserver: Added `disk_size` and `instance_type` properties to the Pageserver API. This data is required to support assigning OptiTech projects to Pageservers based on Pageserver disk usage.
- Proxy: Added error reporting for unusually low `proxy_io_bytes_per_client metric` values.
- Proxy: Added support for additional domain names to enable partner integrations with OptiTech.
...
Added a **Storage** widget to the OptiTech **Dashboard** that shows current project storage size, number of branches, and data size for the default branch. These metrics previously appeared in the **Usage** widget.
![Storage widget](/docs/changelog/storage_widget.png)

### Postgres extension support

Added support for the [pg_stat_statements](https://www.postgresql.org/docs/current/pgstatstatements.html) extension, which provides tracking of plan and execution statistics for SQL statements. For more information about Postgres extensions supported by OptiTech, see [Postgres extensions](/docs/extensions/pg-extensions/).

## Fixes & improvements

- API: Added `consumption_period_start` and `consumption_period_end` properties to the [Get project details](/docs/reference/api/projects/get-project) API. These properties provide the date and time when OptiTech starts and stops measuring usage for the current billing period.
...
- Control Plane: Added an option to allow the OptiTech Control Plane to provide a compute node configuration specification in an API call after a compute node is started.
  Integrations: Added information to the OptiTech Vercel Integration UI to indicate that OptiTech securely stores passwords. OptiTech stores a database user's password in a secure storage vault associated with the OptiTech project.
...
- UI: The **Usage** widget in the OptiTech Dashboard now shows the start date for the project usage metrics shown in the widget.
- UI: Added a link to the **Pro** badge that appears in the OptiTech Console for users that have upgraded to OptiTech's Pro plan. Clicking the badge directs you to the **Billing** page in the OptiTech Console.
- UI: Added a documentation link and text to the **Roles** page in the OptiTech Console explaining the purpose of roles.
...
- UI: Added missing `endpoint_id` to the connection examples accessible from the **Connection Details** widget on the OptiTech Dashboard.
