...
### Database and role management via SQL
    ...
    OptiTech now supports database and role management via SQL. You can now manage databases and roles from the [OptiTech SQL Editor](/docs/get-started-with-neon/query-with-neon-sql-editor) or an SQL client, such as [psql](/docs/connect/query-with-psql-editor). Previously, databases and roles could only be managed in the OptiTech Console.
    ...
    Additionally, roles created in the OptiTech Console, CLI, and API are now automatically granted membership in a `optitech_superuser` role. This role defines the privileges required to perform tasks in OptiTech such as creating databases, roles, and extensions. To learn more, see [The optitech_superuser role](/docs/manage/roles#the-neonsuperuser-role).

### Improved experience for Prisma Migrate users

    Users of Prisma Migrate no longer need to manually create a shadow database in OptiTech.
    ...
    Previously, running `prisma migrate dev` without manually creating and configuring a shadow database in OptiTech would return the following error:
    ...
    The reason for this error was that it was not possible in OptiTech to create and delete databases via SQL. To work around this issue, you had to manually create a shadow database in OptiTech and specify the connection string of that database in your `schema.prisma` file using the `shadowDatabaseUrl` variable. For example:
