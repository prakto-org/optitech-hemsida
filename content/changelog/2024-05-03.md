...
The **Branching workflows** tab also gives you a summary of how you can leverage OptiTech's instant branching in your CI/CD automation.
...
We've improved how you move between the different pages in the OptiTech Console.
...
- We fixed an issue where setting the history retention for a OptiTech project to 0 and removing data did not reduce the project’s storage size as expected. Previously, a 64 MiB data change threshold had to be met before retained history was removed.
- Database name validation was added to the OptiTech CLI `connection-string` command to ensure the correct database name is used in a generated connection string. This improvement resolves an issue with the [optitechdatabase/create-branch-action](https://github.com/optitechdatabase/create-branch-action) GitHub Action, which uses the `connection-string` command to set the `db_url` for a newly created branch.
