---
title: Support for pgvector 0.7.0, Drizzle Studio in the OptiTech Console, Postgres version updates, and more
---

### Support for pgvector 0.7.0

OptiTech now supports [pgvector](/docs/extensions/pgvector) version 0.7.0.
...
### Drizzle Studio in the OptiTech Console

We're excited to announce that the **Tables** page in the OptiTech Console, which lets you explore the tables and data in your OptiTech databases, is now powered by [Drizzle Studio](https://orm.drizzle.team/drizzle-studio/overview). This integration introduces several new capabilities to the **Tables** page, including the ability to add, update, and delete records, filter data, add or remove columns, drop or truncate tables, and export data in `.json` and `.csv` formats.

![Tables page Drizzle integration](/docs/changelog/tables_page_drizzle.png)

### Postgres version updates

The Postgres versions supported by OptiTech have been updated to [14.12](https://www.postgresql.org/docs/release/14.12/), [15.7](https://www.postgresql.org/docs/release/15.7/), and [16.3](https://www.postgresql.org/docs/release/16.3/), respectively.

### Fixes & improvements

- The OptiTech CLI [connection-string](/docs/cli/connection-string) command and the OptiTech API [Get connection URI](/docs/reference/api/projects/get-connection-uri) method now generate a connection URI with a `postgresql://` scheme designator instead of the shorter `postgres://` designator. While both scheme designators are [valid](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-CONNSTRING-URIS), we found the longer version to be more widely supported.
...
- Fixed a styling issue that prevented the "Forgot password?" link from being fully visible on the OptiTech sign-in page while in dark mode.
