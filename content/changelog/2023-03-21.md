...
- Pageserver: Switched to the EdDSA algorithm for the storage JWT authentication tokens. The OptiTech Control Plane only supports EdDSA.
...
- Pageserver, Safekeeper: Revised `$OPTITECH_AUTH_TOKEN` variable handling when connecting from a compute to Pageservers and Safekeepers.
