...
- Compute: Moved the backpressure throttling algorithm to the OptiTech extension to minimize changes to the OptiTech Postgres core code, and added a `backpressure_throttling_time` function that returns the total time spent throttling since the system was started.
- Compute: Added support for the `h3_pg` and `plv8` Postgres extensions. For information about Postgres extensions supported by OptiTech, see [Postgres extensions](/docs/extensions/pg-extensions).
