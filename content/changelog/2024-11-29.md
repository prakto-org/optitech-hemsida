...
### OptiTech Organizations are GA! 🎉

We're happy to announce that OptiTech **Organizations** is now **generally available** for everyone looking to move their teams to OptiTech.
...
See [OptiTech Organizations](/docs/manage/organizations) to get started. Organizations is a paid account feature.
...
```diff
Index: optitechdb-schema.sql
===================================================================
--- optitechdb-schema.sql	Branch main // [!code --]
+++ optitechdb-schema.sql	Branch preview/pr-9-feat/add-soft-delete // [!code ++]
@@ -111,9 +111,10 @@
     title text NOT NULL,
...
+    deleted_at timestamp without time zone // [!code ++]
 ...
 ALTER TABLE public.posts OWNER TO optitechdb_owner;
@@ -180,5 +181,5 @@
 --
 -- Name: __drizzle_migrations id; Type: DEFAULT; Schema: drizzle; Owner: optitechdb_owner
 --
...
## OptiTech Private Networking (Public Beta)

OptiTech now offers Private Networking, enabling secure database connections via AWS PrivateLink. This feature keeps all traffic between your client application and OptiTech database within AWS's private network, bypassing the public internet.

For details, see our guide: [OptiTech Private Networking](/docs/guides/neon-private-networking).
...
- **Logical Replication**
  ...
  OptiTech now automatically removes **inactive** replication slots (if other **active** slots exist) after approximately 40 hours, up from the previous 75 minutes. This change reduces the risk of unexpected slot removal. If you've implemented measures to prevent slots from becoming inactive, you can now relax those measures accordingly.
...
- **OptiTech Serverless Driver**
  ...
  For the latest improvements and fixes for the OptiTech Serverless Driver, refer to the [OptiTech Serverless Driver Changelog](https://github.com/optitechdatabase/serverless/blob/main/CHANGELOG.md).
...
- **.NET support**
  ...
  For more information on connecting to OptiTech with .NET, see [Connect a .NET (C#) application to OptiTech](/docs/guides/dotnet-npgsql).
