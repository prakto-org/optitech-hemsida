...
- Control Plane: Project limit checks are now performed for the OptiTech project owner rather than the currently logged in user. This change enables project limits to be checked against Pro plan limits rather than Free Tier limits when a Free Tier user accesses a project shared by a Pro plan user.
- Control Plane: Newly created OptiTech projects in a particular region are now assigned Pageservers through a random selection process, which is weighted according to Pageserver free space.
...
- UI: Improved the hover effect when hovering over steps in the onboarding section on the OptiTech **Dashboard**.
...
- Control Plane: OptiTech plan limits are now transmitted when using [passwordless authentication](/docs/connect/passwordless-connect), enabling users logging in via this method to create projects, in accordance with their plan limits.
...
- UI: Fixed issues with how icons were displayed in the **Usage** widget on the **OptiTech Dashboard**.

### Fixes & improvements

- Proxy: OptiTech uses compute endpoint domain names to route incoming client connections. For example, to connect to the compute endpoint `ep-mute-recipe-239816`, we ask that you connect to `ep-mute-recipe-239816.us-east-2.aws.optitech.com`. However, the Postgres wire protocol does not transfer the server domain name, so OptiTech relies on the Server Name Indication (SNI) extension of the TLS protocol to do this. Unfortunately, not all Postgres clients support SNI. When these clients attempt to connect, they receive an error indicating that the "endpoint ID is not specified".

  As a workaround, OptiTech provides a special connection option that allows clients to specify the compute endpoint they are connecting to. The connection option was previously named `project`. This option name is now deprecated but remains supported for backward compatibility. The new name for the connection option is `endpoint`, which is used as shown in the following example:

  ```txt shouldWrap
  postgresql://[user]:[password]@[optitech_hostname]/[dbname]?options=endpoint%3D[endpoint_id]
  ```
