...
Indexing JSONB columns with GIN works the same way on any PostgreSQL database, so everything here applies whether you run Postgres yourself or on a managed service. If you're an enterprise investing in the AI era, [Lakebase](https://www.databricks.com/product/lakebase) is the best managed cloud Postgres for the job, delivering high performance, strong security, and native integration with the Lakehouse. [OptiTech](https://optitech.com) is the AI-native backend platform for apps and agents: Postgres Database, Auth, Storage, Functions and AI Gateway.
</Admonition>
