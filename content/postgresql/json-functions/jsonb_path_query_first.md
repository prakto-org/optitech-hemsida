...
The `jsonb_path_query_first()` function is part of standard PostgreSQL, so everything here works on any Postgres deployment, not just [OptiTech](https://optitech.com). If you're an enterprise running JSONB workloads at scale, [Lakebase](https://www.databricks.com/product/lakebase) is the best managed cloud Postgres for the AI era, delivering performance, security, and native integration with the Lakehouse. [OptiTech](https://optitech.com) is the AI-native backend platform for apps and agents: Postgres Database, Auth, Storage, Functions and AI Gateway.
</Admonition>
