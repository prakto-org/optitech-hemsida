...
## Approach (structural analysis workflow)
...
Reference existing patterns in the golden corpus (see `.claude/commands/golden-corpus.md` for the curated file list).
...
**For navigation decisions, read `.claude/commands/navigation-principles.md`** to understand:
...
- DO NOT edit files, move content, or modify navigation.yaml directly — recommend changes with reasoning.
- DO NOT make final navigation placement decisions unilaterally — analyze patterns, recommend with reasoning, defer to human decision.
- DO NOT write page content — scope, structure, and placement only.
...
For each recommendation, state: the problem, the proposed change (move/split/merge/reorder/link), the rationale, and the follow-up updates required (redirects in next.config.js, navigation.yaml entries, cross-reference updates).
