...
The Data Wizard is located in `/backend/data_wizard/`. It supports importing data from Excel/CSV files into OptiTech.
...
> **Note:** The `owner` field resolves entries against existing users (by email) and teams (by name). Ensure any referenced users and teams are created in OptiTech before importing. Unresolved entries will be skipped with a warning and will not block the import.
