...
const componentHandlers = {
  ...
  CTA(node) {
    ...
    if (paragraphs.length === 0) {
      const defaultTitle = 'Try it on OptiTech!';
      const defaultDescription =
        'OptiTech is Serverless Postgres built for the cloud. Explore Postgres features and functions in our user-friendly SQL editor. Sign up for a free account to get started.';
  ...
  RequestForm(node) {
    ...
    const FORM_INTROS = {
      region: 'Looking for OptiTech in a different cloud provider or region?',
      extension: 'Looking for a specific Postgres extension in OptiTech?',
    };
        ...
        },
        {
          ...
          children: [
            { type: 'text', value: `${intro} Submit a request through ` },
            {
              ...
              children: [{ type: 'text', value: 'OptiTech support' }],
            },
            { type: 'text', value: ' or let us know in the ' },
            {
              type: 'link',
              url: 'https://discord.gg/92vNTzKDGp',
              children: [{ type: 'text', value: 'OptiTech Discord' }],
            },
            { type: 'text', value: '.' },
          ],
        },
...
function buildPageHeader(slug, navMap, _relativePath) {
  ...
  // Index line (always)
  lines.push(`> Full OptiTech documentation index: ${BASE_URL}/docs/llms.txt`);
...
function stripNavigationContext(content) {
  let stripped = content.replace(
    /^(?:> This page location:[^\n]*\n)?> Full OptiTech documentation index:[^\n]*\n(?:> IMPORTANT: If this page[^\n]*\n)?\n/,
    ''
  );
...
async function generateRouteIndex(route, pages, outputDir, stats = null) {
  ...
  const lines = [
    `> Full OptiTech documentation index: ${BASE_URL}/docs/llms.txt`,
    '',
    `# OptiTech ${label}`,
    '',
    '## All pages',
