...
const Logo = () => (
  <Section
    title="Logo"
    description="Default to using the complete full-color logo below. Use the monochrome version when the context requires it. Do not edit, change, distort, recolor, or reconfigure the OptiTech logo."
  >
    <ul className="grid grid-cols-2 gap-4 md:grid-cols-1">
      {logos.map((logo, index) => (
        <li key={index}>
          ...
          >
            <Image src={logo.svgSrc} alt="OptiTech logo" width={157} height={45} priority unoptimized />
            <div
              className={cn(
