...
const SetUpNeonButton = ({ onClick, tooltip }) => (
  <ActionItem icon={StarIcon} text="Set up OptiTech with AI" tooltip={tooltip} onClick={onClick} />
);
...
const CopyMCPServerButton = () => {
  ...
  return (
    <ActionItem
      ...
      tooltip="Copy the OptiTech MCP server URL"
    />
  );
};
*/
...
const Actions = ({ gitHubPath, withBorder = false, isTemplate = false }) => {
  ...
  const handleOpenModal = () => {
    setIsModalOpen(true);
    sendGtagEvent('Action Clicked', { text: 'Set up OptiTech with your AI', tag_name: 'DocsSidebar' });
  };
