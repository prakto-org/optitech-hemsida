...
const Plans = () => {
  const t = useTranslations('services.gapAnalysis.plans');
          ...
          <div className="col-span-7 col-start-6 grid grid-cols-2 gap-x-6 md:col-span-full md:col-start-auto md:grid-cols-1 md:gap-y-6">
            {cards.map((card, index) => (
              <div className="flex flex-col rounded-2xl bg-tc-lavender p-8 lg:p-6" key={card.name}>
                ...
                <Link
                  className="mt-8 inline-flex items-center gap-x-2 pt-2 text-sm font-semibold text-tc-purple transition-colors duration-200 hover:text-tc-purple-text"
                  to={CARD_LINKS[index]}
                >
                  {card.cta}
                  <ArrowIcon />
