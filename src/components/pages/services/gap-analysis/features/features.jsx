...
const DocsPanel = ({ mockup }) => (
  <div className="w-full max-w-[420px] rounded-2xl bg-white p-6 shadow-[0_24px_48px_-16px_rgba(23,17,46,0.25)] ring-1 ring-black-pure/5">
    ...
    <div className="mt-4 flex items-center justify-between rounded-xl bg-tc-cloud-light p-4 ring-1 ring-tc-border">
      ...
      <span className="bg-tc-purple px-4 py-1.5 text-xs font-medium text-white">
...
const Features = () => {
  const t = useTranslations('services.gapAnalysis.features');
    ...
    <section className="features bg-white py-24 safe-paddings lg:py-16 md:py-12">
      <Container size="1152">
        <div className="grid grid-cols-2 gap-x-16 lg:gap-x-10 md:block">
          <div>
            {items.map((item, index) => {
              ...
              return (
                <div
                  className="flex min-h-[62vh] flex-col justify-center py-10 md:min-h-0 md:py-8"
                  data-index={index}
                  key={item.title}
                  ref={(element) => {
        ...
        <div className="mt-10 flex justify-center md:mt-8">
          <Link
            className="inline-flex h-11 items-center gap-x-2.5 border border-tc-border-dark px-6 text-sm font-medium text-tc-ink transition-colors duration-200 hover:border-tc-ink"
