...
const Resources = () => {
  const t = useTranslations('services.gapAnalysis.resources');
        ...
        <ul className="mt-16 grid grid-cols-3 gap-x-8 lg:mt-12 lg:gap-x-6 md:mt-9 md:grid-cols-1 md:gap-y-9">
          {items.map((item, index) => (
            <li key={item.title}>
              ...
              <Link
                className="mt-4 inline-flex items-center gap-x-2 text-sm font-semibold text-tc-purple transition-colors duration-200 hover:text-tc-purple-text"
                to={LINKS.blog}
              >
                {t('readMore')}
                <ArrowIcon />
