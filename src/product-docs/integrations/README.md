# AI and Integrations

OptiTech is designed to plug into the rest of your tooling — through its REST API, an MCP server for AI assistants, and supported third-party integrations.

## Available now

- **[API reference](api.md)** — authentication, schema browsing, and curl examples.

## Coming soon

- Personal access tokens — how to issue and rotate them.
- MCP server — exposing OptiTech to AI agents and assistants.
- Third-party integrations — Jira, ServiceNow.
- Outgoing webhooks.
- The in-product chat assistant and agentic workflows.
