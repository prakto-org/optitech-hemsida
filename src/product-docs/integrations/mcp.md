---
description: >-
  This guide explains how to connect your AI assistant to OptiTech using
...
MCP (Model Context Protocol) allows AI assistants like Claude to interact with external tools and services. Think of it as giving your AI a set of capabilities to read and write data in OptiTech.

The OptiTech MCP server provides **90+ tools** covering:
...
2. **Network control** - All API calls to OptiTech go through your local machine. You have full visibility and control over network traffic, and can use your existing firewall rules and proxies.
...
5. **Works offline** - The MCP server itself runs locally. Only the actual OptiTech API calls require network access.

### Step 0: Get the MCP Server Code

The MCP server ships with the OptiTech CLI package. Download it to your machine first.

```bash
git clone <optitech-deployment-repo>
cd optitech/cli
```
...
You need a token to authenticate the MCP server with OptiTech:

1. Log in to OptiTech
...
Navigate to the `cli` folder in your OptiTech installation:

```bash
cd /path/to/optitech/cli
```
...
**Replace `/path/to/optitech/cli`** with your actual path.
...
> "What folders exist in OptiTech?"

If configured correctly, Claude will use the MCP tools to query your OptiTech instance.
...
> "List all risk assessments in OptiTech"
...
#### Step 2: Add the OptiTech server
...
* `/path/to/optitech/cli` with your actual cli folder path
...
* **OptiTech Documentation:** https://optitech-sverige.se/docs
* **Support:** [info@optitech-sverige.se](mailto:info@optitech-sverige.se)
...
| -------------------- | -------- | --------------------------- | ----------------------------------------- |
| `TOKEN`              | Yes      | -                           | Personal Access Token from OptiTech |
| `API_URL`            | No       | `http://localhost:8000/api` | OptiTech API endpoint               |
