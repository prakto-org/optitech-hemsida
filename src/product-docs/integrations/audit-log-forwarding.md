...
- **Tamper-evident retention.** Archive the audit trail outside the application, beyond OptiTech's own 90-day retention window.
...
- **Raw** — the native OptiTech shape (a flat pass-through of the log-entry fields).
...
```json
{
  ...
  "metadata": {
    "version": "1.8.0",
    "product": { "name": "OptiTech", "vendor_name": "OptiTech" },
...
- **SaaS.** Forwarding is pure egress to a destination you own — a SIEM collector URL or a Kafka broker you operate. OptiTech runs no per-tenant infrastructure for this.
