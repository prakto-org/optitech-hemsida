...
This guide details how to configure the bidirectional synchronization between **OptiTech** and **ServiceNow**. This integration allows you to:

* Automatically create ServiceNow records (e.g. GRC Controls) from OptiTech.
* Sync updates (Status, Priority, etc.) from OptiTech to ServiceNow.
* Receive real-time updates from ServiceNow back into OptiTech via Webhooks.

For now, you are only able to sync OptiTech's applied controls to a ServiceNow table.
...
First, configure OptiTech to connect to your ServiceNow instance.
...
3. **Discovery & Mapping:**
   ...
   * **Field Mapping:** Map OptiTech fields (Name, Description, Status) to the corresponding ServiceNow columns.
...
### Configure incoming sync in OptiTech
...
To receive updates _from_ ServiceNow, you must configure a **Business Rule** that pushes data to OptiTech.
...
5. Click **Submit**.
6. **Crucial:** Re-open the record and note the **API Name** field (e.g., `x_12345_optitech_sync` or just `OptiTech_Sync`). You will need this for the script.
...
{% hint style="warning" %}
Replace `'OptiTech_Sync'` in line 4 with the **API Name** you noted in Step 2.1.
{% endhint %}
...
(function executeRule(current, previous) {
    ...
    try {
        ...
        // REPLACE 'OptiTech_Sync' with your actual API Name if different!
        var r = new sn_ws.RESTMessageV2('OptiTech_Sync', 'POST_Event');
        ...
        if (httpStatus < 200 || httpStatus >= 300) {
            gs.error("OptiTech Sync Failed: " + httpStatus + " " + response.getBody());
        }

    } catch (ex) {
        gs.error("OptiTech Sync Error: " + ex.message);
    }
...
Once the integration is configured and enabled, OptiTech will start synchronizing applied controls with ServiceNow records.
...
* A link to the ServiceNow record will be displayed on the applied control page in OptiTech.

The synchronization is automatic. Any update on an applied control in OptiTech will be reflected in the corresponding ServiceNow record.
...
* **On applied control creation:**
  ...
  * This will create a ServiceNow record on the board specified in OptiTech's ServiceNow integration settings. This record will then be linked to the applied control.
...
1. Create a new Control in OptiTech. Check ServiceNow to see if the record appears.
2. Update the **State** of that record in ServiceNow. Refresh OptiTech to see the status change.

#### Common Errors

| **Symptom**                         | **Cause**       | **Solution**                                                                                                                        |
| ----------------------------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **HTTP 401/403 in ServiceNow Logs** | Secret Mismatch | Ensure the `X-OptiTech-Secret` header in ServiceNow matches the Webhook Secret in OptiTech exactly.                               |
| **"RESTMessageV2 is not defined"**  | Scope Issue     | Ensure the script uses the correct **API Name** for the REST Message (e.g., `x_scope_optitech_sync`).                                   |
| **OptiTech doesn't update**   | Missing Filter  | Check the Business Rule filter conditions. Ensure the change you made (e.g., just changing a description) is covered by the filter. |
