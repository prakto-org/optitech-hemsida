...
The Jira integration allows you to synchronize applied controls from OptiTech with issues in your Jira projects. This helps you track the implementation and status of your controls directly in Jira, leveraging your existing workflows.

When the integration is active, OptiTech will allow linking applied controls to Jira issues, or creating new issues when creating an applied control. Updates to the applied control in OptiTech will be reflected in the corresponding Jira issue, and vice-versa.
...
To configure the integration, you will need to perform steps on both the Jira and OptiTech sides.
...
6. Give your token a descriptive label, for example, "OptiTech Integration".
7. Copy the generated API token. You will not be able to see it again. Store it in a safe place. You will need it to configure OptiTech.

<figure><img src="../../.gitbook/assets/image (39).png" alt=""><figcaption></figcaption></figure>

#### 2. Configure OptiTech

1. Log in to your OptiTech instance.
...
The **Synchronization mappings** section tells OptiTech which Jira project to sync against, and how its applied control fields map to your Jira issue fields.
...
`Status` and `Priority` use enumerated values on both sides, so they need explicit value-by-value mappings. The blocks under **Value Mapping** appear automatically for each choice field that has a field-mapping row set, listing every OptiTech value (e.g. `To do`, `Active`, `P1`...) on the left and asking for the matching Jira value on the right.

Rows left empty behave differently per field at sync time:

* **Status (empty row):** on push, OptiTech skips the status transition for that local value. On pull, the local status stays unchanged.
...
For both, the recommendation is the same: map every local value you want OptiTech to keep in sync.
...
> **SaaS only.** By default, the OptiTech API on SaaS is reachable only from within the pod, so Jira's webhook calls cannot reach it. To enable incoming sync, request ingress whitelisting for your tenant by contacting OptiTech support at [info@optitech-sverige.se](mailto:info@optitech-sverige.se). Unless you ask for additional IPs, only Jira's published IP ranges will be allowed through.
...
4. Set the **URL** to `<API_URL>/api/integrations/webhook/<INTEGRATION_CONFIG_ID>/`. The exact value is shown at the bottom of OptiTech's Jira integration page once the outgoing sync configuration has been saved (**Extra > Settings > Integrations > Jira**). If you don't see the URL, finish the outgoing sync form and press **Save** first.
5. Generate a webhook secret and copy it. You'll paste it into OptiTech in the next step.
...
#### 2. Configure on OptiTech

1. On the Jira integration page in OptiTech, paste the **Webhook Secret** you generated in the previous step into the corresponding field.
2. Save the integration configuration.

## What happens once configured

Once the integration is enabled, OptiTech will start synchronizing applied controls with Jira issues against the configured target table (project + issue type).
...
* A link to the Jira issue is shown on the applied control page in OptiTech.

The synchronization is automatic. Any update on an applied control in OptiTech is pushed to the corresponding Jira issue; any update on the Jira issue side fires a webhook to OptiTech.
...
OptiTech pushes to Jira on the following local events:

* Creating an applied control (with `Create remote object` checked, or after manually linking).
* Updating an applied control.

OptiTech reacts to the following Jira webhook events on incoming sync:
...
* Jira Cloud's REST API is enabled by default. On Jira Server / DC, make sure the REST API is reachable from OptiTech's network. SaaS customers can contact support if outbound access is restricted.
