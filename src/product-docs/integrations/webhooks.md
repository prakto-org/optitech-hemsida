...
Webhooks allow OptiTech to notify external systems in real-time when specific events occur within your GRC platform. Instead of polling our API every few minutes to check for changes, we will push data to you as soon as it happens.
...
You can subscribe to events at a granular level. Currently, OptiTech supports the following events:
...
OptiTech adheres to the [Standard Webhooks v1.0.0](https://www.standardwebhooks.com/) [specification](https://github.com/standard-webhooks/standard-webhooks/blob/main/spec/standard-webhooks.md). We prioritize security to ensure that the data sent to your systems is authentic and has not been tampered with.
...
1. Authenticity: The message definitely came from OptiTech.
...
2.  Create an Endpoint: In the OptiTech UI, navigate to Settings > Webhooks.<br>
...
If your receiving endpoint is behind a corporate firewall, you may need to allowlist the IP address of the OptiTech server.
