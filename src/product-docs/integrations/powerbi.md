---
description: >-
  Build your own Power BI reports on top of OptiTech: install the native
...
Both files are provided with each OptiTech Power BI connector release. Pick one path:
...
Copy-Item .\OptiTech.pqx $dir -Force
```
{% endhint %}
...
2. In Power BI Desktop: **Get Data** → search "OptiTech" (category *Online Services*) → enter the URL you use in your browser, e.g. `https://optitech.example.com` (no `/api` suffix).
...
* **"OptiTech rejected your Personal Access Token"** — the token expired or was revoked. Generate a new one and update it in *Data source settings*.
