...
A **collection** is a hand-curated bundle of GRC objects — audits, risk studies, evidences, policies, exceptions — grouped under a single name. It's the building block OptiTech uses for "scope" wherever scope matters: a project's perimeter, an accreditation's evidence pack, or any custom roll-up you want to query as a unit.
...
A collection is a thin envelope around nine many-to-many relationships. Anything in OptiTech that can live in a project's scope can sit inside a collection:
