...
A **domain** is a top-level container in OptiTech. It represents an organizational scope — a business unit, a subsidiary, or any boundary you want to manage access and reporting around.
...
A domain can have a **parent domain** (`parent_folder` internally) — that's how you build a tree of sub-domains beneath a top-level one. The hierarchy lets you mirror the shape of your organization in the platform: a "Group" domain on top, "Region" or "Subsidiary" domains beneath, "Business unit" or "Program" domains beneath those, and so on.
...
{% hint style="info" %}
You can nest domains to whatever depth you need to mirror your organization. The **Parent domain** field on the domain form places a domain under another; leave it empty to keep the domain at the root.
{% endhint %}

The root is reserved for global, cross-organization objects (built-in catalogs, the global library) — your own domains always live one level under it, even when no other hierarchy is in place.

## Organizational-only domains — the "Create IAM groups" flag

Not every domain needs to be an IAM boundary. Sometimes you want a domain purely as an **organizational container** — a folder in the tree to group related work — without the platform spinning up the per-role user groups that come with a "real" scoping domain.
...
The hierarchy is **not frozen at creation**. Editing a domain lets you change its **Parent domain**, which moves the whole sub-tree — the domain itself and every domain beneath it — under the new parent. Use this to:

- Reorganize as your business changes (a program becomes a subsidiary, two business units merge).
...
Almost every operational object in OptiTech is bound to a domain: assessments and audits, applied controls, evidences, risk scenarios, assets, tasks, policies, findings, incidents, exceptions, contracts, entities, and so on. The domain a record lives in is what drives who can see it and how it rolls up in reports.

Because reorganizations happen, the domain assignment is **not permanent**:
