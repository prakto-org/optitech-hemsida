...
- **Linked** — a pointer to a document that lives in another system (Confluence, SharePoint, a DMS, …). The document carries the URL through the same lifecycle; readers open it in place rather than reading content inside OptiTech.
...
When you link one document to another from inside the editor (**Link to document**), OptiTech records the edge automatically. Each document then shows:
