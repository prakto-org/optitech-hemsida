---
description: Organization-defined labels that override the platform's defaults
---

# Terminology

OptiTech ships with a default vocabulary for the values that appear in dropdowns and badges across the UI — risk origin types in EBIOS RM, project status values, qualifications on incidents, accreditation status labels, metric units, and so on. The **terminology** layer lets each organization override those defaults to match its own internal vocabulary, without changing the platform's data model.

## Why it exists

Two organizations can use OptiTech against the same framework and the same methodology, but speak entirely different internal languages. One team's "Risk Origin: state-actor" is another team's "Adversary: nation-state". The terminology surface lets you reshape the labels without forking the platform.
...
For each field path, the platform ships a **built-in** set of entries (state, organized crime, terrorist, activist, … for risk origins, for example). You can:

- **Hide** built-in entries you don't want to surface — the `is_visible` flag controls dropdown inclusion.
- **Add** organization-specific entries alongside the built-ins.
...
Terminology entries live in the root folder by default, meaning they apply organization-wide. Built-in entries cannot be deleted; they can only be hidden.
...
- [Custom roles](../configuration/organization/custom-roles.md) — a different kind of organization-defined override
