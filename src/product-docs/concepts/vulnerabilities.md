...
A **vulnerability** is a weakness in a system, process, or product that could be exploited to compromise confidentiality, integrity, or availability. OptiTech tracks vulnerabilities as first-class objects, separately from the **incidents** they may cause and the **risk scenarios** they feed into.
...
Vulnerabilities don't live in isolation. OptiTech can pull from external feeds to enrich them automatically — see [Security intelligence feeds](../configuration/settings/sec-intel-feeds.md):
