---
description: One toolbar, everywhere — search, filter, sort, choose columns, and act on rows (single or in bulk) on every model table in OptiTech
---

# Working with tables

Almost every list in OptiTech — applied controls, risk scenarios, assets, evidences, audits — is a **model table** built from the same component. That means the toolbar and gestures are identical wherever you go: learn them once and they work everywhere.
