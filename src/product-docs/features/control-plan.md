...
The **Control Plan** is the calendar-style heat-map of your recurrent operational work. Each cell is a task occurrence — color-coded by status — laid out across time, so a quarter's worth of weekly access reviews or monthly backup tests is visible as one strip. It's the answer to _"are we actually running our recurring controls on cadence?"_

## Where to find it

Sidebar → **Control Plan** (under the Overview group).

The page is gated by the `control_plan` feature flag, **default on** — so it ships visible unless an admin disables it.
...
- **Cells** — one per occurrence, color-coded by status:
