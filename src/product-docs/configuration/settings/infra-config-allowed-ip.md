---
description: >-
  Control which IP addresses and CIDR ranges are allowed to reach the OptiTech backend. Administrators manage the allowlist from the settings UI,
...
* **OptiTech SaaS** — already enabled. The **Infrastructure** tab is
...
own (the automatic 10-minute reconciliation mentioned above is specific to OptiTech SaaS). You are responsible for consuming the endpoint and applying the
