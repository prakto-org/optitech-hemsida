...
The **Library builder** is the single place to author library content in OptiTech. It lives at **`/experimental/library-builder`** and replaces the earlier per-object builders (framework builder, matrix editor, preset editor): they are now surfaces _inside_ one builder rather than separate tools.
