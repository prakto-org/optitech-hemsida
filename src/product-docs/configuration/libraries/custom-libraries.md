...
{% hint style="warning" %}
**NOTE:** This section is still under reworking.
{% endhint %}

This documentation explains how to create, maintain and evolve **custom libraries** for OptiTech using **Excel** and **YAML**.

***

## 1. What is a Library in OptiTech?

A **library** is a container that bundles one or more governance objects that can be imported into OptiTech.
...
OptiTech supports deep hierarchies, but **depths above 6 are strongly discouraged** for readability.
...
{% step %}
### Import the library into OptiTech

Upload and validate the generated YAML in OptiTech.
{% endstep %}
...
{% hint style="warning" %}
This section doesn't explain all the object types yet, neither the type of values to put in the columns.
{% endhint %}
...
{% step %}
If you would like to see what this example looks like directly in OptiTech, convert it using the standard conversion tool:
...
{% hint style="warning" %}
Versioning is critical. If the custom framework is already imported into OptiTech and the version number has not been incremented, OptiTech will not suggest an update.
{% endhint %}
...
The visual rendering (orientation, colors, layout) is handled by the OptiTech interface.
...
All tools described below are part of the OptiTech library tooling.

These tools are designed to:

* Reduce human errors
* Enforce the OptiTech data model
...
**Tool:** `convert_library_v2.py`

This is the **most frequently used tool**.

It converts one or more Excel files into **OptiTech-compatible YAML libraries**.

#### Typical use cases

* Generating a YAML file from an Excel framework
* Validating Excel consistency
* Importing libraries into OptiTech
...
{% hint style="danger" %}
If you use `convert_library_v2.py` on an Excel file converted into v2, the YAML structure may be completely different from your original YAML file and break your audits if you import it into OptiTech!

You can use the `convert_v1_to_v2.py` script if you created your framework using the v1 format and provided that you **HAVE NOT YET** imported your framework into OptiTech.
...
## 7. Importing a Custom Library into OptiTech

Once your Excel file has been successfully converted into a YAML file, the final step is to **import the library into OptiTech**.

This operation is performed directly from the OptiTech interface and does not require any additional tooling.
...
If the import fails, OptiTech will display an explicit error message.
...
If the version is unchanged, OptiTech will ignore the update.
...
For advanced or low-level options, use the `--help` flag on any of the conversion tools described above.
