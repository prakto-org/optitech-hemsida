...
By default, logging out of OptiTech only ends the local OptiTech session. The identity provider session stays open, so clicking **Log in with SSO** again signs the user straight back in without re-authenticating.

To also close the identity provider session on logout, enable **Enable service provider-initiated single logout** in the SSO settings. When it is on, logging out of OptiTech redirects the browser through the identity provider's logout endpoint.

This is _service provider-initiated_ single logout: OptiTech asks the identity provider to end **its own** session for the user. Whether that in turn signs the user out of other applications federated to the same identity provider depends on the identity provider's single-logout configuration and is not something OptiTech controls or can guarantee.
