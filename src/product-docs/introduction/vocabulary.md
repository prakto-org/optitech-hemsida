...
A glossary of the terms used in OptiTech. Where a user-facing term differs from the internal model name, both are given.
...
- **Inherent risk** — The natural risk level of a scenario _without any applied controls_. The top tier of OptiTech's three-tier model — useful for ranking scenarios by their underlying severity, independently of the mitigation already in place. Surfaced in the UI when the `inherent_risk` feature flag is on.
...
- **Residual risk** — The risk level expected once all _planned_ applied controls have been implemented — the target state of the action plan. The bottom tier of OptiTech's three-tier model (inherent → current → residual), and the figure used as input to risk-acceptance decisions.
...
- **Webhook endpoint** — A registered URL OptiTech calls when configured events happen (e.g. an audit closed, an applied control updated). Used to notify external systems and trigger downstream automation.
