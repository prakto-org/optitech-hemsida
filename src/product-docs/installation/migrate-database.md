---
description: >-
  You think it is time to change or try a new database on OptiTech? You
...
To stay safe, we strongly recommend creating a backup of the volume, disk, or `db/` folder used to host your OptiTech instance.

We also recommend testing your backup procedure by deploying another OptiTech instance and restoring the backup into it. This will help verify that your backups work correctly before you start switching database engines.
...
{% hint style="warning" %}
The new PostgreSQL-backed instance must run the **exact same OptiTech version** as the source SQLite instance. The restore will refuse a backup produced by a different version. Migrate first, then upgrade afterwards if needed.
{% endhint %}
...
### 2. Stop the instance of OptiTech
...
You can install whichever PostgreSQL setup you prefer, as you only need the environment variables afterwards to connect it to OptiTech. Here are the two main ways:
...
```
POSTGRES_DB=optitech
POSTGRES_USER=optitechuser
...
db:
  ...
  healthcheck:
    test: ["CMD-SHELL", "pg_isready -U optitechuser -d optitech"]
    interval: 10s
    timeout: 5s
    retries: 10
    start_period: 50s
...
**Option B — Cross-host migration, or you want one atomic snapshot.** Use the `clica` CLI shipped under `cli/` instead of the UI export. It packages the database **and** every attachment in a single backup, then restores both in one atomic call:
...
3. Remove the PostgreSQL environment variables (`POSTGRES_NAME`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `DB_HOST`, `DB_PORT`) and the `db` service from your `docker-compose.yml`. With `POSTGRES_NAME` unset, OptiTech falls back to SQLite at `db/optitech.sqlite3`.
...
The same version-match rule applies: the SQLite-backed instance must run the exact same OptiTech version as the PostgreSQL-backed source.
