...
The chart lives at `oci://<optitech-registry>/optitech`.
...
2. **Customise** `custom.yaml`. The most important settings:
   ...
   The chart's `values.yaml` carries the full values table — refer to it for every key with its default and description.
...
4. **Create a namespace and install**:
   ...
   ```sh
   kubectl create ns optitech
   helm install optitech-release oci://<optitech-registry>/optitech \
     -f custom.yaml \
     -n optitech
   ```
...
```sh
kubectl get pods -n optitech
kubectl get ingress -n optitech
```
...
- The Qdrant collection and the indexes are **not** created automatically. After the pods are up, run the indexing commands once from the backend pod (`init_qdrant` creates the collection, `index_objects` indexes your existing risk/control/asset records, `index_libraries` indexes the framework libraries):
  ...
  ```sh
  POD="deploy/optitech-release-backend"
  kubectl exec -n optitech $POD -c backend -- uv run python manage.py init_qdrant
  kubectl exec -n optitech $POD -c backend -- uv run python manage.py index_objects
  kubectl exec -n optitech $POD -c backend -- uv run python manage.py index_libraries --sync
  ```
...
```sh
kubectl create secret generic my-ca -n optitech --from-file=ca.crt=./internal-ca.crt
```
...
Replace `optitech-release` below with your Helm release name.
...
networkPolicy:
  ...
  policyTypes:
    - Ingress
  ingress:
    # internal traffic between OptiTech pods (frontend -> backend, backend -> qdrant)
    - from:
        - podSelector:
            matchLabels:
              app.kubernetes.io/instance: optitech-release
...
```sh
helm upgrade optitech-release oci://<optitech-registry>/optitech \
  -f custom.yaml \
  -n optitech
```
...
```sh
helm uninstall optitech-release -n optitech
```

PostgreSQL persistent volumes are **not** deleted automatically when using the bundled subchart. Inspect with `kubectl get pvc -n optitech` and remove manually if you want a clean slate.

## Legacy

> **Note:** Older Helm charts are no longer maintained. The values shape changed substantially between the legacy chart and the current one (`frontendOrigin` → `global.domain`, no `clientName`/`clusterDomain` shorthand, separate ingress block). New installs should use the OCI registry above.
